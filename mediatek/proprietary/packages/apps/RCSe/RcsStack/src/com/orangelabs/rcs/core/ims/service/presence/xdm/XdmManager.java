...
public class XdmManager {
    ...
    //WITH THE LATEST PATHCES OF GBA, HTTPURLCONNECTION ALSO SUPPORTS GBA
           ...
           try{
               ...
               // Set the connectivity manager
                   ...
                   if(activeNetwork != null) {
                       NetworkCapabilities nc = connectivityMgr.getNetworkCapabilities(activeNetwork);
                       if (logger.isActivated()) {
                           logger.debug("NetworkCapabilities for n/w passed to GBA mdoule are : "+ nc);
                       }
                       
                       if(isGBAxcapAuthenticationType)
                       {
                       //include GBA support
                       GbaHttpUrlCredential gbaCredential = new GbaHttpUrlCredential(AndroidFactory.getApplicationContext(), nafAddress);
     ...
     private HttpResponse sendApacheHttpRequestToXDMS(HttpRequest request)
        {
         ...
         try{
                ...
                DefaultHttpClient client = new DefaultHttpClient(cm, params);
                            ...
                            /*String nafAddress = "";
                nafAddress = xdmServerAddr;
                
                //for GBA
                CredentialsProvider credProvider = new BasicCredentialsProvider();
                credProvider.setCredentials(new AuthScope(AuthScope.ANY_HOST, AuthScope.ANY_PORT), new GbaCredentials(AndroidFactory.getApplicationContext(), nafAddress));
                ...
                */
                ...
                org.apache.http.HttpResponse response = executeRequest("http", requestUri, client, localContext,request);
     ...
     private HttpResponse convertResponsetoXDMFormat(org.apache.http.HttpResponse response)throws IOException{
         ...
         if(response !=null){
             ...
             //add content
             xdmResponse.setContent(EntityUtils.toByteArray(response.getEntity()));
     ...
     private  org.apache.http.HttpResponse executeRequest(String protocol, String request, DefaultHttpClient client, HttpContext localContext,HttpRequest xdmHttpRequest) {
     
         if(xdmHttpRequest.getMethod().equalsIgnoreCase("GET")){
             return executeGetRequest( protocol,  request,  client,  localContext, xdmHttpRequest);
         }else if(xdmHttpRequest.getMethod().equalsIgnoreCase("PUT")){
             return executePutRequest( protocol,  request,  client,  localContext, xdmHttpRequest);
         }else if(xdmHttpRequest.getMethod().equalsIgnoreCase("DELETE")){
             return executeDeleteRequest( protocol,  request,  client,  localContext, xdmHttpRequest);
         }
         else{
             return null;
         }
     ...
     HttpResponse executeHttpConnection(Network network, HttpRequest request)
     {
         if (logger.isActivated()) {
         ...
         try{
            ...
            if ((folder != null) && (folder.getEntry() != null)
             ...
             resp = convertHTTPUrlResponseToXDMFormat(con);
        
             //update etag
             if ((resp.getHeader("etag") != null) && (folder != null) && (folder.getEntry() != null)) {
                  folder.getEntry().setEtag(resp.getHeader("etag"));
              }
            
         }catch(Exception e){
             logger.info( "xdm connection error : " + e.getMessage());
             return null;
         }finally{
             if(con!=null)
             con.disconnect();
     ...
     private HttpResponse convertHTTPUrlResponseToXDMFormat(HttpURLConnection conn){
         HttpResponse xdmResponse = null;
         ...
         if(conn != null)
         {
             try{
                  ...
                  *   {null=[HTTP/1.1 200 OK], 
                        authentication-info=[nextnonce ="78ffff7cffffff225dffffffffffff57", 
                        qop="auth", rspauth="54b38f0ae3a8614ff359293c2f96f2c1", 
                        cnonce="ce908499f83f6c9d", 
                        nc=00000001], 
                        connection=[close], 
                        content-type=[application/resource-lists+xml],
                        date=[Fri, 18 Sep 2015 06:48:39 GMT], 
                        etag=[MAV_13], 
                        server=[Mavenir Application Server], 
                        Vary=[Accept-Encoding],
                        X-Android-Received-Millis=[1442558918521], 
                        X-Android-Response-Source=[NETWORK 200],
                         X-Android-Sent-Millis=[1442558918029]}
                  * 
                  * 
                  */
             ...
             {
                 logger.info( "exception at XDM conversion " + err.getMessage());
                 return null;
             }
     ...
     private  org.apache.http.HttpResponse executeGetRequest(String protocol, String request, DefaultHttpClient client, HttpContext localContext,HttpRequest xdmHttpRequest) {
         try {
             ...
             //String identity = "\""+RcsSettings.getInstance().getUserProfileImsUserName_full()+"\"";
             String identity = "\""+xdmServerLogin+"\"";
             put.addHeader("X-3GPP-Intended-Identity", identity); //add user agent
             
             
            /* if (xdmHttpRequest.getContent() != null) {
                 // Set the content type
                 put.setHeader("Content-Type", xdmHttpRequest.getContentType());
                 put.setEntity(new StringEntity(xdmHttpRequest.getContent(),"UTF-8"));
             } else {
                  put.setHeader("Content-Type", ""+0);
             }*/
            ...
            //  CredentialsProvider credProvider = new BasicCredentialsProvider(); 
              ...
              URL url1 = new URL(nafAddress);
               GbaHttpUrlCredential gbaCredential = new GbaHttpUrlCredential(AndroidFactory.getApplicationContext(), url1.getHost());
               Authenticator.setDefault(gbaCredential.getAuthenticator());
             org.apache.http.HttpResponse response = client.execute(put, localContext);
     ...
     private  org.apache.http.HttpResponse executeDeleteRequest(String protocol, String request, DefaultHttpClient client, HttpContext localContext,HttpRequest xdmHttpRequest) {
         try {
             ...
             //String identity = "\""+RcsSettings.getInstance().getUserProfileImsUserName_full()+"\"";
             String identity = "\""+xdmServerLogin+"\"";
             deleteReq.addHeader("X-3GPP-Intended-Identity", identity); //add user agent
             
             
             if (xdmHttpRequest.getContent() != null) {
                 // Set the content type
                 deleteReq.setHeader("Content-Type", xdmHttpRequest.getContentType());
                 //deleteReq.setEntity(new StringEntity(xdmHttpRequest.getContent(),"UTF-8"));
             } /*else {
                  put.setHeader("Content-Type", ""+0);
             }*/
          ...
          org.apache.http.HttpResponse response = client.execute(deleteReq, localContext);
     ...
     private  org.apache.http.HttpResponse executePutRequest(String protocol, String request, DefaultHttpClient client, HttpContext localContext,HttpRequest xdmHttpRequest) {
         try {
    ...
    public void initialize() {
        ...
        try {
            ...
            if (logger.isActivated()) {
                logger.debug("xcap AUthentication type :"+isGBAxcapAuthenticationType+" ; USER_AGENT : "+USER_AGENT);
            }
            ...
            if ((response != null) && response.isSuccessfullResponse()) {
                ...
                if ((folder == null) || (folder.getEntry() == null)) {
                    ...
                    /*HttpResponse res =  getResourcesList();
                    String PresRules = new String(res.getContent());
                    if (logger.isActivated()) {
            ...
            else if((response != null) && (response.getResponseCode()==404)){ //as per Operator_OP08 req
    ...
    public HttpResponse setRcsList() {
        try {
            ...
            String resList = xdmServerAddr + "/resource-lists/users/"
                    + (user)
...
/*                    +
    ...
    private boolean setUpDefaultNetwork() 
    {
        ...
        {
            try {
                mActiveNetworkConnection = new AndroidDefaultNetwork(AndroidFactory.getApplicationContext());
