...
public abstract class ImsNetworkInterface {
    ...
    public boolean register(int currentPCscfAddress) {
        ...
        if (RcsSettings.getInstance().supportOP01() && (registered == true)){
    ...
    public boolean volteRegister(int currentPCscfAddress){
        ...
        try {
            ...
            if (logger.isActivated()) {
                logger.debug("sipStackListeningPort for listening from RCS_UA is : " + sipStackListeningPort);
            }
           
            //set the RCS SIP stack listening port
           RcsUaAdapter.getInstance().setRCSSipStackPort(sipStackListeningPort);
    ...
    public void volteUnregister(){
        ...
        try{
          //unregister
        unregStatus = ((RcsProxyRegistrationHandler)RcsUaAdapter.getInstance().getRegistrationEventDispatcher()).unregister();    
        
         RcsUaAdapter.getInstance().terminate();
