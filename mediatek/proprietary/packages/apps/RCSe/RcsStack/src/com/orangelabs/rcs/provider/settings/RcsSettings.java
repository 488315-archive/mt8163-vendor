...
*/
 ...
 /**
    ...
    public boolean isSupportOP08() {
        ...
        int subId=(int)SubscriptionManager.getDefaultDataSubId();
        TelephonyManager tm = (TelephonyManager)mContext.getSystemService(Context.TELEPHONY_SERVICE);
        String currentOperator = tm.getSimOperatorNameForSubscription(subId);
        
        //compare the operator name
        if(currentOperator.equals(OP08_OPERATOR_NAME)){
            result = true;
        }
...
/**
    ...
    public Capabilities getMyCapabilities(){
        ...
        if(RcsUaAdapter.getInstance()!=null)
        {
        //if registred via single registartion, ir94 voice call is enabled
        capabilities.setIR94_VoiceCall(RcsUaAdapter.getInstance().isRegistered());
        capabilities.setIR94_VideoCall(RcsUaAdapter.getInstance().isRegistered() && isIR94VideoCallSupported());
        capabilities.setIR94_DuplexMode(RcsUaAdapter.getInstance().isRegistered() && isIR94VideoCallSupported());
        }
    ...
    public boolean isFallbackToPagerModeSupported() {
        return RcsSettings.getInstance().isSupportOP08() && RcsSettings.getInstance().isCPMPagerModeSupported();
    }
...
public int getCurrentPCscfAddresCounter() {
    ...
    */
    ...
    public boolean isGBASupported() {
        ...
        String optr = SystemProperties.get("ro.operator.optr");
        if(optr.equalsIgnoreCase("op01")||optr.equalsIgnoreCase("op08")) {
    ...
    public boolean isIR94VideoCallSupported(){
        ...
        if(isIR94VideoCallSupportedConfig() && isVilteSupportedPlatform() && isVilteSwitchEnabled()){
    ...
    public boolean isIR94VideoCallSupportedConfig(){
        //read from system property "ro.mtk_vilte_support"
        boolean result = false;
        if (instance != null) {
