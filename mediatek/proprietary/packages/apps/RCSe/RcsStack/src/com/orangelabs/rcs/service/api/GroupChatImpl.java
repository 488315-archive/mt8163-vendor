...
public class GroupChatImpl extends IGroupChat.Stub implements ChatSessionListener {
    ...
    public void quitConversation() {
        if (logger.isActivated()) {
            logger.error("GCM Cancel session");
        }
     ...
     * @return Message ID
     */
    public String sendMessageEx(final String text, int msgType) {
        if (logger.isActivated()) {
            logger.info("GCM sendMessage:" + text);
        }
        // Generate a message Id
        ...
        if (RcsSettings.getInstance().supportOP01()) {
            ...
            if (!RichMessagingHistory.getInstance().isOne2OneMessageExists(msgId)) {
                ...
                RichMessagingHistory.getInstance().addGroupChatMessage(
                        session.getContributionID(), msg, ChatLog.Message.Direction.OUTGOING);
            }
        }
    ...
    public void handleSessionAborted(int reason) {
        synchronized(lock) {
            if (logger.isActivated()) {
                logger.error("GCM Session aborted (reason " + reason + ")");
            }
    ...
    public void handleSessionTerminatedByGroupRemote(String cause, String text){
        ...
        synchronized(lock) {
            if (logger.isActivated()) {
                logger.error("GCM handleSessionTerminatedByGroupRemote Session terminated cause=" + cause + ",text ="+ text);
            }
        ...
        synchronized(lock) {
            ...
            for (int i=0; i < N; i++) {
                try {
                    ...
                    listeners.getBroadcastItem(i).onNewExtendMessage(msgApi);
                } catch(Exception e) {
                    if (logger.isActivated()) {
                        logger.error("Can't notify listener", e);
                    }
                ...
                try {
                    ...
                    switch(error.getErrorCode()) {
                        ...
                        case ChatError.SESSION_FORBIDDEN_ERROR:
                            ...
                            code = GroupChat.Error.INVITATION_FORBIDDEN;                          
                            break;
                        default:
                            code = GroupChat.Error.CHAT_FAILED;
                    }
    ...
    public void handleMessageDeliveryStatus(String msgId, String status, String contact) {
        synchronized(lock) {
            if (logger.isActivated()) {
                logger.error("New message delivery status for message " + msgId + ", status " + status + "Contact" + contact);
            }
    ...
    public void handleMessageDeliveryStatus(String msgId, String status, String contact, int errorCode, String statusCode) {
        synchronized(lock) {
            if (logger.isActivated()) {
                logger.error("New message delivery status for message " + msgId + ", status " + status + "Contact" + contact + ", errorcode:" + errorCode);
            }
