...
public class SipInterface implements SipListener {
   ...
   // private boolean sipTraceEnabled = RcsSettings.getInstance().isSipTraceActivated();
    ...
    /**
     * SIP listening Port : request Creation
     */
    private int listeningPortSIPMessage;
    ...
    public SipInterface(String localIpAddress, String proxyAddr,
        ...
        if(RcsSettings.getInstance().isSupportOP08()){
            ...
            //listeningPortSIPMessage is for composeing the message. the prot should be 5060.
            //volte module will not expose the outgoing port and will replace the 5060 in the SIP request with the actual ports
        //initialize the listening port
       listeningPortSIPMessage = 5060;
        
        this.defaultProtocol = ListeningPoint.TCP;
        }
        
        if (logger.isActivated()) {
            logger.debug("SIP stack started at " + localIpAddress + ":" + listeningPort + " ; listeningPortSIPMessage = " +listeningPortSIPMessage);
        }
    }
     ...
     * @return Port number
     */
    public int getListeningPortSIPMessage() {
       // return listeningPort;
        if(!RcsSettings.getInstance().isSupportOP08()){
            return listeningPort;
        } else {
        return listeningPortSIPMessage;
    }
    }
    ...
    public ContactHeader getLocalContact() throws Exception {
       ...
       /**
        ...
        contactURI.setPort(getListeningPortSIPMessage());
        contactURI.setParameter("transport", defaultProtocol);
