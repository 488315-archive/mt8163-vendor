...
public class FileTransferImpl extends IFileTransfer.Stub implements FileSharingSessionListener {
    ...
    public int getState() {
        ...
        if (session instanceof HttpFileTransferSession) {
            ...
            if (state == HttpTransferState.PAUSED) {
                // Session paused
                result = FileTransfer.State.PAUSED;
            } else
    ...
    public void addEventListener(IFileTransferListener listener) {
        if (logger.isActivated()) {
            logger.info("FTS Add an event listener");
        }
    ...
    public void removeEventListener(IFileTransferListener listener) {
        if (logger.isActivated()) {
            logger.info("FTS Remove an event listener");
        }
    ...
    public void handleSessionAborted(int reason) {
        synchronized(lock) {
            ...
            final int N = listeners.beginBroadcast();
            if (logger.isActivated()) {
                logger.info("FTS Session aborted N:" + N);
            }
            for (int i=0; i < N; i++) {
                try {
        ...
        synchronized(lock) {
            ...
            // Check if the file has been transferred or not
              if (session.isFileTransfered()) {
                ...
                final int N = listeners.beginBroadcast();
                if (logger.isActivated()) {
                    logger.info("FTS Session terminated by remote  N:" + N);
                }
                for (int i=0; i < N; i++) {
                    try {
                        listeners.getBroadcastItem(i).onTransferAborted();
    ...
    public void handleTransferError(FileSharingError error) {
        synchronized(lock) {
            ...
            final int N = listeners.beginBroadcast();
            if (logger.isActivated()) {
                logger.info("FTS Session error  N:" + N);
            }
            for (int i=0; i < N; i++) {
                try {
                    int code;
                    switch(error.getErrorCode()) {
                        ...
                        case FileSharingError.MEDIA_RESUME_FAILED:
                            code = FileTransfer.Error.TRANSFER_RESUME;
                            break;
                        default:
                            code = FileTransfer.Error.TRANSFER_FAILED;
                    }
    ...
    public void handleTransferProgress(long currentSize, long totalSize) {
        synchronized(lock) {
            ...
            if(session.isFileTransferPaused()){
                currentSize += FileTransferServiceImpl.getPauseInfo(session.getSessionID()).bytesTransferrred;
                if (logger.isActivated()) {
