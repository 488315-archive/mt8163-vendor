...
public abstract class ImsServiceSession extends Thread {
    ...
    public boolean isSecureProtocolMessage(){
        return getImsService().getImsModule().getCurrentNetworkInterface().getIsSecureProtocol() && !RcsSettings.getInstance().isSupportOP08();
        }
    ...
    public void receiveBye(SipRequest bye) {
        ...
        if (this instanceof GroupChatSession){
            ...
            for(int i=0; i < getListeners().size(); i++) {
                ((ChatSessionListener)getListeners().get(i)).handleSessionTerminatedByGroupRemote(cause, reasonText);
            }
...
// Send ack
            ...
            if(this instanceof FileSharingSession){
                ((FileSharingSession)this).extractFileRange(resp);
            }
