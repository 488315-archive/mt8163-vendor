...
public abstract class StandaloneChatSession extends ChatSession {
    ...
    public void sendMessage(String msgId, String text, int msgType) {
        ...
        if (contentType.equals(CpimMessage.MIME_TYPE)) {
            if (RcsSettings.getInstance().supportOP01()) {
                content = ChatUtils.buildCpimMessageWithDeliveredImdn(
                        from, to, msgId, StringUtils.encodeUTF8(text), InstantMessage.MIME_TYPE);
            }
            else if (useImdn) {
                ...
                content = ChatUtils.buildCpimMessage(
                        from, to, StringUtils.encodeUTF8(text), InstantMessage.MIME_TYPE);
            }
        } else {
            content = text;
        }
        ...
        // Send content
        boolean result = sendDataChunks(msgId, content, contentType);
        ...
        if (!rmHistory.isMessageExists(msgId)) {
            ...
            rmHistory.addExtendChatMessage(msg, ChatLog.Message.Direction.OUTGOING);
        }
    ...
    protected void setMessageType(int msgType) {
