...
public abstract class GroupChatSession extends ChatSession {
        ...
        if(!RcsSettings.getInstance().supportOP01()) {
            ...
            if (!RichMessagingHistory.getInstance().isOne2OneMessageExists(msgId)) {
                ...
                RichMessagingHistory.getInstance().addGroupChatMessage(
                        getContributionID(), msg, ChatLog.Message.Direction.OUTGOING);
            }
        }
    ...
    public void sendGeolocMessage(String msgId, GeolocPush geoloc) {
        ...
        if (!result) {
            ...
            // Notify listeners
            for(int i=0; i < getListeners().size(); i++) {
                ((ChatSessionListener)getListeners().get(i)).handleMessageDeliveryStatus(msgId, ImdnDocument.DELIVERY_STATUS_FAILED, this.getRemoteContact());
            }
        }
    }
