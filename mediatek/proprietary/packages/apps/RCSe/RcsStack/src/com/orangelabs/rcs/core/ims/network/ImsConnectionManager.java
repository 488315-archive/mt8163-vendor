 ...
 /**
  ...
  /**
    ...
    public ImsConnectionManager(ImsModule imsModule) throws CoreException {
        ...
        rcsUaAdapter = RcsUaAdapter.getInstance();
        
        //start rcs adapter is single registration is feasible
        if(rcsUaAdapter.isSingleRegistrationFeasible()){
...
/**
    ...
    */
    ...
    private synchronized void connectionEvent(Intent intent) {
        ...
        logger.debug("isSingleRegistrationSupported : "+rcsUaAdapter.isSingleRegistrationSupported());
         ...
         */
        if(RcsSettings.getInstance().isSupportOP08() && (connectivityMgr.getActiveNetworkInfo()!=null))
        {
            
                if(rcsUaAdapter.isSingleRegistrationSupported()){
            if(rcsUaAdapter.isRegistered()){
                if (logger.isActivated()) {
                    logger.debug("already registered to volte n/w. ignore ");
                }
                return;
            }
            ...
            if (logger.isActivated()) {
                logger.debug("currentNetworkInterface : " +currentNetworkInterface);
            }
            
		            //update volte details
            ((VolteNetworkInterface)currentNetworkInterface).UpdateVolteDetails();
		            loadUserProfile();
		            
		            //update user profile as per IMS details provided by volte 
		            updateUserProfileAcctoIMS();
            
            //String localIpadd_volte = rcsUaAdapter.getHostAddress();
            ...
            return;
                ...
                else if(!rcsUaAdapter.isSingleRegistrationSupported()){
                    
                    //IN CASE OPER : OP08 BUT SINGLE REGISTERTION NOT SUPPORTED CURRENTLY, THEN RETURN. 
                    //ITS BECAUSE THE OP08 ONLY USES SINGLE REGISTRATION
                    if (logger.isActivated()) {
                        logger.debug("Single registration not supported currently for op08. Ignore Connection event");
                    }
...
/**
            ...
            if (onlyApnNetworkInfo != null) {
                ...
                if (isConnected) {
String    localIpAddr = NetworkFactory.getFactory().getLocalIpAddress(mDnsResolvedFields, onlyApnNetworkInfo.getType());                   
 if (currentNetworkInterface == getOnlyApnNetworkInterface()) {
        ...
        if (networkInfo.isConnected() && (localIpAddr != null)) {
            ...
            String currentOpe = tm.getSimOperatorNameForSubscription(subId);
