...
public class AnonymousFetchRequestTask {
    ...
    private SipRequest createSubscribe() throws SipException, CoreException {
        ...
        if(!RcsSettings.getInstance().isCPMSupported()){
            subscribe = SipMessageFactory.createSubscribe(dialogPath, 0);
            } 
        else{
            if (logger.isActivated()) {
                logger.info("createCpimSubscribe CPMS ");
            }            
            subscribe = SipMessageFactory.createCpimSubscribe(dialogPath, 0);
        }
        */
    ...
    private void sendSubscribe(SipRequest subscribe) throws Exception {
      ...
      //@tct-stack Begin: added by tu kun for IOT case 1_5_1 on 20140430.
               // Set the security header
                if (logger.isActivated()) {
                    logger.debug("==================Begin: Tony SIP Subscribe(AnonymousFetchRequestTask)=================");
                }
                if(authenticationAgent != null){
                    authenticationAgent.setAuthorizationHeader(subscribe);
                }
                if (logger.isActivated()) {
                    logger.debug("==================End: Tony SIP Subscribe(AnonymousFetchRequestTask)=================");
                }
        /*@*/
