...
public class RcsSettings {
	...
	public String readParameter(String key) {
		...
		if (Logger.isActivated()) {
            Logger.d(TAG, " Key before Encryption "+ key );
        }
		if(mIsEncrypted)
		{
	try {
	...
	//long startTime = System.currentTimeMillis();
      ...
      //  long timeTaken=System.currentTimeMillis() - startTime;
       ...
       /* if (Logger.isActivated()) {
            Logger.d(TAG,"Time Taken For query read: Key"+ key +": " + timeTaken + "value: "+result);
        }
*/
        if (Logger.isActivated()) {
            Logger.d(TAG," Result before Encryption "+ result );
        }
        return result;
	}
	...
	public void writeParameter(String key, String value) {
      ...
      /*  if (Logger.isActivated()) {
            Logger.d(TAG,"Time Taken For Write: Key"+ key +": " + timeTaken);
        }*/
	}
 ...
 /**
	...
	private void setServiceStatePreference(boolean registrationState){
		...
		SharedPreferences pref = MediatekFactory.getApplicationContext().getSharedPreferences("gsma.joyn.preferences",Activity.MODE_WORLD_READABLE);
...
/** M: Remove all users profile information @{ */
  ...
  /*  *//**
    ...
    private void initParaInBackground() {
        if (Logger.isActivated()) {
            Logger.d(TAG,"initParaInBackground()");
        }
        AsyncTask.execute(new Runnable() {
            @Override
            public void run() {
                ...
                if (Logger.isActivated()) {
                    Logger.d(TAG,"mChatWallpaper = " + mChatWallpaper);
                }
                for (OnWallPaperChangedListener listener : mOnWallPaperChangedListenerList) {
