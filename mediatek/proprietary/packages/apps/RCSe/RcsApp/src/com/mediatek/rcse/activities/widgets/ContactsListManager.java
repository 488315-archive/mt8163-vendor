...
public final class ContactsListManager {
    ...
    private void getContactList(List<RcsContact> contacts) {
        Logger.d(TAG, "getContactList enter!");
        ...
        Logger.d(TAG, "getContactList me:" + myNumber);
        if (contacts != null && contacts.size() != 0) {
            for (RcsContact contact : contacts) {

                Logger.d(TAG,
                        "getContactList, a new contact, need to add"
                                + contact.mNumber);
                if (!CONTACTS_LIST.contains(contact)) {
                    CONTACTS_LIST.add(contact);
                }
                Logger.d(TAG, "getContactList, CONTACTS_LIST is "
                        + CONTACTS_LIST);
    ...
    private class LoadFinishedTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            if (null!= instance && null != instance.getContactsApi() && null !=instance.getContactsApi()
                    .getImBlockedContactsFromLocal()) {
                try {
    ...
    public void onContactsDbChange() {
        ...
        Cursor cursor = null;
        try{
        if(AppSettings.getInstance().isSupportOP08(mContext)){
        ...
        if (cursor != null && cursor.getCount() != 0) {
            ...
            Logger.w(TAG, "getContactList, cursor is null!");
        }
        if (cursor != null) {
            cursor.close();
            cursor = null;
