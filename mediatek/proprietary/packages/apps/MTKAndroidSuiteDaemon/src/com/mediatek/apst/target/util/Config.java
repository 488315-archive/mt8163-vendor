...
public abstract class Config {
    ...
    public static final boolean MTK_GEMINI_SUPPORT = isGeminiSupport();
    public static final boolean MTK_3SIM_SUPPORT = is3SimSupport();
    public static final boolean MTK_4SIM_SUPPORT = is4SimSupport();

    static boolean isGeminiSupport() {
        boolean MTK_GEMINI_SUPPORT = false;
        int SIM_COUNT = TelephonyManager.getDefault().getPhoneCount();
        if((SIM_COUNT == 1)||(SIM_COUNT == 2)) {
            MTK_GEMINI_SUPPORT = true;
        }
        return MTK_GEMINI_SUPPORT;
    }

    static boolean is3SimSupport() {
        boolean MTK_3SIM_SUPPORT = false;
        int SIM_COUNT = TelephonyManager.getDefault().getPhoneCount();
        if(SIM_COUNT == 3) {
            MTK_3SIM_SUPPORT = true;
        }
        return MTK_3SIM_SUPPORT;
    }

    static boolean is4SimSupport() {
        boolean MTK_4SIM_SUPPORT = false;
        int SIM_COUNT = TelephonyManager.getDefault().getPhoneCount();
        if(SIM_COUNT == 4) {
            MTK_4SIM_SUPPORT = true;
        }
        return MTK_4SIM_SUPPORT;
    }
}
