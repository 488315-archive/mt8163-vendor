...
class GpsNormalMode extends GpsMode {
    ...
    public void onResponse(String response) {
        ...
        if (command == 2) {
            ...
            if (index > 0) {
                mResult.addData(0, System.currentTimeMillis() / 100 - mStartTime);
                mResult.addData(0, snr);
            } else {
                mResult.setAverage(0, snr);
            }
        } else if (command == 3) {
...
class GpsSignalMode extends GpsMode {
    void setSvIds(int[] svIds) {
        ...
        for (int i = 0; i < svIds.length; i++) {
            if (svIds[i] != DesenseAtActivity.INVALID_SV_ID) {
                mSvCount++;
            }
        };
...
//        command += "*";
    ...
    public void onResponse(String response) {
        ...
        if (command == 2) {
            int index = Integer.parseInt(splited[5]);
            for (int i = 0; i < 5; i++) {
                if (mSvIds[i] != DesenseAtActivity.INVALID_SV_ID) {
                    float snr = Float.parseFloat(splited[i + 6]);
                    if (index > 0) {
                        mResult.addData(i, System.currentTimeMillis() / 100 - mStartTime);
                        mResult.addData(i, snr);
                    } else {
                        float snrAverage = mResult.getAverage(i);
                        mResult.setAverage(i, (snrAverage == 0) ? snr : (snrAverage + snr) / 2);
    ...
    public void onResponse(String response) {
        ...
        if (command == 2) {
            ...
            if (index > 0) {
                ...
                float snrAverage = mResult.getAverage(0);
                mResult.setAverage(0, (snrAverage == 0) ? snr : (snrAverage + snr) / 2);
                if (mTestState == 0) {
                    mResult.state = TestResult.ResultState.STATE_REF_DONE;
                    if (mClient != null) {
                        mClient.closeClient();
