...
public class AudioVolumeVoice extends Activity implements OnClickListener {
    ...
    public static final String TAG = "EM/AudioVolumeVoice";
    ...
    private void initComponents() {
        ...
        mBandAdatper = new ArrayAdapter<String>(this,
                android.R.layout.simple_spinner_item, mArraySpinnerBand);
    ...
    private void initTableValue() {
        ...
        Elog.d(TAG, "Init UL gain table: mUlMaxValue " + mUlMaxValue + " mUlMinValue " + mUlMinValue
                + " mUlMaxIndex " + mUlMaxIndex + " mUlValueStep " + mUlValueStep);

        //Init Stf gain table value
        strMaxValue = AudioTuningJni.getParams(CATEGORY_VOLUME, PARAM_COMMON, PARAM_STF_VALUE_MAX);
        strValueStep = AudioTuningJni.getParams(CATEGORY_VOLUME, PARAM_COMMON, PARAM_STF_STEP);
        String strIndexList = AudioTuningJni.getChecklist(CATEGORY_SPEECHVOL,
                STF_GAIN, PARAM_STF_LIST);
        ...
        mStfMinValue = mStfMaxValue - mStfValueStep * java.lang.Math.abs(mStfMaxIndex
                - stfMinIndex);
        Elog.d(TAG, "Init Stf gain table: mStfMaxValue " + mStfMaxValue + " mStfMinValue " +
                mStfMinValue + " mStfMaxIndex " + mStfMaxIndex +
                " mStfValueStep " + mStfValueStep + " mStfIndexStep " + mStfIndexStep);
    }

    private void updateValue() {
        //Update Ul Gain
        String strValue = AudioTuningJni.getParams(CATEGORY_SPEECHVOL, TYPE_BAND
                + "," + mCurrentBand + "," + TYPE_PROFILE + "," + mCurrentProfile, UL_GAIN);
        ...
        strValue = AudioTuningJni.getParams(CATEGORY_SPEECHVOL,
                TYPE_BAND + "," + mCurrentBand + "," + TYPE_PROFILE +
                "," + mCurrentProfile, STF_GAIN);
        ...
        //Update Dl Gain
        String strDlValueTable = AudioTuningJni.getParams(CATEGORY_GAINMAP,
                TYPE_PROFILE + "," + mCurrentProfile, DL_MAP);
        mArrayDlTable = strDlValueTable.split(",");
        String strDlGain = AudioTuningJni.getParams(CATEGORY_SPEECHVOL,
                TYPE_BAND + "," + mCurrentBand + "," + TYPE_PROFILE +
                "," + mCurrentProfile, DL_GAIN);
        mArrayDlText.clear();
    ...
    private void setDlGain() {
        ...
        if (!AudioTuningJni.setParams(CATEGORY_SPEECHVOL,
                TYPE_BAND + "," + mCurrentBand + "," + TYPE_PROFILE + "," + mCurrentProfile,
                DL_GAIN, strValueDlGain)) {
    ...
    private void setUlGain() {
        ...
        if ((valueUl > mUlMaxValue) || (valueUl < mUlMinValue)) {
            mStrErrorInfo += getString(R.string.audio_volume_error_ul_invalid,
                    mUlMinValue, mUlMaxValue);
            return;
    ...
    private void setStfGain() {
        ...
        if ((value > mStfMaxValue) || (value < mStfMinValue)) {
            mStrErrorInfo += getString(R.string.audio_volume_error_stf_invalid,
                    mStfMinValue, mStfMaxValue);
            return;
        }

        value = (mStfMaxValue - value) / mStfValueStep; //Get index to minus
        value = value / mStfIndexStep * mStfIndexStep; //Make to be multiple value of index step
