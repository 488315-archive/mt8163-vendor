...
class APNBuilder {
    ...
    private boolean isSupportCdma(int slotId) {
        boolean isSupCdma = false;
        String[] type = TelephonyManagerEx.getDefault().getSupportCardType(slotId);
        CTSCLog.d(TAG, "isSupportCdma, slotId = " + slotId + ", type = " + type);
        if (type != null) {
        ...
        CTSCLog.d(TAG, " isSupCdma = " + isSupCdma);
        return isSupCdma;
    }
   ...
   private boolean isCtInRoaming(String numeric, int subId) {
    ...
    private String updateMccMncForSvlte(int subId, String iccNumeric) {
        ...
        if (isSupportCdma(slotId) && isCtInRoaming(networkNumeric, subId)) {
