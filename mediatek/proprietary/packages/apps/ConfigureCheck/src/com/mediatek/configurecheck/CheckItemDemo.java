...
class CheckIMEI extends CheckItemBase {
    ...
    private static final String ACTION_IMEI_MEID = "intent_action_imei_meid";
    ...
    public boolean onCheck() {
        ...
        CTSCLog.i(TAG, "intent is " + intent);
        if (intent != null) {
        ...
        if (TelephonyManager.getDefault().getPhoneCount() > 1) {
            ...
            String sim1IMEI = mImeiArray[0];
            CTSCLog.e("IMEI", "sim1IMEI: " + sim1IMEI);
            String sim2IMEI = mImeiArray[1];
...
class CheckIMEID extends CheckItemBase {
    ...
    private Boolean initImeiAndMeid() {
        Intent intent = getContext()
                .registerReceiver(null, new IntentFilter(ACTION_IMEI_MEID));
        CTSCLog.i(TAG, "[initImeiAndMeid] intent is " + intent);
        if (intent != null) {
...
class CheckAtciInEM extends CheckItemBase {
    ...
    public boolean onReset() {
        ...
        CTSCLog.v(TAG, "build type: " + type + ", optr: " + optr);
        if (!type.equals("eng")) {
            try {
