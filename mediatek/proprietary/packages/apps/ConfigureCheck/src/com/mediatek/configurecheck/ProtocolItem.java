...
class CheckNetworkMode extends CheckItemBase {
    ...
    private Phone getCdmaPhone() {
        ...
        Phone mPhone = PhoneFactory.getPhone(iCdmaSlotId);
        return mPhone;
    }
...
class CheckCTAFTA extends CheckItemBase {
    ...
    private void setUnlockOption(boolean unlock) {
        ...
        mPhone.invokeOemRilRequestStrings(
                new String[] {"AT+EPCT=" + mCurrentMode + "," + mCurrentFlag, "", "DESTRILD:GSM"},
                mModemATHander.obtainMessage(MODEM_CLSC));
        mCdmaPhone.invokeOemRilRequestStrings(
                new String[] {"AT+ECLSC=" + (unlock ? "1" : "0"), "", "DESTRILD:GSM"},
                mModemATHander.obtainMessage(MODEM_CLSC));
        mCdmaPhone.invokeOemRilRequestStrings(new String[] {"AT+RFSSYNC", "", "DESTRILD:GSM"},
                                                                 null);
    ...
    private void setGprsTransferType(int type) {
        ...
        for (int i = 0 ; i < TelephonyManager.getDefault().getPhoneCount(); i++) {
            ...
            phone.invokeOemRilRequestStrings(new String[] {"AT+EGTP=" + type,
                                                     "", "DESTRILD:GSM"}, null);
            phone.invokeOemRilRequestStrings(new String[] {"AT+EMPPCH=" + type,
                                                     "", "DESTRILD:GSM"}, null);
        }
    }
...
class CheckWorldPhone extends CheckItemBase {
    private static final String TAG = "CheckWorldPhone";
    private AlertDialog mAlertDialog = null;
    private static IWorldPhone sWorldPhone = null;
    private static int VALUE_SET = 11;

    CheckWorldPhone(Context c, String key) {
        super(c, key);
        setTitle(R.string.world_phone_switch_title);
        setNote(R.string.world_phone_switch_note);

        if (WorldPhoneUtil.isWorldPhoneSupport()) {
            sWorldPhone = PhoneFactory.getWorldPhone();
            setProperty(PROPERTY_AUTO_CHECK|PROPERTY_AUTO_CONFG);
        } else {
            CTSCLog.d(TAG, "world phone not support");
    ...
    public check_result getCheckResult() {
        ...
        if (airplaneMode == 1) {
            CTSCLog.d(TAG, "current is flight mode, switch fail");
            setValue("flight Mode");
        ...
        CTSCLog.i(TAG, "modemtype = " + modemtype);
        StringBuilder value = new StringBuilder("");
        value.append("Modem:")
             .append(modemtype)
             .append("\n");
        setValue(value.toString());
        if (modemtype == VALUE_SET) {
            mResult = check_result.RIGHT;
        } else {
            mResult = check_result.WRONG;
        }
    ...
    private void switchModemAlert(long millisUntilFinished, long countDownInterval) {
        ...
        mAlertDialog.setMessage("Waiting rf reboot");
    ...
    public boolean onReset() {
        ...
        if (WorldPhoneUtil.isWorldPhoneSupport() && WorldPhoneUtil.isWorldModeSupport()) {
            WorldMode.setWorldMode(VALUE_SET);
        } else {
            setValue("no support value 11");
        }
        setRebootFlag(true);
...
class CheckWorldPhoneAndModemTest extends CheckItemBase {
    private static final String TAG = "CheckWorldPhoneAndModemTest";
    private CheckWorldPhone mWorldPhone = null;
    private CheckItemBase mModemTest = null;
    private boolean mIsWaiting = false;

    CheckWorldPhoneAndModemTest(Context c, String key) {
        super(c, key);
        setTitle(R.string.WORLD_PHONE_and_ModemTest_title);
        setProperty(PROPERTY_AUTO_CHECK|PROPERTY_AUTO_CONFG);

        mWorldPhone = new CheckWorldPhone(c, CheckItemKeySet.CI_MODE_SPE_VALUE);
        if (key.equals(CheckItemKeySet.CI_MODE_SPE_VALUE_FTA)) {
            mModemTest = new CheckCTAFTA(c, CheckItemKeySet.CI_CTAFTA_FTA);
        } else if (key.equals(CheckItemKeySet.CI_MODE_SPE_VALUE_RS)) {
            mModemTest = new CheckCTAFTA(c, CheckItemKeySet.CI_CTAFTA_RS);
        } else if (key.equals(CheckItemKeySet.CI_MODE_SPE_VALUE_ANITE)) {
            mModemTest = new CheckCTAFTA(c, CheckItemKeySet.CI_CTAFTA_ANITE);
        }

        StringBuilder note = new StringBuilder("");
        note.append(mWorldPhone.getNote())
            .append("\n\n");
        if (mModemTest != null) {
            note.append(mModemTest.getNote());
        }
        setNote(note.toString());
    }
    ...
    public check_result getCheckResult() {
        CTSCLog.i(TAG, "onCheck");
        mWorldPhone.getCheckResult();
        if (mModemTest != null) {
            mModemTest.getCheckResult();
        }

        StringBuilder value = new StringBuilder("");
        value.append("Modem:")
             .append(mWorldPhone.getValue())
             .append("\n");
        if (mModemTest != null) {
            value.append("Modem Test:");
            if (mIsWaiting) {
                value.append("Modem Switch is not ready");
            } else {
                value.append(mModemTest.getValue());
            }
        }
        setValue(value.toString());

        mResult = check_result.WRONG;
        if (check_result.UNKNOWN == mWorldPhone.mResult) {
            mResult = check_result.UNKNOWN;
        }

        if (mModemTest != null
              && check_result.UNKNOWN == mModemTest.mResult) {
            mResult = check_result.UNKNOWN;
        }
        if (check_result.RIGHT == mWorldPhone.mResult
                && mModemTest != null && check_result.RIGHT == mModemTest.mResult) {
            mResult = check_result.RIGHT;
        }
    ...
    public boolean onReset() {
        ...
        if (check_result.RIGHT != mWorldPhone.mResult) {
            ...
            //if switch modem, so must set Modem Test again
            //it takes nearly 10s to switch modem successfully
            CTSCLog.d(TAG, "wait 10s to set ModemTest...");
            mIsWaiting = true;
            new Handler().postDelayed(new Runnable() {
                public void run() {
                    CTSCLog.d(TAG, "10s pass, now to set Mode test");
                    mIsWaiting = false;
                    if (mModemTest != null) {
                        mModemTest.onReset();
                    }
        ...
        } else if (check_result.RIGHT != mModemTest.mResult) {
            if (mModemTest != null) {
                mModemTest.onReset();
            }
            sendBroadcast();
...
class CheckIRsetting extends CheckItemBase {
    ...
    public boolean onReset() {
        ...
        if (getKey().equals(CheckItemKeySet.CI_IR_WCDMA_INTEGRITY_CHECK)) {
            ...
            for (int i = 0; i < 2; i++)
            {
                mSubId = SubscriptionManager.getSubId(i);
                if (mSubId != null) {
                    mPhoneType= TelephonyManager.getDefault().getCurrentPhoneType(mSubId[0]);
                    if (PhoneConstants.PHONE_TYPE_CDMA == mPhoneType) {
                        mCdmaSlotId = i;
                        break;
                    }
                }
            }
            mPhone = PhoneFactory.getPhone(mCdmaSlotId);
            if (mPhone == null) {
                CTSCLog.i("@M_" + TAG, "mPhone is null " );
                setValue(R.string.WCDMA_Integrity_off_value);
                return true;
            } else {
                mIsWaiting = true;
