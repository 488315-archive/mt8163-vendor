...
public class LocationProviderImpl implements ILocationProvider {
    ...
    public void recordLocation(boolean recordLocation) {
        LogHelper.i(TAG, "[recordLocation], mRecordLocation = " + mRecordLocation
    ...
    private void startReceivingLocationUpdates() {
        LogHelper.i(TAG, "startReceivingLocationUpdates ++++");
        ...
        if (mLocationManager != null) {
            try {
                ...
                LogHelper.i(TAG, "fail to request location update, ignore", ex);
            } catch (IllegalArgumentException ex) {
                LogHelper.i(TAG, "provider does not exist " + ex.getMessage());
            }
    ...
    private void stopReceivingLocationUpdates() {
        if (mLocationManager != null) {
            LogHelper.i(TAG, "stopReceivingLocationUpdates++++");
            for (int i = 0; i < mLocationListeners.length; i++) {
                try {
                    mLocationManager.removeUpdates(mLocationListeners[i]);
                } catch (Exception ex) {
                    LogHelper.i(TAG, "fail to remove location listners, ignore", ex);
                }
            }
            LogHelper.i(TAG, "stopReceivingLocationUpdates----");
        }
    ...
    private class LocationListener implements android.location.LocationListener {
        ...
        public LocationListener(
                String provider) {
            LogHelper.i(TAG, "[LocationListener] provider = " + provider);
            mProvider = provider;
        ...
        public void onLocationChanged(Location newLocation) {
            ...
            if (!mValid) {
                LogHelper.i(TAG, "Got first location.");
            }
        ...
        public Location current() {
            LogHelper.i(TAG, "[current],mValid = " + mValid);
