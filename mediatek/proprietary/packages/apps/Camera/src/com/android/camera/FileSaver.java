...
public class FileSaver {
    ...
    private synchronized String createName(int fileType, long dateTaken) {
        ...
        if (mFileNamer.get(fileType) != null) {
            Log.d(TAG, "[createName], mIsRawEnabled = " + mIsRawEnabled);
            if (mIsRawEnabled) {
                name = mFileNamer.get(fileType).generateRawName(dateTaken);
            } else {
    ...
    private abstract class RequestOperator implements SaveRequest {
        ...
        public void saveRawToDatabase(RequestOperator r) {
            Log.i(TAG, "[saveRawToDatabase]...");
            ...
            try {
                r.mUri = mContentResolver.insert(Files.getContentUri("external"), values);
                if (r.mUri != null) {
