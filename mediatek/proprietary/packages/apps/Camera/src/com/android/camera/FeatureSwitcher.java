...
public class FeatureSwitcher {
    ...
    private static final int HARDWAER_LEVEL_UNKONWN = -1;
    private static int sMainCamHardwareLevel = HARDWAER_LEVEL_UNKONWN;
    private static int sSubCamHardwareLevel = HARDWAER_LEVEL_UNKONWN;
    ...
    public static boolean isApi2Enable(Activity activity) {
        ...
        updateHardwareLevel(activity);
        if (sMainCamHardwareLevel != HARDWAER_LEVEL_UNKONWN) {
            enable = enable || sMainCamHardwareLevel !=
                    CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_LEGACY;
        }
        if (sSubCamHardwareLevel != HARDWAER_LEVEL_UNKONWN) {
            enable = enable || sSubCamHardwareLevel !=
                    CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_LEGACY;
        }
    ...
    private static void updateHardwareLevel(Activity activity) {
        if (sMainCamHardwareLevel != HARDWAER_LEVEL_UNKONWN ||
                sSubCamHardwareLevel != HARDWAER_LEVEL_UNKONWN) {
            return;
        }
                ...
                for (int i = 0; i < cameraIds.length; i++) {
                    if (String.valueOf(
                            CameraCharacteristics.LENS_FACING_FRONT).equalsIgnoreCase(
                                    cameraIds[i])) {
                        sSubCamHardwareLevel = camManager.
                                getCameraCharacteristics(cameraIds[i]).
                                get(CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL);
                    } else if (String.valueOf(
                            CameraCharacteristics.LENS_FACING_BACK).equalsIgnoreCase(
                                    cameraIds[i])) {
                        sMainCamHardwareLevel = camManager.
                                getCameraCharacteristics(cameraIds[i]).
                                get(CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL);
                    }
                }
        ...
        Log.i(TAG, "backCamLevel: " + sMainCamHardwareLevel +
                " frontCamLevel: " + sSubCamHardwareLevel);
    }
}
