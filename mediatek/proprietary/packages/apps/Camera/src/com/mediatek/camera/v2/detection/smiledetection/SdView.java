...
public class SdView implements ISdView {
    private static final Tag TAG = new Tag(SdView.class.getSimpleName());
    ...
    public void showSmileView() {
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                LogHelper.d(TAG, "onSmileStarted");
    ...
    public void updateSmileView() {
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                LogHelper.d(TAG, "onSmile perform ShutterButton click");
                long currentInterval = System.currentTimeMillis() - mLastCaptureTime;
                if (currentInterval < CAPTURE_INTERVAL_MS) {
                    LogHelper.e(TAG, "onSmile waitting for next capture:" + currentInterval + "ms");
                    return;
                }
