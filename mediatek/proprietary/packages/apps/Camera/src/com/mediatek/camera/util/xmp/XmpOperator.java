...
public class XmpOperator {
    ...
    public static final boolean ENABLE_BUFFER_DUMP;
    ...
    static {
        ...
        if (xmpCfg.exists() || SystemProperties.getInt(CFG_PROPERTY_NAME, 0) == 1) {
    ...
    public boolean initialize(String srcFilePath) {
        ...
        String fileFormat = srcFilePath.substring(srcFilePath.length() - TEMP_NUM3, srcFilePath
                .length());
    ...
    public byte[] getJpsDataFromJpgFile(String filePath) {
        ...
        try {
            ...
            if (ENABLE_BUFFER_DUMP) {
                mXmpInterface.writeBufferToFile(DUMP_PATH + DUMP_FOLDER_NAME + mFileName
                        + "Jps_Read.jpg", out);
            }
    ...
    public byte[] writeJpsAndMaskAndConfigToJpgBuffer(String fileName, byte[] jpgBuffer,
        ...
        if (jpgBuffer == null || jpsData == null || jsonBuffer == null) {
            Log.d(TAG, "<writeJpsAndMaskAndConfigToJpgBuffer> jpgBuffer or" +
                    "jpsData or jsonBuffer is null!!!");
            return null;
        }
        ...
        if (ENABLE_BUFFER_DUMP && fileName != null) {
            mFileName = fileName + "/";
            makeDir(DUMP_PATH + DUMP_FOLDER_NAME + mFileName);
            mXmpInterface.writeBufferToFile(DUMP_PATH + DUMP_FOLDER_NAME + mFileName + "In.jpg",
                    jpgBuffer);
            mXmpInterface.writeBufferToFile(DUMP_PATH + DUMP_FOLDER_NAME + mFileName
                    + "Jps_Written.jpg", jpsData);
            mXmpInterface.writeStringToFile(DUMP_PATH + DUMP_FOLDER_NAME + mFileName
                    + "Json_Written.txt", jsonString);
        }
        Log.d(TAG, "jsonBuffer size: " + jsonBuffer.length + ", jsonString: " + jsonString);
        StereoDebugInfoParser parser = new StereoDebugInfoParser(jsonBuffer);
        byte[] jpsMask = parser.getMaskBuffer();
        if (jpsMask == null) {
            Log.d(TAG, "<writeJpsAndMaskAndConfigToJpgBuffer> parsed jpsMask is null!!!");
            return null;
        }
        if (ENABLE_BUFFER_DUMP && fileName != null) {
            mXmpInterface.writeBufferToFile(DUMP_PATH + DUMP_FOLDER_NAME + mFileName
        ...
        if (is.readUnsignedShort() != XmpInterface.SOI) {
            Log.d(TAG, "<writeJpsAndMaskAndConfigToJpgBuffer> image is not begin with 0xffd8!!!");
            return null;
        }
        writeJpsAndMaskAndConfig(is, os, jpsData, jsonBuffer, jpsMask);
    ...
    /**
     * getJpsConfigInfoFromFile.
     *
     * @param filePath
     *            file path
     * @return jps config info
     */
    public JpsConfigInfo getJpsConfigInfoFromFile(String filePath) {
        ...
        if (ENABLE_BUFFER_DUMP) {
            mXmpInterface.writeStringToFile(DUMP_PATH + DUMP_FOLDER_NAME + mFileName
                    + "Config_Read.txt", jpsConfigInfo.toString());
        }
        return jpsConfigInfo;
    }

    private void writeJpsAndMaskAndConfig(ByteArrayInputStreamExt is, ByteArrayOutputStreamExt os,
            byte[] jpsData, byte[] jsonBuffer, byte[] jpsMask) {
    ...
    private XMPMeta makeXmpMainDataInternal(XMPMeta meta, JpsConfigInfo configInfo) {
        if (configInfo == null || meta == null) {
            Log.d(TAG, "<makeXmpMainDataInternal> error, please make sure meta or" +
                    "JpsConfigInfo not null");
            return null;
        }
        mXmpInterface.registerNamespace(MEDIATEK_IMAGE_REFOCUS_NAMESPACE, MTK_REFOCUS_PREFIX);
    ...
    private void makeXmpMainDataforSegment(XMPMeta meta, JpsConfigInfo configInfo) {
        // add for segment
    ...
    private void writeJpsAndMaskToStream(ByteArrayOutputStreamExt os, byte[] jpsData,
        ...
        try {
            ...
            for (int i = 0; i < jpsAndMaskArray.size(); i++) {
                ...
                os.writeShort(section.length + 2 + TEMP_NUM4);
                // 4 bytes
    ...
    private int calcJpgOutStreamSize(ArrayList<Section> sections, int jpgBufferSize,
        ...
        for (int i = 0; i < sections.size(); i++) {
            ...
            if (sec.mIsXmpMain || sec.mIsXmpExt || sec.mIsJpsData || sec.mIsJpsMask) {
                oldJpsDataAndMskLen += (sec.mLength + 2);
            }
        }
    ...
    private boolean updateOnlyDepthInfoWoBuffer(RandomAccessFile rafOut, XMPMeta meta,
        ...
        if (rafOut == null || meta == null || depthBufferInfo == null) {
            Log.d(TAG, "<updateOnlyDepthInfoWoBuffer> input params are null, return false!!!");
            return false;
        }
        ...
        if (newXmpMainBuffer == null) {
            Log.d(TAG,
                    "<updateOnlyDepthInfoWoBuffer> updated xmp main data is null, return false!!!");
            return false;
        }
        try {
    ...
    private boolean writeOnlyDepthBuffer(RandomAccessFile rafOut, DepthBufferInfo depthBufferInfo) {
        ...
        if (depthBufferInfo.depthData == null && depthBufferInfo.xmpDepthData == null) {
            Log.d(TAG,
                    "<writeOnlyDepthBuffer> 2 depth buffers are null, skip write depth buffer!!!");
            return true;
        }
        try {
            int totalCount = 0;
            ...
            for (int i = 0; i < depthDataArray.size(); i++) {
                ...
                rafOut.writeShort(section.length + 2 + TEMP_NUM4);
                // 4 bytes
    ...
    private byte[] getDepthDataFromSections(RandomAccessFile rafIn, boolean isDepthOrXmpDepth) {
        try {
            ...
            return getDepthData(rafIn, isDepthOrXmpDepth, dataLen, i);
        } catch (IOException e) {
