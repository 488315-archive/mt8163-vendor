...
import com.mediatek.camera.v2.stream.multicamera.MultiCameraStream;
...
public class StreamManager {
    ...
    // Sink Camera Stream
    private MultiCameraStream                mSinkStreamCtrl;
    ...
    public ISinkStreamCtrl getSinkStreamController() {
    ...
    private MultiCameraStream getSinkStream() {
        if (mSinkStreamCtrl == null) {
            // current not need the recording stream, in future maybe need, so
            // keep it
            mSinkStreamCtrl = new MultiCameraStream(getPreviewStream(), getCaptureStream(),
                    getRecordStream());
        }
        return mSinkStreamCtrl;
    }
}
