...
public class CfbStreamControllerImpl implements ICfbStreamController, ICaptureStream {
    ...
    public CfbStreamControllerImpl(ICaptureStream captureStreamController) {
        LogHelper.i(TAG, "[CfbStreamControllerImpl]");
    ...
    public void openStream(StreamStatusCallback streamStatusCallback) {
        if (DEBUG) {
            LogHelper.i(TAG, "[openStream] +++++,streamStatusCallback = " + streamStatusCallback);
        }
        // new JPEG encoder
        ...
        try {
            ...
            LogHelper.e(TAG, "[openStream] error : " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public void setParameters(List<String> key, List<String> value) {
        ...
        LogHelper.i(TAG, "[setParameters] the parameter is : " + mBaseParameters.flatten());
    ...
    public void startCapture(CaptureStatusCallback captureStatusCallback) {
        if (DEBUG) {
            LogHelper.i(TAG, "[startCapture] captureStatusCallback = " + captureStatusCallback
    ...
    public void setCurrentVideoTag(boolean isVideo) {
        LogHelper.i(TAG, "[setCurrentVideoTag] isVideo = " + isVideo);
    ...
    public void closeStream() {
        if (DEBUG) {
            LogHelper.i(TAG, "[closeStream] mCameraEffectHalSession = " + mCameraEffectHalSession);
        }
        mIsSizeChange = true;
    ...
    public boolean updateCaptureSize(Size pictureSize, int pictureFormat) {
        LogHelper.i(TAG, "[updateCaptureSize] picturesize's width = " + pictureSize.getWidth()
        ...
        if (mCurrentPictureSize != null && mCurrentPictureSize.getWidth() == pictureSize.getWidth()
            ...
            mIsSizeChange = false;
            LogHelper.i(TAG, "picture size and format is not changed");
            return false;
        }
        ...
        if (mIsInVideoMode) {
            LogHelper.i(TAG,
                    "[updateCaptureSize] current is in video mode,so do noting Effect Hal");
            return false;
        }
        ...
        if (mOutPutSurfaces != null) {
            for (int i = 0; i < mOutPutSurfaces.size(); i++) {
                LogHelper.i(TAG, "[updateCaptureSize],before clear the surface, curren have : "
    ...
    public Map<String, Surface> getCaptureInputSurface() {
        LogHelper.i(TAG, "[getCaptureInputSurface]+++++, mIsInVideoMode = " + mIsInVideoMode
        ...
        if (inputSurfaces.size() != 0) {
            for (int i = 0; i < inputSurfaces.size(); i++) {
                ...
                LogHelper.i(TAG, "getCaptureInputSurface ,surface = " + inputSurfaces.get(i));
            }
        }
        return mInputSurface;
    }
    ...
    private void setOutputSurface(List<Surface> surfaces, List<BaseParameters> surfaceParameters) {
        LogHelper.i(TAG, "[setOutputSurface] surfaces'length = " + surfaces.size()
