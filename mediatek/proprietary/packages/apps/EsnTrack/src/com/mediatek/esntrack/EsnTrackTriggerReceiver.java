...
public class EsnTrackTriggerReceiver extends BroadcastReceiver {
    ...
    private boolean isCdmaAction(Context context, Intent intent) {
        ...
        TelephonyManager mTelephonyManager = (TelephonyManager) context
                .getSystemService(Context.TELEPHONY_SERVICE);
        if (Const.ACTION_CDMA_DATA_CONNECTION_ACTIVE.equalsIgnoreCase(intent.getAction())) {
            boolean isCdmaData = IsDataConnectionByCdma(context);
            Log.d(TAG, "[ESN TRACK] DATA_CONNECTION" + isCdmaData);
            return isCdmaData;
        }
        if ((mTelephonyManager.getSimCount() == 1)) {
            if (Const.ACTION_CDMA_AUTO_SMS_REGISTER_FEASIBLE.equalsIgnoreCase(intent.getAction())) {
                boolean isReady = intent.getBooleanExtra("ready", false);
                Log.d(TAG, "[ESN TRACK] feasible isReady:" + isReady);
                if (!isReady) {
                    return false;
                }
            }
            Log.d(TAG, "isCdmaAction  single SIM project so must be CDMA, return true");
            return true;
        }

        if (Const.ACTION_BOOTCOMPLETED.equalsIgnoreCase(intent.getAction())) {
            Log.d(TAG, "[ESN TRACK] BOOT COMPLETE");
            return true;
        }

        if (Const.ACTION_CDMA_UTK_MENU_SELECTION.equalsIgnoreCase(intent.getAction())) {
            if (intent.getExtras() != null) {
                int whichSIM = intent.getExtras().getInt(PhoneConstants.SLOT_KEY);
                int subId[] = SubscriptionManager.getSubId(whichSIM);
                if (subId != null) {
                    int phoneType = TelephonyManager.getDefault().getCurrentPhoneType(subId[0]);
                    Log.d(TAG, "[ESN TRACK] UTK phoneType:" + phoneType);
                    if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                        return true;
                    }
                }
            }
            Log.d(TAG, "[ESN TRACK] UTK selection but not a cdma action");
            return false;
        }
        ...
        if (Const.ACTION_CDMA_AUTO_SMS_REGISTER_FEASIBLE.equalsIgnoreCase(intent.getAction())) {
            boolean isReady = intent.getBooleanExtra("ready", false);
            Log.d(TAG, "[ESN TRACK] DualSIM Case feasible isReady:" + isReady);
            if (!isReady) {
                return false;
            }
        }
        ...
        if (intent.getExtras() != null
                && intent.getExtras().containsKey(PhoneConstants.SUBSCRIPTION_KEY)) {
            int whichSIM = intent.getExtras().getInt(PhoneConstants.SUBSCRIPTION_KEY);
