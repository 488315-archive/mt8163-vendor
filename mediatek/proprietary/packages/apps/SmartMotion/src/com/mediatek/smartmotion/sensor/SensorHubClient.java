...
public class SensorHubClient {
    ...
    public int addRequest(int type) {
        LogUtils.d(TAG, "addRequest:" + type);
        if (mSensorHubManager == null) {
            LogUtils.d(TAG, "mSensorHubManager is null");
            return -1;
        }
        if (mSensorHubManager == null || !mSensorHubManager.getContextList().contains(type)) {
            LogUtils.d(TAG, "mSensorHubManager does not contain this type");
            return -1;
        }
        ...
        case ContextInfo.Type.CARRY:
            condition = builder.createCondition(ContextInfo.Carry.IN_POCKET,
                    Condition.OP_EQUALS, 1);
            break;
        ...
        case ContextInfo.Type.USER_ACTIVITY:
            ...
            break;
        case ContextInfo.Type.FREE_FALL:
            condition = builder.createCondition(ContextInfo.FreeFall.VALUE, Condition.OP_EQUALS, 1);
            break;
        case ContextInfo.Type.SNAPSHOT:
            condition = builder.createCondition(ContextInfo.Snapshot.VALUE, Condition.OP_EQUALS, 1);
            break;
        case ContextInfo.Type.PDR:
            Condition cd1 = builder.createCondition(ContextInfo.PDR.Value.X,
                    Condition.OP_EQUALS, 100);
            Condition cd2 = builder.createCondition(ContextInfo.PDR.Value.Y,
                    Condition.OP_EQUALS, 100);
            condition = builder.combineWithAnd(cd1, cd2);
            break;
        default:
            break;
        }
