...
public class SignificantMotionEnabler extends SmartMotionEnabler {
    ...
    private final TriggerEventListener mSensorEventListener = new TriggerEventListener() {
        public void onTrigger(TriggerEvent event) {
            LogUtils.d(TAG, "SMD_onTrigger: sensor=" + event.sensor + ",values="
                + getValules(event.values));
            playSound();
            if (mIsChecked) {
                boolean result = mSensorManager.requestTriggerSensor(mSensorEventListener, mSensor);
                LogUtils.d(TAG, "SMD_onTrigger: sensor=" + mSensor + ", result=" + result);
            }
        }
    ...
    private final OnLoadCompleteListener mLoadCompleteCallback = new OnLoadCompleteListener() {
        public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
            synchronized(mSounds) {
                LogUtils.d(TAG, "SMD_onLoadComplete: sample=" + sampleId + ",loadStatus=" + status);
                mSounds.notify();
            }
        }
    };
    ...
    protected void enableSensor() {
        ...
        if (mSensor != null) {
            boolean result = mSensorManager.requestTriggerSensor(mSensorEventListener, mSensor);
            LogUtils.d(TAG, "SMD_enableSensor: sensor=" + mSensor + ", result=" + result);
        } else {
            LogUtils.d(TAG, "SMD_enableSensor: failed to enableSensor, mSensor is null.");
        }
    }
    ...
    protected void disableSensor() {
        ...
        LogUtils.d(TAG, "SMD_disableSensor: sensor=" + mSensor + ", result=" + result);
    }
    ...
    private void initSoundPool() {
        ...
        try {
            ...
            LogUtils.e(TAG, "SMD_initSoundPool: InterruptedException", ex);
        }
    }

    private void playSound() {
        LogUtils.i(TAG, "SMD_playSound>>>streamId=" + mSoundStreamId);
        if (mSounds == null) {
            initSoundPool();
        }
