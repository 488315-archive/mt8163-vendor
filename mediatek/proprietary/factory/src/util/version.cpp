...
static int create_md_verinfo(char *info, int *len)                                                          
{
   ...
   #if defined(MTK_FTM_C2K_SUPPORT) && !defined(EVDO_FTM_DT_VIA_SUPPORT)    
    #ifdef MTK_ECCCI_C2K
    if(1 == modem[modem_number].Conn_Init(ccci_path[modem_number],modem_number+1,g_SIGNAL_Callback[modem_number]))
    #else
    asciDevice = viatelAdjustDevicePathFromProperty(VIATEL_CHANNEL_AT);
    if(1 == modem[modem_number].Conn_Init(asciDevice,modem_number+1,g_SIGNAL_Callback[modem_number]))
    #endif
    {
	     if(g_Flag_VPUP != 1)
	     {
