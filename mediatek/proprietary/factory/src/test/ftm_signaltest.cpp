...
{   
    ...
    g_Flag_VPUP = 0 ;
    #ifdef MTK_ECCCI_C2K
    if(0 == modem[modem_number].Conn_Init(ccci_path[modem_number],modem_number+1,g_SIGNAL_Callback[modem_number]))
    #else
                  ...
                  g_Flag_VPUP = 0 ;
					...
					for(i=0 ;i<modem_number; i++) 
					{
					    query_modem_status(modem[i]);
					  	#if defined(MTK_FTM_SVLTE_SUPPORT) || defined(MTK_FTM_SRLTE_SUPPORT)
