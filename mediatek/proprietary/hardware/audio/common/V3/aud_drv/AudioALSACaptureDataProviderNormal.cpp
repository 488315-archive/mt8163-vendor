...
status_t AudioALSACaptureDataProviderNormal::open()
{
    ...
    if (HasLowLatencyCapture())
    {
        ...
        mConfig.period_size = (SizeVector * kReadBufferSize_lowlatency / mConfig.channels) / 4;  //interrupt 5ms
        mConfig.format = PCM_FORMAT_S32_LE;
        // audio low latency param - record - read from kernel
        kReadBufferSize = SizeVector * kReadBufferSize_lowlatency;
    }
    else
    {
        mConfig.period_count = 2;
        //period size will impact the interrupt interval
        mConfig.period_size = (SizeVector * kReadBufferSize_normal / mConfig.channels) / 4;  //interrupt 20ms
        mConfig.format = PCM_FORMAT_S32_LE;
        kReadBufferSize = SizeVector * kReadBufferSize_normal;
    }
#else
    mConfig.period_count = 2;
    //period size will impact the interrupt interval
    ...
    if (HasLowLatencyCapture())
    {
        mConfig.period_count = 2;
        //period size will impact the interrupt interval
        mConfig.period_size = (SizeVector * kReadBufferSize_lowlatency / mConfig.channels) / 2;  //interrupt 5ms
    ...
    else
    {
        mConfig.period_count = 2;
        //period size will impact the interrupt interval
        mConfig.period_size = (SizeVector * kReadBufferSize_normal / mConfig.channels) / 2;  //interrupt 20ms
        mConfig.format = PCM_FORMAT_S16_LE;
        kReadBufferSize = SizeVector * kReadBufferSize_normal;
    }
#else
