...
put(const ILscMgr::TSF_INPUT_INFO_T& rInStat, MUINT8 fgOpt)
{
    ...
    {
        MINT32 i4NumX = u4BlkNumX;  //m_rTsfEnvInfo.ImgWidth;
        MINT32 i4NumY = u4BlkNumY;  //m_rTsfEnvInfo.ImgHeight;
...
tsfSetSL2(const MTK_TSF_SL2_PARAM_STRUCT& rSL2)
{
    ...
    {
        // TG coordinate
        rSl2Cfg.i4CenterX = rSL2.SL2_CENTR_X * u4TgW / u4BinW;
        rSl2Cfg.i4CenterY = rSL2.SL2_CENTR_Y * u4TgH / u4BinH;
        rSl2Cfg.i4R0      = rSL2.SL2_RR_0 * u4TgW / u4BinW;
        rSl2Cfg.i4R1      = rSL2.SL2_RR_1 * u4TgW / u4BinW;
        rSl2Cfg.i4R2      = rSL2.SL2_RR_2 * u4TgW / u4BinW;
...
tsfInit()
{
    ...
    #if 1
    ILscTbl::TransformCfg_T rCropCfg = m_pLsc->getCurTrfmCfg();
...
tsfCfgChg()
{
    ...
    for (i = 0; i < RING_TBL_NUM; i++)
    {
        m_prLscTbl[i]->setConfig(m_rLscBaseTbl.getConfig());
    }
#if 0
    ...
    tsfSetTbl(*m_prLscTbl[(m_u4TblIdx & RING_TBL_MSK)]);
...
setConfig(ESensorMode_T eSensorMode, MUINT32 u4W, MUINT32 u4H, MBOOL fgForce)
{
    ...
    ILscTbl::Config rTblCfg = m_pLsc->getLut(2)->getConfig();
