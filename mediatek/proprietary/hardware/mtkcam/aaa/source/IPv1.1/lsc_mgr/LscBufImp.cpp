...
dump(const char* filename) const
{
    ...
    for (i = numCoef-1; i >= 0; i-=4)
    {
        ...
        fprintf(fsdblk, " %8d %8d %8d", coef1, coef2, coef3);
        fprintf(fhwtbl,"0x%08x, ", val);
        coef1 = ((val& 0x80000000) >> 31);

        val = *pData++;
        coef3 = (val& 0xFFE00000) >> 21;
        coef2 = (val& 0x001FFC00) >> 10;
        coef1 += ((val& 0x000003FF) << 1);
        fprintf(fsdblk, " %8d %8d %8d", coef1, coef2, coef3);
        fprintf(fhwtbl,"0x%08x, ", val);

        val = *pData++;
        coef3 = (val& 0xFFC00000) >> 22;
        coef2 = (val& 0x003FF800) >> 11;
        coef1 = val& 0x000007FF;
        fprintf(fsdblk, " %8d %8d %8d", coef1, coef2, coef3);
        fprintf(fhwtbl,"0x%08x, ", val);
...
allocBuf(MUINT32 x_size, MUINT32 y_size)
{
    ...
    if (pHeap == NULL) {
        MY_ERR("[%s] ImageBufferHeap create fail", m_strName.c_str());
        return MFALSE;
    }
    ...
    {
        MY_ERR("[%s] Stuff ImageBuffer lock fail",  m_strName.c_str());
        return MFALSE;
    }
...
freeBuf()
{
    ...
    if (m_pImgBuf == NULL) {
        MY_ERR("sensorDev(0x%02x), id(%d), name(%s): Stuff ImageBuffer not exist", m_u4SensorDev, m_u4Id, m_strName.c_str());
        return MFALSE;
    }
...
showInfo() const
{
    LSC_LOG("sensor(0x%02x), name(%s), id(%d), virtAddr(%p), phyAddr(%p), size(%d), Img(%dx%d), Blk(%d,%d,%d,%d,%d,%d)",
        m_u4SensorDev, m_strName.c_str(), m_u4Id, getTable(), getPhyAddr(), getSize(),
        m_rConfig.i4ImgWd, m_rConfig.i4ImgHt, m_rConfig.i4BlkX, m_rConfig.i4BlkY, m_rConfig.i4BlkW, m_rConfig.i4BlkH, m_rConfig.i4BlkLastW, m_rConfig.i4BlkLastH);
...
setConfig(ILscBuf::Config rConfig)
{
    ...
    if (u4Size != getSize())
    {
        ...
        {
            MUINT32 LSCI_XSIZE = (m_rConfig.i4BlkX+1)*4*128/8;
 ...
 *******************************************************************************/
ILscBuf::
ILscBuf(MUINT32 sensorDev, MUINT32 u4Id, const char* strName, E_BUF_MODE_T eBufMode)
{
