...
doUpdateCmd(const ParamIspProfile_T* pParam)
{
    ...
    CAM_TRACE_FMT_BEGIN("3A_Update #(%d)", pParam->i4MagicNum);
...
on3AProcFinish(MUINT32 u4MagicNum, MUINT32 u4MagicNumCur)
{
    ...
    if (m_pCbSet)
    {
        ...
        MY_LOG_IF(m_fgLogEn, "[%s] #(%d,%d), SOF(0x%x), m_rCapParam.capType(%d), RequestCbSet.vNumberSet[0](%d)", __FUNCTION__, u4MagicNum, u4MagicNumCur, i4CurId, m_rCapParam.u4CapType, RequestCbSet.vNumberSet[0]);
...
getResultCur(MUINT32 i4FrmId, Result_T& rResult)
{
    ...
    if (-1 == i4Ret)
    {
        MY_LOG_IF(m_fgLogEn, "[%s] wait result", __FUNCTION__);
        m_rResultCond.waitRelative(m_rResultMtx, (long long int)5000000);
        MY_LOG_IF(m_fgLogEn, "[%s] wait result done", __FUNCTION__);
