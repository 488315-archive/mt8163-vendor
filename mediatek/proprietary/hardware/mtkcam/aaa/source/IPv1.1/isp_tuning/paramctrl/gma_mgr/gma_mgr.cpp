    ...
    , mLogEn (0)
{
    ...
    MY_LOG_IF(1, "[%s()] m_pGmaAlgo(%p), m_eSensorDev(%d), m_pIspNvramGmaPara(%d)", __FUNCTION__, m_pGmaAlgo, m_eSensorDev, m_pIspNvramGmaPara);
    ...
    mEnv.rTuningGmaParam = *((MTK_GMA_TUNING_PARAM_T*)m_pIspNvramGmaPara);
...
MVOID GmaMgr::printInfo() const
{
    ...
    */
    ...
    for (int i=0; i<120; i+=10)
    {
        MY_LOG( "%s: mInInfo.rGMAAEInfo.u4Histogrm[%d-%d]=(%x,%x,%x,%x,%x,  %x,%x,%x,%x,%x)"
            , __FUNCTION__
            , i
            , i+9
            , pHist[i], pHist[i+1], pHist[i+2], pHist[i+3], pHist[i+4]
    ...
    {
        int i=120;
        MY_LOG( "%s: mInInfo.rGMAAEInfo.u4Histogrm[%d-%d]=(%x,%x,%x,%x,%x,  %x,%x,%x)"
            , __FUNCTION__
            , i
            , i+7
            , pHist[i], pHist[i+1], pHist[i+2], pHist[i+3], pHist[i+4]
