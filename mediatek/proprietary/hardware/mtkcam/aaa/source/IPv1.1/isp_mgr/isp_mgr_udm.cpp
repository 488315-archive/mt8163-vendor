...
get(ISP_NVRAM_UDM_T& rParam, const dip_x_reg_t* pReg) const
{
    if (pReg)
    {
        ...
        GET_REG_INFO_BUF(DIP_X_UDM_TILE_EDGE, tile_edge);
        GET_REG_INFO_BUF(DIP_X_UDM_DSL, dsl);
        GET_REG_INFO_BUF(DIP_X_UDM_SPARE_1, spare_1);
        GET_REG_INFO_BUF(DIP_X_UDM_SPARE_2, spare_2);
        GET_REG_INFO_BUF(DIP_X_UDM_SPARE_3, spare_3);
    }
    return MTRUE;
}
...
apply(EIspProfile_T eIspProfile, dip_x_reg_t* pReg)
{
    ...
    //MBOOL bSL2 = ISP_MGR_SL2_T::getInstance(m_eSensorDev).isEnable();
    //reinterpret_cast<ISP_DIP_X_UDM_SL_CTL_T*>(REG_INFO_VALUE_PTR(DIP_X_UDM_SL_CTL))->UDM_SL_EN = bSL2;
