...
MRESULT AeMgr::setSceneMode(MUINT32 u4NewAEScene)
{
    ...
    if (m_eAEScene != eAEScene) {
        MY_LOG("[setSceneMode] m_eAEScene: %d old:%d\n", eAEScene, m_eAEScene);
        m_eAEScene = eAEScene;
        if(m_pIAeAlgo != NULL) {
...
MRESULT AeMgr::setAEISOSpeed(MUINT32 u4NewAEISOSpeed)
{
    ...
    if (m_u4AEISOSpeed != eAEISOSpeed) {
        MY_LOG("[setAEISOSpeed] m_u4AEISOSpeed: %d old:%d\n", eAEISOSpeed, m_u4AEISOSpeed);
...
MRESULT AeMgr::setAEAutoFlickerMode(MUINT32 u4NewAEAutoFLKMode)
{
    ...
    if (m_eAEAutoFlickerMode != eNewAEAutoFLKMode) {
        ...
        MY_LOG("[Flicker] m_eAEAutoFlickerMode: %d\n", m_eAEAutoFlickerMode);
        if(m_pIAeAlgo != NULL) {
            //AE P-line
            m_pIAeAlgo->getEVIdxInfo(m_u4IndexMax ,m_u4IndexMin ,m_u4Index);
...
MRESULT AeMgr::setAEFlickerMode(MUINT32 u4NewAEFLKMode)
{
    ...
    if (m_eAEFlickerMode != eAEFLKMode) {
        MY_LOG("[Flicker] AEFlickerMode: %d old:%d\n", eAEFLKMode, m_eAEFlickerMode);
...
{
    MY_LOG("[setAEFlickerModePlineIdx] Set Flicker value %d %d\n",m_eAEFlickerMode, a_eAEFlickerMode);
    setIsoSpeed(m_u4AEISOSpeed);
    return S_AE_OK;
}
...
MRESULT AeMgr::setAETable(eAETableID a_AEPreTableID, eAETableID a_AECapTableID)
{
    ...
    if (m_bManualAEtable == 0){
        ...
        if(FAILED(mr)) {
            MY_ERR("[setAETable]Search Capture Pline table:%d error \n", a_AEPreTableID);
        }
     } else {
...
MRESULT AeMgr::setAEMinMaxFrameRatePlineIdx(MINT32 a_eAEMinFrameRate, MINT32 a_eAEMaxFrameRate)
{
  ...
  MY_LOG("[setAEMinMaxFrameRatePlineIdx] Max frame rate :%d %d %d %d %d",a_eAEMaxFrameRate, m_u4IndexMax, m_u4IndexMin, m_u4Index, i4fps);
  ...
  MY_LOG("[setAEMinMaxFrameRatePlineIdx] Min frame rate :%d %d %d %d %d %d",m_i4AEMinFps, m_u4IndexMax, m_u4IndexMin, m_u4Index, i4PlineIndex, i4fps);
  return  S_AE_OK;
}
...
MRESULT AeMgr::setAEMaxFrameRateGainIndexRange()
{
  ...
  i430fps = 300;
  MY_LOG_IF(m_3ALogEnable,"Set Gain Index Range for LCE");
...
MRESULT AeMgr::setManualAETable(MUINT32 a_AEPreTableID, MUINT32 a_AECapTableID)
{
    ...
    switch(a_AEPreTableID) {
        case 27:
            ...
            break;
        default:
            MY_LOG("[setManualAETable] The Preview AE table enum value is incorrectly:%d\n", a_AEPreTableID);
    ...
    switch(a_AECapTableID) {
        case 27:
            ...
            break;
        default:
            MY_LOG("[setManualAETable] The Capture AE table enum value is incorrectly:%d\n", a_AECapTableID);
            eCapturePLineTableID = m_pCaptureTable->eID;
            break;
    }
    MY_LOG("[setManualAETable] Preview TableID:%d -> %d  Capture TableID:%d -> %d\n",
    ...
    if(FAILED(mr)) {
        MY_ERR("[setManualAETable]Search Preview Pline table:%d error \n", ePreviewPLineTableID);
    }

    mr = searchAETable(m_pAEPlineTable, eCapturePLineTableID, &m_pCaptureTable);

    if(FAILED(mr)) {
        MY_ERR("[setManualAETable]Search Capture Pline table:%d error \n", eCapturePLineTableID);
    }
