...
MRESULT AeMgr::PreviewAEInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    MY_LOG("[PreviewAEInit] m_eSensorDev:%d m_i4SensorIdx:%d AEconfig:%d/%d/%d/%d/%d/%d \n",
    ...
    // Allocate AAO separation buffer
    if(m_pAAOSeparatedBuf== NULL) {
        m_pAAOSeparatedBuf = (MVOID *)malloc(m_u4AAOSepAWBsize+m_u4AAOSepAEsize+m_u4AAOSepHistsize+m_u4AAOSepOverCntsize);
        if(m_pAAOSeparatedBuf == NULL) {
            MY_ERR("AAO separated statistic memory is not enough - 0\n");
        } else {
            MY_LOG("AAO separated memory allocated success - 1\n");
        }
    } else {
        MY_LOG("AAO separated memory is allocated already - 1\n");
    }
    return S_AE_OK;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
MRESULT AeMgr::cameraPreviewInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    property_get("debug.ae_mgr.dumpaao", value, "0");
...
// AEInit(rParam);
    ...
    MY_LOG("[Start] m_eSensorDev:%d m_eSensorTG:%d m_i4SensorIdx:%d\n", m_eSensorDev, m_eSensorTG, m_i4SensorIdx);
    // AE statistics and histogram config
...
MRESULT AeMgr::uninit()
{
    ...
    if(m_pAAOSeparatedBuf != NULL) {
        free(m_pAAOSeparatedBuf);
        m_pAAOSeparatedBuf = NULL;
        MY_LOG("[AeMgr::uninit] AE statistic memory\n");
    }
...
{
  MY_LOG("setAeMeterAreaEn en=%d, m_isAeMeterAreaEn=%d",en, m_isAeMeterAreaEn);
...
MRESULT AeMgr::setAEMeteringArea(CameraMeteringArea_T const *sNewAEMeteringArea)
{
    ...
    MY_LOG("setAEMeteringArea m_isAeMeterAreaEn=%d",m_isAeMeterAreaEn);
        ...
        for(ifaceIdx=0; ifaceIdx < m_eAEFDArea.u4Count; ifaceIdx++) {
            ...
            if((m_eZoomWinInfo.u4XOffset != 0) || (m_eZoomWinInfo.u4YOffset != 0)){
                if ( (m_eAEFDArea.rAreas[ifaceIdx].i4Left != 0)||(m_eAEFDArea.rAreas[ifaceIdx].i4Right != 0)  ||
                        ...
                        m_eAEFDArea.rAreas[ifaceIdx].i4Left   = -1000 + (m_eZoomWinInfo.u4XOffset)*2000/(m_eZoomWinInfo.u4XWidth+2*m_eZoomWinInfo.u4XOffset)  + (m_eAEFDArea.rAreas[ifaceIdx].i4Left + 1000)*m_eZoomWinInfo.u4XWidth/(m_eZoomWinInfo.u4XWidth+2*m_eZoomWinInfo.u4XOffset);
                        m_eAEFDArea.rAreas[ifaceIdx].i4Right = -1000 + (m_eZoomWinInfo.u4XOffset)*2000/(m_eZoomWinInfo.u4XWidth+2*m_eZoomWinInfo.u4XOffset)  + (m_eAEFDArea.rAreas[ifaceIdx].i4Right+ 1000)*m_eZoomWinInfo.u4XWidth/(m_eZoomWinInfo.u4XWidth+2*m_eZoomWinInfo.u4XOffset);
                        m_eAEFDArea.rAreas[ifaceIdx].i4Top    = -1000 + (m_eZoomWinInfo.u4YOffset)*2000/(m_eZoomWinInfo.u4YHeight+2*m_eZoomWinInfo.u4YOffset) + (m_eAEFDArea.rAreas[ifaceIdx].i4Top+ 1000)*m_eZoomWinInfo.u4YHeight/(m_eZoomWinInfo.u4YHeight+2*m_eZoomWinInfo.u4YOffset);
                        m_eAEFDArea.rAreas[ifaceIdx].i4Bottom = -1000 + (m_eZoomWinInfo.u4YOffset)*2000/(m_eZoomWinInfo.u4YHeight+2*m_eZoomWinInfo.u4YOffset) + (m_eAEFDArea.rAreas[ifaceIdx].i4Bottom+ 1000)*m_eZoomWinInfo.u4YHeight/(m_eZoomWinInfo.u4YHeight+2*m_eZoomWinInfo.u4YOffset);
                }
                MY_LOG("[%s():Modified] AE FD meter area Idx:%d Left:%d Right:%d Top:%d Bottom:%d InitWidth:%d InitHeight:%d\n",__FUNCTION__, ifaceIdx,
                ...
                MY_LOG("[%s():Previous] AE FD meter area Idx:%d Left:%d Right:%d Top:%d Bottom:%d \n",__FUNCTION__, ifaceIdx,
...
MRESULT AeMgr::setAEMinMaxFrameRate(MINT32 i4NewAEMinFps, MINT32 i4NewAEMaxFps)
{
    ...
    if ((m_i4AEMinFps != i4NewMinFPS) || (m_i4AEMaxFps != i4NewMaxFPS)) {
        ...
        MY_LOG("[setAEMinMaxFrameRate] m_i4AEMinFps: %d m_i4AEMaxFps:%d\n", m_i4AEMinFps, m_i4AEMaxFps);
        if(m_pIAeAlgo != NULL) {
            //New AE-Pline
            m_pIAeAlgo->getEVIdxInfo(m_u4IndexMax ,m_u4IndexMin ,m_u4Index);
            setAEMinMaxFrameRatePlineIdx(m_i4AEMinFps ,m_i4AEMaxFps);
...
MBOOL AeMgr::setSensorMode(MINT32 i4NewSensorMode , MUINT32 width, MUINT32 height)
{
    ...
    MY_LOG("[%s()] m_eSensorMode/Hbinwidth/Hbinheight: %d/%d/%d -> %d/%d/%d , u4BlocksizeX/Y/Shift = %d/%d/%d \n", __FUNCTION__, m_eSensorMode,m_u4Hbinwidth,m_u4Hbinheight,
...
MRESULT AeMgr::setZoomWinInfo(MUINT32 u4XOffset, MUINT32 u4YOffset, MUINT32 u4Width, MUINT32 u4Height)
{
    ...
    if((u4XOffset >= 0)&&(u4XOffset <= 8192)&&(u4YOffset >= 0)&&(u4YOffset <= 8192)
        ...
        MY_LOG("[AeMgr::setZoomWinInfo] Wrong zoom window size %d/%d/%d/%d \n", u4XOffset, u4Width, u4YOffset, u4Height);
    }
    return S_AE_OK;
}
...
MINT32 i4ActiveItem = 0;
    ...
    if(m_eAETargetMode == AE_MODE_MVHDR_TARGET) {
        ...
        aaoSeparation(pAEStatBuf, m_u4BlockNumX, m_u4BlockNumY,
                      m_pAAOSeparatedBuf,
                      (m_pAAOSeparatedBuf+ m_u4AAOSepAWBsize),
                      (m_pAAOSeparatedBuf+ m_u4AAOSepAWBsize+ m_u4AAOSepAEsize),
                      (m_pAAOSeparatedBuf+ m_u4AAOSepAWBsize+ m_u4AAOSepAEsize+ m_u4AAOSepHistsize));
        pAEStatisticBuf = m_pAAOSeparatedBuf;
    }
    ...
    if(m_eAEMode == LIB3A_AE_MODE_OFF) {
        ...
        if ((i4ActiveItem & E_AE_AE_CALC) || (bCalculateAE == MTRUE)) {
            ...
            } else if ((bAAASchedule == MTRUE)||(m_eAETargetMode == AE_MODE_IVHDR_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET)||(m_eAETargetMode == AE_MODE_ZVHDR_TARGET)){
...
{
  MY_LOG("AFLampExp, expOn: %d %d %d %d %d %d",exp, afe, isp, expOn, afeOn, ispOn);
  double rDif;
  ...
  MY_LOG("AFLampExpp ispIncRat=%lf r=%lf", ispIncRat, r);
...
//    }
    ...
    m_i4HdrSeg = sAEInfo->i4vHdrRMGSeg;
...
MRESULT AeMgr::AEInit(Param_T &rParam)
{
    ...
    MY_LOG("[AEInit] AAO separation size(To AeAlgo):%d/%d/%d/%d \n",
...
MRESULT AeMgr::getExposureInfo(ExpSettingParam_T &strHDRInputSetting)
{
    ...
    if(m_pIAeAlgo != NULL) {
        ...
        if((rSWHdrInfo.eAeTargetMode == AE_MODE_IVHDR_TARGET)||(rSWHdrInfo.eAeTargetMode == AE_MODE_ZVHDR_TARGET)) {
...
if(m_bPerframeAEFlag){
            ...
            if ((m_i4IspGainDelayFrames == m_i4ShutterDelayFrames)||m_rAEInitInput.rAEPARAM.bPerFrameAESmooth) {
                    updateAEInfo2ISP(AE_SENSOR_PER_FRAME_STATE, m_rAEOutput.rPreviewMode.u4IspGain);
                    MY_LOG("[%s()] Isp gain value(current) = %d\n", __FUNCTION__, m_rAEOutput.rPreviewMode.u4IspGain);
            } else if (m_i4IspGainDelayFrames == m_i4ShutterDelayFrames + 1){
                    updateAEInfo2ISP(AE_SENSOR_PER_FRAME_STATE, m_u4PreviousISPgain);
...
#endif
                MY_LOG("[%s:e] i4SensorDev:%d VDNum %d, Delay %d/%d/%d, Prev %d/%d/%d, Output %d/%d/%d State:%d FrameRate:%d/%d Flare:%d/%d \n", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
...
MRESULT AeMgr::updatePreviewParamsByzVHDR(strAEOutput *sAEInfo)
{
    ...
    strZVHDRInputSetting.u4SensorGain = sAEInfo->EvSetting.u4AfeGain*sAEInfo->EvSetting.u4IspGain >> 10;
    strZVHDRInputSetting.i4LV = sAEInfo->Bv + 50;

    if(m_p3ANVRAM != NULL) {
        strZVHDRInputSetting.u41xGainISO = m_p3ANVRAM->rAENVRAM.rDevicesInfo.u4MiniISOGain;
        strZVHDRInputSetting.u4SaturationGain = m_p3ANVRAM->rAENVRAM.rDevicesInfo.u4MinGain;
    } else {
        strZVHDRInputSetting.u41xGainISO = 100;
        strZVHDRInputSetting.u4SaturationGain = 1216;
        MY_LOG("[updatePreviewParamsByzVHDR] NVRAM data is NULL\n");
    }

    if((m_bRealISOSpeed == MTRUE)&&(m_u4AEISOSpeed != 0)){ //Eng mode
           strZVHDROutputSetting.u4SEExpTimeInUS =  sAEInfo->EvSetting.u4Eposuretime ;
        ...
        strZVHDROutputSetting.u4LE_SERatio_x100 = strZVHDROutputSetting.u4LEExpTimeInUS *100 / strZVHDROutputSetting.u4SEExpTimeInUS;
        strZVHDROutputSetting.u4LE_SERatio_x100 = ((strZVHDROutputSetting.u4LE_SERatio_x100 + 50) / 100) * 100;
    }
    ...
    MY_LOG("[%s] i4SensorDev:%d zVHDR 60Hz:%d Shutter:%d Gain:%d %d %d 1xGainISO:%d SatGain:%d Workaround:%d SE:%d %d %d LE:%d %d %d Ratio:%d\n", __FUNCTION__, m_eSensorDev,
                    strZVHDRInputSetting.bIs60HZ,
                    strZVHDRInputSetting.u4ShutterTime,
                    strZVHDRInputSetting.u4SensorGain,
                    sAEInfo->EvSetting.u4AfeGain,
                    sAEInfo->EvSetting.u4IspGain,
    ...
    if(m_bRealISOSpeed == MTRUE){ //Eng mode
           ...
           rSensorOutputData.u4SensorExpTime =  strZVHDROutputSetting.u4LEExpTimeInUS ;
           rSensorOutputData.u4SensorGain  =  strZVHDROutputSetting.u4LESensorGain ;
           rSensorOutputData.u4IspGain     =  strZVHDROutputSetting.u4LEISPGain ;
    ...
    MY_LOG("[%s] i4SensorDev:%d zVHDR workound:%d New SE:%d %d %d LE:%d %d %d Ratio:%d SGG1Gain:%d\n", __FUNCTION__, m_eSensorDev,
                    m_strIVHDROutputSetting.bEnableWorkaround,
                    m_strIVHDROutputSetting.u4SEExpTimeInUS,
                    m_strIVHDROutputSetting.u4SESensorGain,
                    m_strIVHDROutputSetting.u4SEISPGain,
        ...
        if(m_bSetShutterValue) {    // update shutter value
            if (m_rAEInitInput.rAEPARAM.bPerFrameAESmooth){
                if ((m_u4UpdateShutterValue != m_PreEvSettingQueue[0].u4Eposuretime)||
                    ...
                    (m_u4UpdateShutterValue != m_PreEvSettingQueue[2].u4Eposuretime)    ){
...
MRESULT AeMgr::monitorAndReschedule( MBOOL bAAASchedule, MUINT32 u4AvgYcur, MUINT32 u4AvgYStable, MBOOL *bAECalc, MBOOL *bAEApply )
{
    ...
    if(bAAASchedule){
        ...
        *bAECalc  = MFALSE;
        *bAEApply = MFALSE;
        MY_LOG("[%s():3A Schedule] Calc:%d Apply:%d bAAASchedule:%d framePerAECycle:%d Stable:%d AEMonitorStable:%d VdCnt:%d ISPdelay:%d  FaceChange/FaceWOCnt:%d/%d TouchArea:%d u4CwvYcur:%d u4CwvYpre:%d u4AvgYcur:%d\n",
                      __FUNCTION__, *bAECalc, *bAEApply,  bAAASchedule, i4framePerAECycle, m_bAEStable, m_bAEMonitorStable, m_i4WaitVDNum,
                      m_i4IspGainDelayFrames, m_bFaceAEAreaChage, m_u4WOFDCnt, m_bTouchAEAreaChage ,u4AvgYcur, u4AvgYStable, m_pIAeAlgo->getBrightnessAverageValue());
        return 0;
    }
    ...
    MY_LOG("[%s()] Calc:%d Apply:%d Cnt:%d Frame:%d ReSchedule:%d Exit:%d Skip:%d bStatChange:%d bAAASchedule:%d framePerAECycle:%d Stable:%d AEMonitorStable:%d VdCnt:%d ISPdelay:%d FaceArea:%d FaceWOCnt:%d TouchArea:%d u4CwvYcur:%d u4CwvYpre:%d u4AvgYcur:%d\n",
