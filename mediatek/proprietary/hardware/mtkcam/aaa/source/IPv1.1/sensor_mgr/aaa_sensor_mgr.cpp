...
set2ShutterControl(MINT32 i4SensorDev, MUINT32 a_u4LEExpTime, MUINT32 a_u4SEExpTime)
{
    ...
    MY_LOG_IF(m_bDebugEnable, "[setVHDRParams] Id:%d Shutter:%d %d \n", i4SensorDev, a_u4LEExpTime, a_u4SEExpTime);
...
MRESULT
AAASensorMgr::
setStereoFrameSyncExpAndFps(MINT32 i4SensorDev, MUINT32 i4ExpTime ,MINT32 i4Fps)
{
    MINT32 ret = S_AAA_SENSOR_MGR_OK;

    // Set Exposure time and FPS (waiting for lungbro)
    if(i4SensorDev == ESensorDev_Main) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN, SENSOR_CMD_SET_SENSOR_EXP_TIME, (MUINTPTR)&i4ExpTime, 0, 0);
    } else if(i4SensorDev == ESensorDev_Sub) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB, SENSOR_CMD_SET_SENSOR_EXP_TIME, (MUINTPTR)&i4ExpTime, 0, 0);
    } else if(i4SensorDev == ESensorDev_MainSecond) {
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_2, SENSOR_CMD_SET_SENSOR_EXP_TIME, (MUINTPTR)&i4ExpTime, 0, 0);
    } else {
        MY_ERR("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
    MY_LOG("[setStereoFrameSyncFps] SensorDev:%d Exposure Time:%d Fps:%d \n", i4SensorDev, i4ExpTime,i4Fps);
...
MRESULT
AAASensorMgr::
updateStereoFrameSyncFps(MINT32 i4SensorDev,MUINT32 i4ExpTimeMain,MUINT32 i4ExpTimeMain2)
{
    MINT32 ret = S_AAA_SENSOR_MGR_OK;

    // Inform exposure time to Hardware sync
    if(i4SensorDev == ESensorDev_Main) {
        m_i4FrameSyncMainFps = m_pHwSyncDrvObj->sendCommand(HW_SYNC_CMD_GET_FRAME_TIME,NSCam::SENSOR_DEV_MAIN, i4ExpTimeMain, 0x0);
    } else {
        MY_LOG("[%s()] Sensor type warning:%d", __FUNCTION__, i4SensorDev);
    }
    MY_LOG("[setSensorExpTime:FrameSync] i4SensorDev:%d exp:%d/%d fps:%d/%d (main/main2)\n", i4SensorDev, i4ExpTimeMain,i4ExpTimeMain2,m_i4FrameSyncMainFps,m_i4FrameSyncMain2Fps);
