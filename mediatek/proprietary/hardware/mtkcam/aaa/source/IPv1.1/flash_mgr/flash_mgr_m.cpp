...
#define logI(fmt, arg...) MY_LOG(fmt, ##arg)
#define logV(fmt, arg...) {if(g_isVerboseEn) logI(fmt, ##arg);}
#define logE(fmt, arg...) MY_ERR("MError: func=%s line=%d: " fmt, __FUNCTION__, __LINE__, ##arg)
#define LogW(fmt, arg...) MY_ERR("FlashWarning: func=%s line=%d: " fmt, __FUNCTION__, __LINE__, ##arg)
//==============================
...
void FlashMgrM::hw_isoToGain(int iso, int* afe, int* isp)
{
    ...
    if(devInfo.u4MaxGain >= 16320) {
        ...
        logI("sensor max gain larger than 16320, set to 16320");
        maxGain = 16320;
    }
    else
...
void FlashMgrM::hw_capIsoToGain(int iso, int* afe, int* isp)
{
    ...
    logI("hw_capIsoToGain dev_maxG %d", (int)maxGain);
...
int FlashMgrM::getDebugInfo(FLASH_DEBUG_INFO_T* p)
{
    ...
    logI("m_bAlgoTagReady=%d ln=%d",m_bAlgoTagReady, __LINE__);
...
{
    logI("isFlashOnCalibration() line=%d mIsCalibration=%d",__LINE__,mIsCalibration);
...
int FlashMgrM::setPfParaToAe()
{
    ...
    logI("setPfParaToAe afe, isp %d %d %d, ind=%d", exp, afe, isp, bestInd);
    return 0;
}
void dispStrobeNvram(NVRAM_CAMERA_STROBE_STRUCT* nv)
{
