...
{
    MY_LOG("[%s]", __FUNCTION__);
    MY_LOG("[%s] IsRecordingFlash=%d", __FUNCTION__, m_bIsRecordingFlash);
    return
...
postCommand(ECmd_T const r3ACmd, const ParamIspProfile_T* pParam)
{
    ...
    {
        MY_LOG_IF(m_3ALogEnable,"[%s] capture flow", __FUNCTION__);
        ...
        MY_LOG_IF(m_3ALogEnable,"[%s] i4OperMode(%d)", __FUNCTION__, i4OperMode);
#if CAM3_FLASH_FEATURE_EN
        if(!pParam->bMainFlash && !mbIsHDRShot && (i4OperMode != EOperMode_Meta))
#endif
        {
...
send3ACtrl(E3ACtrl_T e3ACtrl, MINTPTR iArg1, MINTPTR iArg2)
{
    ...
    switch (e3ACtrl)
    {
        ...
        case E3ACtrl_SetOperMode:
            ...
            MY_LOG("[%s] prev_mode(%d), new_mode(%d)", __FUNCTION__, i4OperMode, iArg1);
            break;
