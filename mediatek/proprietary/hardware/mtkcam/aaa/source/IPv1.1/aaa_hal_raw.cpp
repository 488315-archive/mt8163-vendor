...
{
    MY_LOG("[%s] i4MagicNum(%d)", __FUNCTION__, u4FrmId);
...
#warning "FIXME"    
    ...
    // Flicker
#warning "FIXME"
    MINT32 i4FlkStatus = 0; //IAeMgr::getInstance().getAEAutoFlickerState(m_i4SensorDev);
...
{
    MY_LOG("[%s] #(%d)", __FUNCTION__, u4MagicNum);
    // pass1 result
...
MBOOL Hal3ARaw::postCommandDerived(ECmd_T const eCmd, MINTPTR const i4Arg)
{
    ...
    if (eCmd == ECmd_Update)
    {
        #warning "FlashMgr::doPreviewOneFrame not ready"
        ...
        #endif
        {
            ...
            if (mbSemAFIRQWait == 0)
            {
                ...
                MY_LOG_IF(1, "[AFThread] sem_post mSemAFThreadstart, semAFProcEnd before wait = %d\n", Val);
            }
        }
...
MBOOL Hal3ARaw::setParams(Param_T const &rNewParam)
{
    ...
    if (rNewParam.u1TonemapMode == MTK_TONEMAP_MODE_CONTRAST_CURVE)
    {
        ...
        for (i = i4Cnt; i != 0; i--)
        {
            ...
            *pArrayIn++ = x;
            *pArrayOut++ = y;
            MY_LOG("[Blue]#%d(%f,%f)", rNewParam.u4MagicNum, x, y);
        }
        ...
        for (i = i4Cnt; i != 0; i--)
        {
            ...
            *pArrayIn++ = x;
            *pArrayOut++ = y;
            MY_LOG("[Green]#%d(%f,%f)", rNewParam.u4MagicNum, x, y);
        }
        ...
        for (i = i4Cnt; i != 0; i--)
        {
            ...
            *pArrayIn++ = x;
            *pArrayOut++ = y;
            MY_LOG("[Red]#%d(%f,%f)", rNewParam.u4MagicNum, x, y);
        }
...
*/
    ...
    MY_LOG("[%s] m_rParam.u1ShadingMapMode(%d)", __FUNCTION__, m_rParam.u1ShadingMapMode);
...
{
    MY_LOG("[%s] fgEnable(%d)", __FUNCTION__, fgEnable);
...
MBOOL Hal3ARaw::setFlashLightOnOff(MBOOL bOnOff/*1=on; 0=off*/, MBOOL bMainPre/*1=main; 0=pre*/)
{
    ...
    if (!bOnOff) FlashMgr::getInstance().turnOffFlashDevice(m_i4SensorDev);
    else //flash on
...
{
    MUINT32 u4Id = 0;
    NSIoPipe::NSCamIOPipe::IHalCamIO* pPipe = (NSIoPipe::NSCamIOPipe::IHalCamIO*)NSIoPipe::NSCamIOPipe::INormalPipe::createInstance(m_i4SensorOpenIdx, LOG_TAG);//iopipe2.0
...
*/
            IEventIrq::ConfigParam IrqConfig(m_i4SensorDev, m_i4SensorOpenIdx, 5000, IEventIrq::E_Event_Af);
