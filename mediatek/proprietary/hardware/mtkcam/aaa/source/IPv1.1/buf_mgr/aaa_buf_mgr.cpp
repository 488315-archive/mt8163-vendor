...
#include <CamIO/ICamsvStatisticPipe.h>
...
public:
    ...
    MBOOL init(MINT32 const i4SensorIdx, MINT32 const i4SensorMode, MINT32 const i4SubsampleCount=1);
    MBOOL uninit();
    MBOOL start();
    MBOOL stop();
    MBOOL debugPrint();
...
MBOOL
IAAABufMgr::
init(MINT32 const i4SensorDev, MINT32 const i4SensorIdx, MINT32 const i4SensorMode, MINT32 const i4SubsampleCount)
{
    MBOOL ret_main, ret_sub, ret_main2;
    ret_main = ret_sub = ret_main2 = MTRUE;

    if (i4SensorDev & ESensorDev_Main)
        ret_main = AAABufMgr::getInstance(ESensorDev_Main).init(i4SensorIdx, i4SensorMode, i4SubsampleCount);
    if (i4SensorDev & ESensorDev_Sub)
        ret_sub = AAABufMgr::getInstance(ESensorDev_Sub).init(i4SensorIdx, i4SensorMode, i4SubsampleCount);
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AAABufMgr::getInstance(ESensorDev_MainSecond).init(i4SensorIdx, i4SensorMode, i4SubsampleCount);
...
MBOOL
AAABufMgr::
init(MINT32 const i4SensorIdx, MINT32 const i4SensorMode, MINT32 const i4SubsampleCount)
{
    ...
    // virtual channal init
    MY_LOG("[%s] Virtual Channal Pipe createInstance\n", __FUNCTION__);
    mpCamsvSttPipe = ICamsvStatisticPipe::createInstance(m_i4SensorIdx, LOG_TAG); // alwasy use main
    if( !mpCamsvSttPipe ) {
        MY_ERR("ICamsvStatisticPipe createInstance fail");
        return MFALSE;
    }
    MY_LOG("[%s] Virtual Channal Pipe init\n", __FUNCTION__);
    if (MFALSE == mpCamsvSttPipe->init()) {
        MY_ERR("ICamsvStatisticPipe init fail");
        return MFALSE;
    }

    MBOOL bRet = mpCamsvSttPipe->sendCommand(ECAMSVSPipeCmd_SET_CAMSV_SENARIO_ID, (MINTPTR)&i4SensorMode, NULL,NULL);

    // Statistic Pipe and Virtual Channal Pipe config
    MUINT enable;
    property_get("debug.stt_flow.enable", value, "3");
    enable = atoi(value);
    ...
    mpSttPipe->configPipe(statParm, i4SubsampleCount);
    m_bCamsvEnable = mpCamsvSttPipe->configPipe(statParm);
    if(!m_bCamsvEnable)
        MY_ERR("Disable PDAF deque/enque");
...
uninit()
{
    ...
    if (m_Users == 0) // There is no more User after decrease one User
    {
      ...
      mpSttPipe->destroyInstance(LOG_TAG);
        ...
        mpCamsvSttPipe->uninit();
        mpCamsvSttPipe->destroyInstance(LOG_TAG);
    }
...
dequeueHwBuf(STT_PORTS port)
{
    ...
    static MUINT32 count;
        switch(port){
          case STT_AAO:
            sprintf(fileName, "/sdcard/aao/aao_%d.raw", rLastBuf.mMetaData.mMagicNum_tuning);
            break;
          case STT_AFO:
            sprintf(fileName, "/sdcard/afo/afo_%d.raw", rLastBuf.mMetaData.mMagicNum_tuning);
            break;
          case STT_FLKO:
            sprintf(fileName, "/sdcard/flko/flko_%d.raw", rLastBuf.mMetaData.mMagicNum_tuning);
            break;
        }
 ...
 * PDAF
 *********************************/
    if(portIDMap[port] == PORT_AFO && m_bCamsvEnable)
    {
        // deque Virtual Channal Pipe
        PortID _dq_portID;
        if( MFALSE == mpCamsvSttPipe->deque(_dq_portID, rDQBuf) ){
            MY_ERR("mpCamsvSttPipe deque fail\n");
            return MFALSE;
        }

        size = rDQBuf.mvOut.size();
        BufInfo rPDAFBuf = rDQBuf.mvOut.at( size-1 );

        StatisticBufInfo rHwPDBuf;
        rHwPDBuf.mMagicNumber = rPDAFBuf.mMetaData.mMagicNum_tuning;
        rHwPDBuf.mSize = rPDAFBuf.mSize;
        rHwPDBuf.mVa = reinterpret_cast<MUINTPTR>(rPDAFBuf.mVa);

        IAfMgr::getInstance().doPDVC(m_eSensorDev, reinterpret_cast<MVOID *>(&rHwPDBuf));
        ...
        if(m_bDebugEnable)
        {
            ...
            {
                char fileName[64];
                sprintf(fileName, "/sdcard/pdaf/pdaf_%d.raw", cnt);
                ...
                if (NULL == fp)
                {
                    MY_ERR("fail to open file to save img: %s", fileName);
                    MINT32 err;
                    err = mkdir("/sdcard/pdaf", S_IRWXU | S_IRWXG | S_IRWXO);
                    MY_LOG("err = %d", err);
                }
                else
                {
                    MY_LOG_IF(1,"%s\n", fileName);
                    fwrite(reinterpret_cast<void *>(rPDAFBuf.mVa), 1, rPDAFBuf.mSize, fp);
                    fclose(fp);
                }
            }
        }

        // enque Virtual Channal Pipe
        if( MFALSE == mpCamsvSttPipe->enque(rDQBuf)){
            MY_ERR("mpCamsvSttPipe enque fail\n");
            return MFALSE;
        }
    }
