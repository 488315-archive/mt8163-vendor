...
TEST(HAL_TEST, N3DHAL_PREVIEW_TEST)
{
    ...
    const MUINT32 CC_SIZE = p2a3Info.szIMG2O.w * p2a3Info.szIMG2O.h;
    n3dHalParam.ccImgMain1 = new MUINT8[CC_SIZE];
    n3dHalParam.ccImgMain2 = new MUINT8[CC_SIZE];
    ...
    do {
        ...
        expectSize = CC_SIZE;
        readSize = fread(n3dHalParam.ccImgMain1, 1, expectSize, fp);
        fclose(fp);
        if(readSize != expectSize) {
            if(errno) {
                MY_LOGE("error: %s", ::strerror(errno));
        ...
        expectSize = CC_SIZE;
        readSize = fread(n3dHalParam.ccImgMain2, 1, expectSize, fp);
        fclose(fp);
        if(readSize != expectSize) {
            if(errno) {
                MY_LOGE("error: %s", ::strerror(errno));
