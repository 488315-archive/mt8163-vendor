...
*
********************************************************************************/
MINT32 EisDrvImp::CreateMemBuf(MUINT32 &memSize,const MUINT32 &bufCnt, IMAGE_BUF_INFO *bufInfo)
{
    MINT32 err = EIS_RETURN_NO_ERROR;
    EIS_LOG("-");
    return err;
...
*
*******************************************************************************/
MINT32 EisDrvImp::DestroyMemBuf(const MUINT32 &bufCnt, IMAGE_BUF_INFO *bufInfo)
{
    MINT32 err = EIS_RETURN_NO_ERROR;
...
MINT32 EisDrvImp::ConfigEisReg(const EIS_SCENARIO_ENUM &aSce,const MUINT32 &aSensorTg)
{
    ...
    else
    {
        ...
        m_pNormalPipe->sendCommand(NSImageio::NSIspio::EPIPECmd_SET_EIS_CBFP,(MINTPTR)m_pEisP1Cb,-1,-1);

        m_pSGG2P1Cb = new TuningNotifyImp_SGG2();
        m_pNormalPipe->sendCommand(NSImageio::NSIspio::EPIPECmd_SET_SGG2_CBFP,(MINTPTR)m_pSGG2P1Cb,-1,-1);
...
MBOOL EisDrvImp::GetEisSupportInfo(const MUINT32 &aSensorIdx)
{
    ...
    m_pNormalPipe->sendCommand(NSImageio::NSIspio::EPIPECmd_GET_EIS_INFO,aSensorIdx,(MINTPTR)&eisInfo, -1);
...
MVOID EisDrvImp::GetRegSetting(void *data)
{
    ...
    {
        EIS_ERR("mRrz_scale_Width(%d) is not sync to mImgWidth(%d)",mRrz_scale_Width,mImgWidth);
        EIS_ERR("mRrz_scale_Height(%d) is not sync to mImgHeight(%d)",mRrz_scale_Height,mImgHeight);
    }
...
*
********************************************************************************/
MINT32 EisDrvImp::GetEisHwStatistic(MINTPTR bufferVA, EIS_STATISTIC_STRUCT *apEisStat)
{
    MINT32 i;

    if (!bufferVA)
    {
        EIS_LOG("bufferVA(%p) is NULL!!!",bufferVA);
...
MVOID EisDrvImp::GetFeoStatistic(FEO_STAT_DATA *aFeoStatData)
{
    ...
    MUINT32 *pFeoAddr = NULL;//TBD Must be fixed later
...
void EisP1Cb::p1TuningNotify(MVOID *pInput,MVOID *pOutput)
{
    ...
    if(g_debugDump > 0)
    {
        ...
        EIS_LOG("YUV(%d),pixMode(%d)",aEisInInfo->bYUVFmt,aEisInInfo->pixMode);
    }

    EIS_LOG("TG(%u,%u),RRZ In(%u,%u), RRZ crop x/y(%u,%u), RRZ crop w/h(%u,%u), RMX(%u,%u), pixelMode(%d)",
    ...
    if(aEisInInfo->bYUVFmt== MFALSE)    // RAW sensor specific register
    {
        ...
        if ( (aEisInInfo->pixMode > 0) && (aEisInInfo->pixMode <3))
        {
            if (_this->mIsFirst == 1)
            {
                EIS_LOG("pixel mode: %d", aEisInInfo->pixMode);
            }

            tempW = tempW >> aEisInInfo->pixMode;
            if (tempW != aEisInInfo->sHBINOut.w)
            {
                EIS_WRN("RRZ shift HDS(%D) is different to HBIN(%d)", tempW, aEisInInfo->sHBINOut.w);
                tempW = aEisInInfo->sHBINOut.w;
            }
    ...
    _this->mSensor_Width = aEisInInfo->RRZ_IN_CROP.in_size_w; //input size of RRZ, binning from sensor or ISP
    _this->mSensor_Height = aEisInInfo->RRZ_IN_CROP.in_size_h; //input size of RRZ, binning from sensor or ISP
    ...
    if(g_debugDump > 0)
    {
        ...
        EIS_LOG("sce(%u),Pixel Mode(%u)",_this->mEisHwCfgSce,_this->mIs2Pixel);
    }
