        ...
        MUINT32 w, MUINT32 h, MUINT32 format, MBOOL isContinuous)
{
    ...
    for (MUINT32 i = 0; i < planeCount; i++)
    {
        ...
        HDR_LOGV("allocBuffer strideInBytes[%d] strideInBytes(%d)",
                i, strideInBytes[i]);
    }
    ...
    if (isContinuous)
    {
        ...
        HDR_LOGV("allocBuffer all plane size(%d)", allPlaneSize);
    ...
    else
    {
        ...
        HDR_LOGD("allocBuffer addr(%p) size(%dx%d) format(0x%x)",
                pBuf.get(), w, h, format);
