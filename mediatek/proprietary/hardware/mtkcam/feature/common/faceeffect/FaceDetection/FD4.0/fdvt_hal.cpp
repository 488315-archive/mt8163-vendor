...
//**********************************************************************//
   ...
   /* if(SD_mode!=1)
    ...
    IMAGE_MAX_WIDTH  = MHAL_FDVT_FTBUF_W;//image_width_array[0];
    IMAGE_MAX_HEIGHT = MHAL_FDVT_FTBUF_H;//image_height_array[0];
...
*/
    ...
    FDFTInitInfo.FDTBufWidth = IMAGE_MAX_WIDTH;//image_width_array[0];// µ¹OT¥Îªº, 320x240
    FDFTInitInfo.FDTBufHeight = IMAGE_MAX_HEIGHT;//image_height_array[0];
...
MUINT8 *ImageBufferPhy2,
MUINT8 *ImageBufferPhy3,
MBOOL   AE_Stable
)
{
    Mutex::Autolock _l(gInitLock);
    ...
    FDOps.ae_stable = AE_Stable;
    if (ImageBufferPhy3 != NULL) {
        FDOps.inputPlaneCount = 3;
    } else if (ImageBufferPhy2 != NULL) {
        FDOps.inputPlaneCount = 2;
    } else {
        FDOps.inputPlaneCount = 1;
    }
    FDOps.ImageBufferPhyPlane1 = ImageBufferPhy;
    FDOps.ImageBufferPhyPlane2 = ImageBufferPhy2;
    FDOps.ImageBufferPhyPlane3 = ImageBufferPhy3;
    FDOps.ImageScaleBuffer = ImageScaleBuffer;
    FDOps.ImageBufferRGB565 = ImageBuffer1;
    if(EnableSWResizerFlag)
    {
        ...
        LockFTBuffer(this);
        MY_LOGD("FDVTMain IN : %p", ImageBufferPhy);

        m_pMTKFDVTObj->FDVTMain(&FDOps);
    ...
    {
        FDOps.ImageScaleBuffer = ScaleImages;
        m_pMTKFDVTObj->FDVTMain(&FDOps);

        if(SDEnable)
        {
            FDOps.fd_state = FDVT_SD_MODE;
            FDOps.ImageBufferRGB565 = ImageBuffer2;
            m_pMTKFDVTObj->FDVTMain(&FDOps);
        }
    }
