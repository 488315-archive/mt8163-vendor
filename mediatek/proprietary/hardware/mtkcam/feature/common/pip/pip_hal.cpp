...
#include <IHalSensor.h>     // (Under mediatek\hardware\include\) For SensorStaticInfo struct.

using namespace NSCam;

#include "pip_hal_imp.h"

#include <v1/camutils/CamInfo.h> // For queryHalSensorDev().
 ...
 **************************************************************************/
#define PIP_SENSOR_SIZE_25M   (26214400 + 1310720)    // (25 * 1024 * 1024) * (100% + 5%)
...
MBOOL PipHalImp::GetHwMaxFrameRate(MINT32 i4SensorDev, MUINT32 &ZsdHwMaxFrameRate, MUINT32 &NonZsdHwMaxFrameRate)
{
    ...
    if (i4SensorType == NSCam::SENSOR_TYPE_RAW) // RAW sensor.
    {
        ...
        {
            if (i4SensorSize > PIP_SENSOR_SIZE_25M)   // SensorSize > 25M. PIP only support 25M+25M. So main > 25M will return 0 (indicate error).
            {
            ...
            else if (i4SensorSize > PIP_SENSOR_SIZE_16M)   // 25M >= SensorSize > 16M
            {
        ...
        {
            if (i4SensorSize > PIP_SENSOR_SIZE_25M)   // SensorSize > 25M. PIP only support 25M+25M. So sub > 25M will return 0 (indicate error).
            {
                ZsdHwMaxFrameRate    = 0;
                NonZsdHwMaxFrameRate = 0;
            }
            else if (i4SensorSize > PIP_SENSOR_SIZE_13M)   // 25M >= SensorSize > 13M
            {
                ZsdHwMaxFrameRate    = 15;
                NonZsdHwMaxFrameRate = 30;
            }
