...
MVOID WaitHub::triggerSignal(MUINT32 signal)
{
  TRACE_N_FUNC_ENTER(mName);
  Autolock lock(mMutex);
  TRACE_N_FUNC(mName, "signal: 0x%03x => 0x%03x", mSignal, mSignal|signal);
  mSignal |= signal;
  mCondition.broadcast();
  TRACE_N_FUNC_EXIT(mName);
}
...
MUINT32 WaitHub::waitSignal()
{
  TRACE_N_FUNC_ENTER(mName);
  Autolock lock(mMutex);
  const MUINT32 TRIGGER = SIGNAL_STOP | SIGNAL_CB | SIGNAL_DATA | SIGNAL_IDLE_CB | SIGNAL_SYNC_CB | SIGNAL_NEED_SYNC_BREAK;
