...
{
    TRACE_FUNC_ENTER();
    this->addWaitQueue(&mRequestQueue);
    TRACE_FUNC_EXIT();
}
...
MBOOL HelperNode::onData(DataID id, const RequestPtr &request)
{
    TRACE_FUNC_ENTER();
    TRACE_FUNC("Frame %d: %s arrived", request->mRequestNo, ID2Name(id));
    MBOOL ret = MFALSE;
    if( id == ID_P2A_TO_HELPER )
    {
        mRequestQueue.enque(request);
        ret = MTRUE;
    }
    TRACE_FUNC_EXIT();
    return ret;
...
MBOOL HelperNode::onThreadLoop()
{
    ...
    if( !mRequestQueue.deque(request) )
    {
    ...
    TRACE_FUNC_ENTER();
    request->mTimer.startHelper();
    TRACE_FUNC("Frame %d in Helper", request->mRequestNo);
    processHelper(request);
