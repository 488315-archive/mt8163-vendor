...
MBOOL StreamingFeatureNode::dumpData(const RequestPtr &request, IImageBuffer *buffer, const char *prefix)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MFALSE;
    if( buffer && prefix )
    {
        ...
        TRACE_FUNC("dump to /sdcard/dump/%04d_%s_%dx%d.bin", request->mRequestNo, prefix, width, height);
        ret = dumpToFile(buffer, "/sdcard/dump/%04d_%s_%dx%d.bin", request->mRequestNo, prefix, width, height);
    }
    TRACE_FUNC_EXIT();
    return ret;
}
