...
MBOOL GPUNode::processGPU(const RequestPtr &request, const ImgBuffer &warpMap, const ImgBuffer &fullImg)
{
    ...
    MSize outSize;
    outSize.w = fullImgSize.w/((MFLOAT)EIS_FACTOR/100.0f);
    outSize.h = fullImgSize.h/((MFLOAT)EIS_FACTOR/100.0f);
    ...
    if( configGpuWarp(request) && inGBAddr && outGBAddr )
    {
    ...
    if( request->needDump() )
    {
        ...
        dumpData(request, outBuffer.mData->getImageBufferPtr(), "gpu");
    }
...
MBOOL GPUNode::configGpuWarp(const RequestPtr &request)
{
    TRACE_FUNC_ENTER();
    MBOOL ret = MTRUE;
    ...
    if( feature != mCurrentFeature )
    {
        ...
        else
        {
            ...
            if( (ret = mGpuWarp.config(inAddr, outAddr, feature)) == MFALSE )
            {
