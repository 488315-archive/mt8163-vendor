...
{
    TRACE_FUNC_ENTER();
    MY_LOGD_IF(miPipeLogEnable, "requestId(%d) +", request->getRequestNo());
    MBOOL ret = MFALSE;
    //
    ...
    EffectRequestPtr request)
{
    ...
    if(!requireAlgoDataFromRequest(request, sInData, pMYS, pDMW))
    {
        ...
        sInData.downScaledImg = nullptr;
        goto lbExit;
    }
    //
    sInData.depthMap = (MUINT8*)pDMW->getBufVA(0);
    sInData.downScaledImg = pMYS.get();
    //
    ...
    //
    MY_LOGD_IF(miPipeLogEnable, "executeAlgo processing +");
    ...
    if(mbDumpImageBuffer)
    {
        ...
        // dump image buffer
        makePath("/sdcard/vsdof/bokeh/result/gf/", 0660);
        string saveFileName = "";
        if(mGFMode == ADVANCE)
        {
            saveFileName = string("/sdcard/vsdof/bokeh/result/gf/DMG_")+
                           to_string(request->getRequestNo())+
                           string(".yuv");
            smDMGImgBuf->mImageBuffer->saveToFile(saveFileName.c_str());
        }
        saveFileName = string("/sdcard/vsdof/bokeh/result/gf/DMBG_")+
                       to_string(request->getRequestNo())+
                       string(".yuv");
        smDMBGImgBuf->mImageBuffer->saveToFile(saveFileName.c_str());
        saveFileName = string("/sdcard/vsdof/bokeh/result/gf/MYS_")+
                       to_string(request->getRequestNo())+
                       string(".yuv");
        pMYS->saveToFile(saveFileName.c_str());
        saveFileName = string("/sdcard/vsdof/bokeh/result/gf/DMW_")+
                       to_string(request->getRequestNo())+
                       string(".yuv");
