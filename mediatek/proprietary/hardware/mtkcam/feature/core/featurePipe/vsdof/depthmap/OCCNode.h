...
private:
    ...
    MVOID cleanUp();
    MBOOL prepareOCCParams(ImgInfoMapPtr& pDPEImgInfo, ImgInfoMapPtr& pN3DImgInfo,OCC_HAL_PARAMS& enqueParams, OCC_HAL_OUTPUT& outputParams, ImgInfoMapPtr& toWMFImgInfo);
    // debug
    MVOID debugOCCParams(sp<IImageBuffer> smMV_Y, sp<IImageBuffer> smSV_Y, sp<IImageBuffer> smDMP_L,
                        sp<IImageBuffer> smDMP_R, sp<IImageBuffer> confidenceMap, sp<IImageBuffer> ldcMap,
                        sp<IImageBuffer> downScaledImg, sp<IImageBuffer> depthMap);
private:
    // Job queue
