...
)
{
    ...
    if(SENSOR_MAIN & sensorIdx ) {
        ...
        if (ret < 0) {
           LOG_ERR("SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ error\n");
           return SENSOR_UNKNOWN_ERROR;
        }
        ...
        if (ret < 0) {
            LOG_ERR("SENSOR_FEATURE_GET_PERIOD error\n");
            return SENSOR_UNKNOWN_ERROR;
        }
...
)
{
    ...
    LOG_MSG("imgsensor_drv mUsers = %d\n", mUsers);
...
ImgSensorDrv::setScenario(SENSOR_DRIVER_SCENARIO_T scenarioconf)
{
    ...
    for(i=0; i<1; i++) {
        if(SENSOR_SCENARIO_ID_UNNAMED_START != tempId[i]) {//ToDo: remove
            ...
            if (ret < 0) {
                 LOG_ERR("SENSOR_FEATURE_SET_FRAMERATE error\n");
                 return SENSOR_UNKNOWN_ERROR;
            }
            //HDR Enable
            ...
            if (FeaturePara32) {
                if(FeaturePara32 >= 1000) {
                    ...
                    LOG_ERR("Sensor clock too slow = %d %d\n", FeaturePara32, pFeaturePara16[0]);
                }
                LOG_MSG("DevID = %d, m_LineTimeInus[0] = %d, m_LineTimeInus[1] = %d Scenario id = %d, PixelClk = %d, PixelInLine = %d\n",
...
{
    MINT32 ret = 0;
    LOG_MSG("[ImgSensorDrv]EventType = %d, Timeout=%d\n",EventType,Timeout);
    switch (EventType) {
...
ImgSensorDrv::close(MINT32 sensorIdx)
{
    ...
    err = ioctl(m_fdSensor, KDIMGSENSORIOC_X_SET_CURRENT_SENSOR, &sensorIdx);
    if (err < 0) {
        LOG_ERR("[KDIMGSENSORIOC_X_SET_CURRENT_SENSOR] Err-ctrlCode (%s) \n", strerror(errno));
        return -errno;
    }
...
{
    LOG_MSG("ACDK_SENSOR_RESOLUTION_INFO_STRUCT");
    MINT32 err = SENSOR_NO_ERROR;
