...
createInstance(const ConfigParam& rCfg, const char* strUser)
{
    ...
    {
        case E_Event_Vsync:
            HwEventIrqType<E_Event_Vsync>::getInstance()->init(rCfg, strUser);
            return HwEventIrqType<E_Event_Vsync>::getInstance();
        case E_Event_Af:
            HwEventIrqType<E_Event_Af>::getInstance()->init(rCfg, strUser);
            return HwEventIrqType<E_Event_Af>::getInstance();
    }
...
init(const ConfigParam& rCfg, const char* strUser)
{
    ...
    m_rWaitIrq.UserInfo.Status  = (m_rCfgParam.eEventType == E_Event_Vsync) ? /*ISP_DRV_IRQ_INT_STATUS_VS1_ST*/ISP_DRV_IRQ_INT_STATUS_SOF1_INT_ST : ISP_DRV_IRQ_INT_STATUS_AF_DON_ST/*AF*/;
