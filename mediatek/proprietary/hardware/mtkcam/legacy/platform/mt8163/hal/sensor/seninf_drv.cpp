...
//
    ...
    LOG_MSG("mpCSI2RxAnalog0RegAddr 0x%x = mpCSI2RxAnalogRegStartAddr 0x%x, \n", mpCSI2RxAnalog0RegAddr, mpCSI2RxAnalogRegStartAddr);
...
#endif
        // 8163: 364Mhz
        // ISP CLK = 364Mhz , settel delay count =  (x ns *  364 M )/1000
        msettleDelay = dataSettleDelay * 364 /1000;
...
int SeninfDrvImp::setMclk1IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    if(mpCAMIODrvRegAddr != NULL) {
        ...
        *(mpCAMIODrvRegAddr) |= (ioDrivingCurrent<<6); // [6:4] = 0:2mA, 1:4mA, 2:6mA, 3:8mA
    }
...
int SeninfDrvImp::setMclk2IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    if(mpCAMIODrvRegAddr != NULL) {
        *(mpCAMIODrvRegAddr) &= ~0x0C;
        *(mpCAMIODrvRegAddr) |= (ioDrivingCurrent<<2); // [2:1] = 0:2mA, 1:4mA, 2:6mA, 3:8mA
    }
    LOG_MSG("[setIODrivingCurrent]:%d 0x%08x\n", (int) ioDrivingCurrent, (int) (*(mpCAMIODrvRegAddr)));
...
int SeninfDrvImp::setMclk3IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    if(mpCAMIODrvRegAddr != NULL) {
        ...
        *(mpCAMIODrvRegAddr) |= (ioDrivingCurrent<<5); // [2:1] = 0:2mA, 1:4mA, 2:6mA, 3:8mA
    }
