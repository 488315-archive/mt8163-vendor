...
static MBOOL gListenLogOn = MFALSE;     // can change by setting property: debug.af_mgr.enable

DAF_TBL_STRUCT m_DAF_TBL;
Mutex m_LockListenData;

void afListenHandler(ASensorEvent event)
{
    Mutex::Autolock lock(m_LockListenData);
    ...
    switch(event.type)
    {
        ...
        {
			gPreAcceTS = gAcceTS;
			MY_LOG_IF(gListenLogOn, "[debug] gPreAcceTS  in ASENSOR_TYPE_ACCELEROMETER:%lld", gPreAcceTS);
        ...
        {
			gPreGyroTS = gGyroTS;
			MY_LOG_IF(gListenLogOn, "[debug] gPreGyroTS in ASENSOR_TYPE_GYROSCOPE:%lld", gPreGyroTS);
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    if(m_NVRAM_LENS.rAFNVRAM.i4SensorEnable > 0)
    {
		...
		//fix get acce data error
		//gPreAcceTS = gAcceTS;
		MY_LOG_IF((m_bDebugEnable&1),"[debug] Cancel gAcceTS in do AF");
