...
#undef LOG_ERR
#undef LOG_AST
#define LOG_VRB(fmt, arg...)        do { if (P2PipeWrapper_FrmB_DbgLogEnable_VERBOSE) { BASE_LOG_VRB(fmt, ##arg); } } while(0)
#define LOG_DBG(fmt, arg...)        do { if (P2PipeWrapper_FrmB_DbgLogEnable_DEBUG  ) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
#define LOG_INF(fmt, arg...)        do { if (P2PipeWrapper_FrmB_DbgLogEnable_INFO   ) { BASE_LOG_INF(fmt, ##arg); } } while(0)
    ...
    EBufferTag bufferTag)
{
    ...
    LOG_INF("bufPackageNum(%d),misV3(%d),magic(%d),tuningData(%d)",bufPackageNum,misV3,rParams.mvMagicNo.size(),rParams.mpTuningData.size());
