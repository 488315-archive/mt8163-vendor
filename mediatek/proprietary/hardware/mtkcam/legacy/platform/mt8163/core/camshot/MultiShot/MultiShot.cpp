...
extern "C" {
    // here, undef INT32, because MediaTypes.h will define it, it has conflicts with jpeg include file
    #undef INT32
    #include "jpeglib.h"
    #include "jerror.h"
}
...
#ifndef MTKCAM_INHOUSE_GPU // Mali: 32 bytes alignment
...
#else   // sph : 128 alignment
        // Because Disp buffer is from gralloc, which size is 128 alignment
        // In Cshot, the disp buf is copied from PostView buf , but PostView buf is 32 alignment
        // that causes size unmatched and disp buf is cancelled...

// 8163P3 sph is 128 bytes alignment
#define ALIGN_128(x) ( (~127) & ((127) + (x)) )

    MUINT32 u4BufSize  = ALIGN_128(mShotParam.u4PostViewWidth) * mShotParam.u4PostViewHeight * 1.5;

    MY_LOGD("[InHouse GPU 128 alignment] u4BufSize (%d) <= (%d x %d x 1.5) ", 
            u4BufSize, ALIGN_128(mShotParam.u4PostViewWidth), mShotParam.u4PostViewHeight);
            
    // [CS]+
    IMEM_BUF_INFO postviewMem;
    memset(&postviewMem, 0, sizeof(IMEM_BUF_INFO));

    allocImgMem("PostView_128Align", mShotParam.ePostViewFmt, ALIGN_128(mShotParam.u4PostViewWidth), mShotParam.u4PostViewHeight, postviewMem);

    ImgBufInfo rImgBufInfo;
    //
    setImageBuf(mShotParam.ePostViewFmt, ALIGN_128(mShotParam.u4PostViewWidth), mShotParam.u4PostViewHeight,  rImgBufInfo, postviewMem);
    mPostViewMem.push_back(postviewMem);
    // {CS]-
#endif
