...
submitRequest(IPipelineModelMgr::AppRequest& request)
{
    ...
    if( bReconfig )
    {
        ...
        {
            Mutex::Autolock _l(mPipelineLock);
            mpCurrentPipeline = mConfigParams.pPipelineModel = pPipeline = NULL;
        }
        //
...
)
{
    ...
    //
    MY_LOGD("pipeline scene(%d); current scene(%d)", pipelineScene, curConfig.pipelineScene);
