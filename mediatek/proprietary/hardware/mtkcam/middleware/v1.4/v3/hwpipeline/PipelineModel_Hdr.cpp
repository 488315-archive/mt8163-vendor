...
~PipelineHdrImp()
{
    MY_LOGD("hdr pipeline destroyed-0");
}
...
{
    MY_LOGD("hdr pipeline destroyed-1");
}
   ...
   for (size_t frm_idx = 0; frm_idx < mvHdrCaptureParams.size(); frm_idx++ ) {
    ...
    builder.setNodeEdges( evaluateResult.edges );
        ...
        if (frm_idx==0) {
            ...
            {
                MY_LOGD("replace streaminfo id:%#" PRIxPTR, evaluateResult.vUpdatedImageInfos.keyAt(i));
