...
*/
    ...
    if(mDupImage.planes == 1) {
        pDstY = mPureYBuf;
        if (mpFDHalObj->halFDGetVersion() != HAL_FD_VER_HW40 || mSDEnable || mGDEnable) {
    ...
    // Do FD
    FDRet = mpFDHalObj->halFDDo(0, (MUINT8 *)mFDImgBuffer, pDstY,  mSDEnable, RotateInfo, (MUINT8 *)mDupImage.PAddrY, NULL, NULL, AEStable);
    if(FDRet) {
