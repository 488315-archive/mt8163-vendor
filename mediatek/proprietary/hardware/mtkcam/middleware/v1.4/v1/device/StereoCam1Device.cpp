...
onInit()
{
    ...
    else if(i4DeviceNum != 3) // main, sub and main2
    {
        MY_LOGE("i4DeviceNum(%d) != 3, should not open stereo cam in such situation!", i4DeviceNum);
        return 0;
    }else{
        MY_LOGD("i4DeviceNum(%d)", i4DeviceNum);
    }

    uint32_t expectedSensorDev_Main     = -1;
    uint32_t expectedSensorDev_Main2    = SENSOR_DEV_MAIN_2;
    // #if 1 == STEREO_CAM_FACING
    //     expectedSensorDev_Main = SENSOR_DEV_MAIN;
    // #endif
    // #if 2 == STEREO_CAM_FACING
    //     expectedSensorDev_Main = SENSOR_DEV_SUB;
    // #endif
    MY_LOGD("temp hard code stereo main cam = SENSOR_DEV_MAIN");
    expectedSensorDev_Main = SENSOR_DEV_MAIN;
    ...
    for(int i = 0; i < i4DeviceNum; i++)
    {
        ...
        MY_LOGD("openId=%d, DevIdx=%d", i, curDevIdx);

        if(curDevIdx == expectedSensorDev_Main){
            mSensorId_Main = i;
        }

        if(curDevIdx == expectedSensorDev_Main2){
            mSensorId_Main2 = i;
        }
    }
...
#if 0 //TBD
    mpSync3AMgr = NS3Av3::ISync3AMgr::getInstance();
    if  ( ! mpSync3AMgr ) {
    ...
    if  ( ! mpSync3AMgr->init(0, getOpenId_Main(), getOpenId_Main2()) ) {
...
#endif
        mpHal3a_Main = NS3Av3::IHal3A::createInstance(
        ...
        mpHal3a_Main2 = NS3Av3::IHal3A::createInstance(
                NS3Av3::IHal3A::E_Camera_3,
                getOpenId_Main2(),
                LOG_TAG);
        if  ( ! mpHal3a_Main2 ) {
