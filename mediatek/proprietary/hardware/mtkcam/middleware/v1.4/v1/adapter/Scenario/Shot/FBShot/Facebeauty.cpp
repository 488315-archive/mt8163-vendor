...
//#define _WORKAROUND_MULTI_PLANE
    ...
    if (Srcbufinfo != NULL) {
        Srcbufinfo->unlockBuf(LOG_TAG);
        Srcbufinfo->loadFromFile("/sdcard/img_3600x2160_i422.yuv");
        Srcbufinfo->lockBuf(LOG_TAG, eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN | eBUFFER_USAGE_SW_WRITE_OFTEN);
        Srcbufinfo->saveToFile("/sdcard/dsrcimg.yuv");
    }
    MY_LOGD("createFullFrame -");
    return MTRUE;
#endif
...
SDKcreateFBJpegImg()
{
 ...
 IImageBuffer *tempJpegBuf = allocMem(eImgFmt_BLOB,
                                    ...
                                    1);
#endif
    uint8_t *pJpegBuf = (uint8_t *)tempJpegBuf->getBufVA(0);//(MINT8 *)mDstImgBuffer->getBufVA(0);//(MINT8 *)tempJpegBuf->getBufVA(0);
...
#endif
   //tempJpegBuf->saveToFile("/sdcard/cfbResult.jpg");
...
doCapture()
{
    ...
    //mpSource->saveToFile("/sdcard/set6buffer.yuv");
