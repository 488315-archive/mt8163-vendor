...
sp<ICamAdapter> createDefaultAdapter(String8 const& rAppMode, int32_t const i4OpenId, sp<IParamsManager> pParamsMgr);
#endif
#if '1'==MTKCAM_HAVE_MTKSTEREO
sp<ICamAdapter> createStereoAdapter(String8 const& rName, int32_t const i4OpenId, sp<IParamsManager> pParamsMgr);
#endif
...
)
{
    ...
    // stereo temp dev
    {
        ...
        if(stereoMwEnable)
        {
            ...
            {
                #if (('1'==MTKCAM_HAVE_MTKSTEREO) && ('1'==MTKCAM_HAVE_MTKDEFAULT))
                CAM_LOGD("stereo temp dev => hard code createMtkStereoCamAdapter");
                return  createStereoAdapter(s8AppMode, i4OpenId, pParamsMgr);
                #endif
            }
        }
    }
