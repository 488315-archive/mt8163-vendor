    ...
    switch (atoi(mpsz8RawSaveMode))
    {
        ...
        default:
            if  ( (u4RawImgSize > 0) && (puRawImgBuf != 0) && (ms8RawFilePath != ""))
            {
                ...
                {
                    mEngParam.mi4MFLLnum++;
                    sprintf(mpszSuffix, "%s__%dx%d_%d_%d-%02d", ms8RawType.string(), mEngParam.u4SensorWidth, mEngParam.u4SensorHeight, mEngParam.u4Bitdepth, mEngParam.u4RawPixelID, mEngParam.mi4MFLLnum); /* info from EngShot::onCmd_capture */
                }
                else
                {
                    sprintf(mpszSuffix, "%s__%dx%d_%d_%d", ms8RawType.string(), mEngParam.u4SensorWidth, mEngParam.u4SensorHeight, mEngParam.u4Bitdepth, mEngParam.u4RawPixelID); /* info from EngShot::onCmd_capture */
                }
				...
				Unpack_MT6589_RAW unpack(mEngParam.u4rawStride , mEngParam.u4SensorWidth, mEngParam.u4SensorHeight, mEngParam.u4Bitdepth, mEngParam.u4RawPixelID, (const_cast<uint8_t*>(puRawImgBuf)), pucRawBuf, rawSize);
