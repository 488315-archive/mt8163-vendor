...
previewEnabled() const
{
    ...
    {
        ret =  (mpStateManager->isState(IState::eState_Preview)||recordingEnabled());
    }
    MY_LOGD("ret (%d)", ret);
    return ret;
}
...
#warning "FIXME"
       ...
       mbNeedResumPreview == MTRUE)
    {
        ...
        ret = mpFlowControl->resumePreview();
        if ( ret != OK )
        {
    ...
    {
        CamManager::getInstance()->incPreviewDevice(getOpenId());
...
forceStopAndCleanPreview()
{
    ...
    CamManager::getInstance()->decPreviewDevice(getOpenId());
    //
#warning "FIXME"
#if 0
    mpFlowControl = NULL;
