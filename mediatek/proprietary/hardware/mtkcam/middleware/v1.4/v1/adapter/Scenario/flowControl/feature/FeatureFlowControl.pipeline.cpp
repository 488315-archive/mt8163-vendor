...
#ifdef FEATURE_MODIFY
    // Sensor
    mSensorParam.vhdrMode = mpParamsManager->getVHdr();
    MY_LOGD("VHDR sensor mode:%d size:%dx%d, fps:%d pixel:%d vhdrMode:%d",
        mSensorParam.mode,
...
constructRecordingPipeline()
{
    ...
    // Sensor
    mSensorParam.vhdrMode = mpParamsManager->getVHdr();
    MY_LOGD("sensor mode:%d size:%dx%d, fps:%d pixel:%d vhdrMode:%d",
        mSensorParam.mode,
        mSensorParam.size.w, mSensorParam.size.h,
        mSensorParam.fps,
        mSensorParam.pixelMode,
