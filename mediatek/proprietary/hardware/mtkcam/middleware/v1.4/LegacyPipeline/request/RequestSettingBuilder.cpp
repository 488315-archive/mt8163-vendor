...
getRequest(MUINT32 const& requestId, IMetadata &nextAppRequest, IMetadata &nextHalRequest)
{
...

    {
        ...
        mpParamsMgrV3->getCropRegion(reqCropRegion, reqSensorCropRegion);
        //
...
*/
                    ...
                    // 3A region
                    {
                        ...
                        // af region
                        {
                            ...
                            if ( OK == mpParamsMgrV3->getParamsMgr()->parseCamAreas( s.string(), areas, max ) ) {
                                ...
                                while ( it != areas.end() ) {
                                    int xmin = ((*it).left   + 1000)*reqCropRegion.s.w/2000 + reqCropRegion.p.x;
                                    int ymin = ((*it).top    + 1000)*reqCropRegion.s.h/2000 + reqCropRegion.p.y;
                                    int xmax = ((*it).right  + 1000)*reqCropRegion.s.w/2000 + reqCropRegion.p.x;
                                    int ymax = ((*it).bottom + 1000)*reqCropRegion.s.h/2000 + reqCropRegion.p.y;
                                    //
    ...
    if ( mLogLevel >= 1 && mpMetadataConverter != 0)
    {
        mpMetadataConverter->dump(nextAppRequest);
        mpMetadataConverter->dump(nextHalRequest);
    }
