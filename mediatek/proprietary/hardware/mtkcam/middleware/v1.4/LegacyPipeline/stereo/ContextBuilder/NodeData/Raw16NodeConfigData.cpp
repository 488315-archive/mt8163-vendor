    ...
    PipelineContext* pipeContext)
{
    ...
    if(metaManager != NULL)
    {
        if(lMetaIOMap.vIn.size()>0 || lMetaIOMap.vOut.size()>0)
            mvMetaIOSet.add(lMetaIOMap);
    }
    //
    ...
    {
        lImageIOMap.addIn(eSTREAMID_IMAGE_PIPE_RAW_OPAQUE);
        lImageIOMap.addOut(eSTREAMID_IMAGE_PIPE_RAW16);
        mvInStreamSet.add(eSTREAMID_IMAGE_PIPE_RAW_OPAQUE);
        mvStreamUsageSet.add(
                        eSTREAMID_IMAGE_PIPE_RAW16,
                        eBUFFER_USAGE_SW_READ_OFTEN |eBUFFER_USAGE_HW_CAMERA_READWRITE);
        mvOutStreamSet.add(eSTREAMID_IMAGE_PIPE_RAW16);
        if(lImageIOMap.vIn.size()>0 || lImageIOMap.vOut.size()>0)
            mvImageIOSet.add(lImageIOMap);
    }
}
//
void
