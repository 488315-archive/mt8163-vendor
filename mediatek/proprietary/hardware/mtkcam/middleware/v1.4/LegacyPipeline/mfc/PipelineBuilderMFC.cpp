...
{
    if (pipelineMode == LegacyPipelineMode_T::PipelineMode_Hdr)
    {
    ...
    else if (pipelineMode == LegacyPipelineMode_T::PipelineMode_Mfll)
    {
...
public:
    static MBOOL update(Param& param, MBOOL isMemoryIn, MBOOL isMainResuest = MTRUE)
    {
        if (isMemoryIn)
        {
            return updateJpeg(param, isMemoryIn, isMainResuest);
        }
        else
        {
            return updateRaw(param, isMainResuest) &&
                   updateYuv(param, isMainResuest) &&
                   updateJpeg(param, isMemoryIn, isMainResuest);
        }
    }

private:
    static MBOOL updateRaw(Param& param, MBOOL /*isMainResuest*/)
    {
        ...
        if (imgoRaw == NULL && rrzoRaw == NULL)
        {
            CAM_LOGE("should config imgo or rrzo at least");
            return MFALSE;
        }
    ...
    static MBOOL updateYuv(Param& param, MBOOL isMainResuest)
    {
        // do nothing if not from a main request
        if (!isMainResuest) return MTRUE;
        ...
        // do nothing if yuv00 (e.g. postview) is not required
        if (yuv00 == NULL)
        {
            CAM_LOGI("yuv00 (e.g. postview) is not set");
            return MTRUE;
        }

        ASSERT_NOT_NULL(rrzoRaw, "should config rrzo");

        param.mapP1->addOut(rrzoRaw->getStreamId());
        param.mapMfcResized->addIn(rrzoRaw->getStreamId());
        param.mapMfcResized->addOut(yuv00->getStreamId());

        return MTRUE;
    }

    static MBOOL updateJpeg(Param& param, bool isMemoryIn, MBOOL isMainResuest)
    {
        sp<Configuration> config = param.config;
        if (config == NULL)
        {
        ...
        // do nothing if jpeg is not required
        if (jpeg == NULL)
        {
            CAM_LOGW("jpeg stream is not set");
            return MTRUE;
        }
        ...
        sp<PipelineContext>& pipelineContext, MUINT32& flag)
{
    ...
    PipelineImageParam srcParam;
    MBOOL isMemoryIn = querySrcParam(srcParam);
    if(isMemoryIn)
    {
        CAM_LOGI("get image from memory");
        mConfiguration->imageInfo_imgoRaw = srcParam.pInfo;
    ...
    // image
    {
        if (isMemoryIn)
        {
            buildStream(pipelineContext, srcParam);
        }
        // set to pipeline context
        for (size_t i = 0; i < vParams.size(); i++)
        {
    ...
    // configure node
    {
        ...
        NodeT::ConfigParams cfgParam;
        {
            ...
            cfgParam.pInResizedRaw = imageInfo_rrzoRaw;
        ...
        addStreamToSet(vIn, imageInfo_rrzoRaw);
        addStreamToSet(vIn, controlMeta_App);
...
sp<ILegacyPipeline> PipelineBuilderMFC::create()
{
    ...
    // 4. build nodes
    // TODO: change node configuration when support ZSD mode
    ...
    // 7. create requests for capture settings
    ...
    // 8. create and equip legacy pipeline with the following items
    //    - pipeline context
    //    - request builder
    //    - result processor
    //    - configuration
    //    - a set of capture parameters
...
MERROR PipelineBuilderMFC::mainRequestHelper(sp<RequestBuilder>& requestBuilder)
{
    ...
    MBOOL isMemoryIn = querySrcParam(srcParam);
    {
        ...
        param.mapMfcResized = &imageIOMapMfcResized;
    ...
    if (configFlag & eNodeConfigHdr)
    {
        ...
        else
        {
            requestBuilder->setIOMap(
                    eNODEID_HdrNode,
                    IOMapSet().add(imageIOMapMfc)
                              .add(imageIOMapMfcResized),
                    IOMapSet().add(metaIOMapMfc));
        }
    }

    // Mfll: following HDR IOMap, but not included Resized RAW.
        ...
        const sp<ResultProcessor>& resultProcessor)
{
    ...
    // Image input image
    {
        ...
        if ( !querySrcParam(params) ) {
            ...
            CAM_LOGD("Src buffer provider StreamId:%d",params.pInfo->getStreamId() );
            mpTimestampProcessor->registerCB(params.pProvider);
        }
    }
