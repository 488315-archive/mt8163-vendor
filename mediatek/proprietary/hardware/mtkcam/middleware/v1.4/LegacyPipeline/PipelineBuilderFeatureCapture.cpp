...
)
{
    ...
    MBOOL bMemoryIn = querySrcParam(srcParam);
    if(bMemoryIn)
    {
        mpConfig->pImage_ImgoRaw = srcParam.pInfo;
    ...
    // Image
    {
        ...
        if (bMemoryIn)
        {
            buildStream(rpPipelineContext, srcParam);
        }
        //
        for ( size_t i = 0; i < vParams.size(); ++i ) {
    ...
    // Node
    {
        ...
        if(isNeedP2TimeSharingMode())
        {
            ...
            pNode = new MyNodeActorT( NodeT::createInstance(P2FeatureNode::PASS2_TIMESHARING) );
        }
        else
        {
