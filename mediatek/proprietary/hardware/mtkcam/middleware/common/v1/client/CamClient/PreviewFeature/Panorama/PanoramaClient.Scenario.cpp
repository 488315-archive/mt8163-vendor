...
CreateMotionSrc(MVOID * srcbufadr, int ImgWidth, int ImgHeight, MVOID * dstbufadr)
{
    ...
    //*****************************************************************************//
     ...
     MY_LOGV("DDP_Performance_RGB565 Start");
    ...
    Mcount++;
    #endif
    MY_LOGV("DDP_Performance_RGB565 End");
    return ret;
}
/******************************************************************************
 *
...
mHalCamFeatureProc(MVOID * bufadr, int32_t& mvX, int32_t& mvY, int32_t& dir, MBOOL& isShot)
{
    ...
    if (isShot)
    {
        ...
        if (mPanoramaFrameIdx < PanoramaNum)
        {
            ...
            {
                char sourceFiles[80];
                sprintf(sourceFiles, "%sshot%d_%dx%d_mvx%dmvy%ddir%d.raw", DUMP_FOLDER,
                    mPanoramaFrameIdx, mPanoramaFrameWidth, mPanoramaFrameHeight,
                    PanoramaResult.MV_X, PanoramaResult.MV_Y, mStitchDir);
