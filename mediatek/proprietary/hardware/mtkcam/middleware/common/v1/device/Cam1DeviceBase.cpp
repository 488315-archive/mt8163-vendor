...
onInit()
{
    ...
    pParamsMgrV3 = IParamsManagerV3::createInstance(mDevName, mi4OpenId, mpParamsMgr);
    pParamsMgrV3->setStaticData();
    //
    ret = true;
lbExit:
    MY_LOGD("- ret(%d)", ret);
...
sendCommand(int32_t cmd, int32_t arg1, int32_t arg2)
{
    ...
    case CAMERA_CMD_ENABLE_RAW16_CALLBACK:
         ...
         if(mIsRaw16CBEnabled)
         {
             MY_LOGD("flowcontrol CAMERA_CMD_ENABLE_RAW16_CALLBACK enable");
             mpCamAdapter->sendCommand(CAMERA_CMD_ENABLE_RAW16_CALLBACK, 1, 0);
         }
         //
         break;
