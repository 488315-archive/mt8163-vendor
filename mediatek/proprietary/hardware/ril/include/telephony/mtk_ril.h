...
typedef enum {
    ...
    RIL_E_BT_SAP_ALREADY_POWERED_OFF = 2006,
    RIL_E_BT_SAP_ALREADY_POWERED_ON = 2007,
    /// @}
...
/*
 * Returned by RIL_UNSOL_MBIM_RESPONSE, on error status != 0.
 */
typedef struct {
    int             status;     /* A RIL_DataCallFailCause, 0 which is PDP_FAIL_NONE if no error */
    int             suggestedRetryTime; /* If status != 0, this fields indicates the suggested retry
    ...
    int             cid;        /* Context ID, uniquely identifies this call */
    int             active;     /* 0=inactive, 1=active/physical link down, 2=active/physical link up */
    int             mtuSize;    /* MTU received from network
                                   Value <= 0 means network has either not sent a value or
                                   sent an invalid value */
    char            type[8];       /* One of the PDP_type values in TS 27.007 section 10.1.1.
                                   ...
                                   PDP_FAIL_ONLY_SINGLE_BEARER_ALLOWED this is the type supported
                                   such as "IP" or "IPV6" */
    char            ifname[16];     /* The network interface name */
    char            addresses[3*(MAX_IPV6_ADDRESS_LENGTH + 1)];  /* A space-delimited list of addresses with optional "/" prefix length,
                                   e.g., "192.0.1.3" or "192.0.1.11/16 2001:db8::1/64".
                                   May not be empty, typically 1 IPv4 or 1 IPv6 or
                                   one of each. If the prefix length is absent the addresses
                                   are assumed to be point to point with IPv4 having a prefix
                                   length of 32 and IPv6 128. */
    char            dnses[5*(MAX_IPV6_ADDRESS_LENGTH + 1)];      /* A space-delimited list of DNS server addresses,
                                   e.g., "192.0.1.3" or "192.0.1.11 2001:db8::1".
                                   May be empty. */
    char            gateways[3*(MAX_IPV6_ADDRESS_LENGTH + 1)];   /* A space-delimited list of default gateway addresses,
                                   e.g., "192.0.1.3" or "192.0.1.11 2001:db8::1".
                                   May be empty in which case the addresses represent point
                                   to point connections. */
} RIL_Data_Call_Response_MBIM;
...
/** Used by RIL_REQUEST_REDIAL/RIL_REQUEST_EMERGENCY_REDIAL/RIL_REQUEST_C2K_EMERGENCY_REDIAL */
typedef struct {
    char * address;
    int clir;
            /* (same as 'n' paremeter in TS 27.007 7.7 "+CLIR"
...
typedef enum {
    ...
    /* AT command error or data call registration fail*/
    MBIM_CMD_ERROR = -1,
    MBIM_REG_FAIL = -2,
    MBIM_DEREG_FAIL = -3
} MBIM_DataCall_Status;
...
#define RIL_REQUEST_EMERGENCY_REDIAL (RIL_LOCAL_REQUEST_VENDOR_BASE + 13)
#define RIL_REQUEST_EMERGENCY_SESSION_BEGIN (RIL_LOCAL_REQUEST_VENDOR_BASE + 14)
#define RIL_REQUEST_EMERGENCY_SESSION_END (RIL_LOCAL_REQUEST_VENDOR_BASE + 15)
/******************************************************************/
...
typedef enum {
    ...
    DATA_CHANNEL,
    MISC_CHANNEL,
#ifdef MTK_SRLTE_SUPPORT
    SIM_CHANNEL,
    RIL_SUPPORT_CHANNELS
#else
    RIL_SUPPORT_CHANNELS,
    SIM_CHANNEL = AT_CHANNEL
#endif
} RILChannelId;
