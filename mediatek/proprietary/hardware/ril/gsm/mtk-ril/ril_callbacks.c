...
static void onRequest(int request, void *data, size_t datalen, RIL_Token t, RIL_SOCKET_ID socket_id)
{
    ...
    if (radioState == RADIO_STATE_UNAVAILABLE &&
            ...
            request != RIL_REQUEST_OEM_HOOK_RAW_REDIRECT && //This is for ATCI
...
void queryBearer(RIL_SOCKET_ID rid)
{
    ...
    char *line;
    int ret;
    char worldMode_prop[PROPERTY_VALUE_MAX] = {0};
    int worldMode = 0;
    ...
    if (s_simSwitchVersion == 2) {
        property_get("ril.nw.worldmode.activemode", worldMode_prop, "1");
        ...
        property_get("ril.active.md", worldMode_prop, "0");
        worldMode = atoi(worldMode_prop);
...
static void initializeCallback(void *param)
{
    ...
    // For distinguishing between R10 and R11.
    err = at_send_command("AT+CEPPI=?", &p_response, getDefaultChannelCtx(rid));
    if (isATCmdRspErr(err, p_response)) {
        RLOGD("3GPP R10 and before!");
        spec_3gpp_version = MD_3GPP_R10;
    } else {
        RLOGD("3GPP R11 and after!");
        spec_3gpp_version = MD_3GPP_R11;
    }
...
/* M: End - abnormal event logging for logger */
    ...
    updatePhoneIdToMDIdProperty(rid);
...
void sendRadioCapabilityDoneIfNeeded () {
    ...
    //if (sessionId != 0) {
        ...
        for (i = 0; i < SIM_COUNT; i++) {
            ...
            RIL_onUnsolicitedResponseSocket(RIL_UNSOL_RADIO_CAPABILITY, rc, sizeof(RIL_RadioCapability), i);
        }
    //}
}
...
static void *mainLoop(void *param)
{
    ...
    while (s_main_loop) {
        ...
        queryBearer(RIL_SOCKET_1);
        if (SIM_COUNT >= 2) {
            queryBearer(RIL_SOCKET_2);
            if (SIM_COUNT >= 3) {
                queryBearer(RIL_SOCKET_3);
                if (SIM_COUNT >= 4) {
                    queryBearer(RIL_SOCKET_4);
                }
            }
        }
...
void updatePhoneIdToMDIdProperty(RIL_SOCKET_ID rid) {
    char property_value[PROPERTY_VALUE_MAX] = { 0 };

    property_get("ril.rilproxy", property_value, "0");
    RLOGI("ril.rilproxy = %s", property_value);

    if (strcmp(property_value, "1") == 0) {
        return;
    }

    // No need to set property for DSDS, since it always map to MD1
    if (isDualTalkMode()) {
        RLOGI("DSDA: L+G");
        if (RIL_SOCKET_1 == rid) {
            if (GEMINI_SIM_1 == getMappingSIMByCurrentMode(rid)) {
                property_set("ril.phone1.mapped.md", "MD1");
                property_set("ril.phone2.mapped.md", "MD2");
            } else {
                property_set("ril.phone1.mapped.md", "MD2");
                property_set("ril.phone2.mapped.md", "MD1");
            }
        }
    }
}
