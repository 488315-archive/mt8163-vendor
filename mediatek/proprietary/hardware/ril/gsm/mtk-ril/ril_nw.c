...
int getSingnalStrength(char *line, int *response)
{
    ...
    int *rssi_in_qdbm = &response[12];    //mGsmRssiQdbm
    int *rscp_in_qdbm = &response[13];    //mGsmRscpQdbm
    int *ecn0_in_qdbm = &response[14];    //mGsmEcn0Qdbm
    ...
    if(at_tok_hasmore(&line)){
        ...
        *rsrp_in_qdbm = ((*rsrp_in_qdbm)/4)*(-1);
        /// M: [ALPS02400166] make sure mLteRsrp in AOSP's expected scale
        if(*rsrp_in_qdbm > 140) {
            *rsrp_in_qdbm = 140;
    ...
    if(rat >= 0)
    {
        ...
        if(err >= 0)
        {
            ...
            while (skipDetach == 0 && detachCount < 10) {
                ...
                if (err >= 0 && p_response->success != 0) {
                    LOGD("requestSetPreferredNetworkType: success");
                    skipDetach = 1;
                    ril_errno = RIL_E_SUCCESS;
                } else {
                    detachCount++;
                    LOGE("requestSetPreferredNetworkType(): fail, count=%d", detachCount);
                    sleep(1);
                }
                at_response_free(p_response);
            }
...
void onNitzTimeReceived(char* urc, const RIL_SOCKET_ID rid)
{
    ...
    *line = '\0';
    ...
    // concatenate the remaining with <time> if any
    if (at_tok_hasmore(&urc) && strlen(urc) > 0) {
...
void onModulationInfoReceived(char *urc, const RIL_SOCKET_ID rid)
{
    ...
    RIL_onUnsolicitedResponseSocket(RIL_UNSOL_MODULATION_INFO, &response, count * sizeof(int), rid);
    return;
