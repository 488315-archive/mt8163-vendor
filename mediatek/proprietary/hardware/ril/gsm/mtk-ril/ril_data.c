...
extern bool isPdnHoStartFromProperty(PdnInfo *pInfo);
...
/// M: IMS/EPDG @}
    ...
    if (requestParamNumber != 18 && requestParamNumber != 8 && requestParamNumber != 17) {
    ...
    for(i = 0; i < pdnInfoSize; i++) {
        if (pdn_info[i].interfaceId == interfaceId && !pdn_info[i].isDedicateBearer) {
            ...
            case CME_L4C_CONTEXT_CONFLICT_DEACT_ALREADY_DEACTIVATED:
                ...
                break;
            case CME_LAST_PDN_NOT_ALLOW_R11:
                if(spec_3gpp_version == MD_3GPP_R10) goto error;
...
static int isFallbackPdpNeeded(RIL_SOCKET_ID rid, const char* apn, int protocol)
{
    ...
    if (ret) {
        if ((IPV4V6 != protocol) || ((NULL != apn) && (NULL != strstr(apn, "ims")))) {
            LOGD("requestSetupData protocal is IPv4 or IPv6 and/or apn is ims");
            ret = 0;
        }
    }
