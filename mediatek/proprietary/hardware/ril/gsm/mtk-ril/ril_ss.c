...
/// M: Simulate USSI by CS @{
static bool isSimulateUSSI() {
    char temp[255];
    int simulate;
    property_get("persist.ims.ussi.simulate", temp, "0");
    simulate = atoi(temp);
    if (simulate == 1) {
        return true;
    }
...
static void requestCancelUssi(void * data, size_t datalen, RIL_Token t)
{
    ...
    if (isSimulateUSSI()) {
        ...
        err = at_send_command("AT+EIUSD=2,2", &p_response, SS_CHANNEL_CTX);
    }
...
static void onUssd(char *s, RIL_SOCKET_ID rid)
{
    ...
    if (isSimulateUSSI()) {
        LOGD("Simulate USSI by CS");
        char *ussiData[3] = {"1", finalData[0], hasStr ? finalData[1] : ""};
        LOGD("ussiData[0]= %s", ussiData[0]);
        LOGD("ussiData[1]= %s", ussiData[1]);
        LOGD("ussiData[2]= %s", ussiData[2]);
        IMS_RIL_onUnsolicitedResponse (
                RIL_UNSOL_ON_USSI,
                ussiData, 3 * sizeof(char *),
                rid);
    } else {
        RIL_onUnsolicitedResponseSocket (
                RIL_UNSOL_ON_USSD,
