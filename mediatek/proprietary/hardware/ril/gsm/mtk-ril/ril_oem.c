...
extern pthread_mutex_t s_switch_mutex;
...
// MBIM start
extern int onRequestPdnActivation(const char* param, RIL_Token t);
extern int onRequestPdnDeactivation(RIL_Token t);
// MBIM end
...
#ifdef MTK_RIL
extern void clearAllProxyList();
#endif

extern void setIaCache(const char* iccid, const char* protocol, const int authtype,
...
void requestOemHookRaw(void * data, size_t datalen, RIL_Token t)
{
    ...
    RLOGD("requestOemHookRaw : receive %s", buffer);
    if (strStartsWith(buffer, "AT+SCGACT")) {
        if (strStartsWith(buffer, "AT+SCGACT=1")) {
            char* param = buffer + strlen("AT+SCGACT=1,");
            err = onRequestPdnActivation(param, t);
        } else if (strStartsWith(buffer, "AT+SCGACT=0")) {
            err = onRequestPdnDeactivation(t);
        } else {
            RLOGD("requestOemHookRaw : The parameters of SCGACT are not expected");
        }

        RLOGD("requestOemHookRaw : err : %d", err);

        if (err < 0) {
            if (err == MBIM_CMD_ERROR) {
                RLOGE("requestOemHookRaw : CMD ERROR");
                goto error;
            } else {
                if (err == MBIM_REG_FAIL) {
                    RLOGE("requestOemHookRaw : Failed to activate PDN");
                    line = (char *) alloca(13);
                    memset(line, 0, 13);
                    strcpy(line, "\r\nREG_FAIL\r\n");
                    RLOGD("line = <%s>", line);
                    ...
                    RLOGE("requestOemHookRaw : Failed to deactivate PDN");
                    line = (char *) alloca(15);
                    memset(line, 0, 15);
                    strcpy(line, "\r\nDEREG_FAIL\r\n");
                    RLOGD("line = <%s>", line);
        ...
        RLOGD("requestOemHookRaw : NOT SCGACT");
    }
...
void requestOemHookStrings(void * data, size_t datalen, RIL_Token t)
{
    ...
    RLOGD("OEM_HOOK_STRINGS : receive %s", cur[0]);
    if (strStartsWith(cur[0], "AT+SCGACT")) {
        if (strStartsWith(cur[0], "AT+SCGACT=1")) {
            char* param = cur[0] + strlen("AT+SCGACT=1,");
            err = onRequestPdnActivation(param, t);
        } else if (strStartsWith(cur[0], "AT+SCGACT=0")) {
            err = onRequestPdnDeactivation(t);
        } else {
            RLOGD("OEM_HOOK_STRINGS : The parameters of SCGACT are not expected");
        }

        RLOGD("OEM_HOOK_STRINGS : err : %d", err);

        if (err < 0) {
            RLOGE("OEM_HOOK_STRINGS : Failed to activate or deactivate PDN");
            RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, NULL, 0);
            return;
        } else {
            RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
        ...
        RLOGD("OEM_HOOK_STRINGS : NOT SCGACT");
    }
    ...
    *************************************/
    ...
    if (on_off)
    {
        ...
        if (isEnableModulationReport()) {
            err = at_send_command("AT+EMODCFG=1", &p_response, OEM_CHANNEL_CTX);
            if (err != 0 || p_response->success == 0)
    ...
    else
    {
        ...
        /* Disable EMODCFG URC */
        if (isEnableModulationReport()) {
            err = at_send_command("AT+EMODCFG=0", &p_response, OEM_CHANNEL_CTX);
            if (err != 0 || p_response->success == 0)
...
static void clearIaCache(RILChannelCtx *pChannel)
{
    setIaCache("", "", 0, "", 0, "");
    ...
    property_set(PROPERTY_IA_APN_SET_ICCID, "");
}
...
int doSimSwitchWithoutMdReset() {
    ...
    RLOGI("&&&& try to get s_switch_mutex");
    ...
    for (i = 0; i < getSupportChannels(); i++) {
        ...
        if (p_channel_old->fd >= 0) {
            close(p_channel_old->fd);
        }
        p_channel_old->fd = -1;
        p_channel_old->readerClosed = 1;
    ...
    if (p_channel_cmd->fd >= 0) {
        close(p_channel_cmd->fd);
    }
    p_channel_cmd->fd = -1;
    p_channel_cmd->readerClosed = 1;
    ...
    while (retry) {
        usleep(300 * 1000);
        retry = 0;
        RLOGI("check if all readers are closed");
        for (i = 0; i < getSupportChannels(); i++) {
            p_channel_tmp = getChannelCtxbyId(i);
            kill_rc = pthread_kill(p_channel_tmp->tid_reader,0);
            if (kill_rc == ESRCH) {
                // do nothing
            } else {
                pthread_kill(p_channel_tmp->tid_reader, SIGUSR1);
                RLOGI("reader %s is alive", p_channel_tmp->myName);
                retry = 1;
            }
        }
    }
    ...
    for (i = 0; i < getSupportChannels(); i++) {
        ...
        if ((i % RIL_CHANNEL_OFFSET) != RIL_CMD_3) {
            p_channel_old = getChannelCtxbyId(i);
            //RLOGI("******** requestSetRadioCapability: try to unlock channel %d, name %s, %x",
    ...
    RLOGI("&&&& try to unlock s_switch_mutex");
...
extern ApplyRadioCapabilityResult applyRadioCapability(RIL_RadioCapability* rc, RIL_Token t)
{
    ...
    if ((!isCdmaLteDcSupport()) && (recevSimSwitchReqCount != SIM_COUNT)) {
        RLOGI("only handle last request for sim switch");
        return ApplyRC_NONEED;
    }
    if (s_simSwitchVersion == 2 && isVsimEnabled() != 1) {
        s_isSimSwitching = 1;
...
extern void requestSetRadioCapability(void * data, size_t datalen, RIL_Token t)
{
    ...
    switch (rc.phase) {
        ...
        case RC_PHASE_APPLY:
            ...
            if (s_simSwitchVersion == 1 || isVsimEnabled() == 1) {
                ...
                if (applyRcResult == ApplyRC_SUCCESS) {
                    ...
                    int ret = doSimSwitchWithoutMdReset();
                    s_isSimSwitching = 0;
                    if (ret == 0) {
                        // fail case
                        RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, responseRc, sizeof(RIL_RadioCapability));
...
extern void bootupGetIccid(RIL_SOCKET_ID rid) {
    ...
    if (!result) {
        ...
        if ((isSvlteSupport() || isSrlteSupport()) && (isRuimCard != NULL)) {
            ...
            upadteSystemPropertyByCurrentModeGemini(rid, PROPERTY_ICCID_SIM[0],
                    PROPERTY_ICCID_SIM[1], PROPERTY_ICCID_SIM[2], PROPERTY_ICCID_SIM[3], "N/A");
        }
    }
}
...
void onWorldModeStateChanged(int state){
    if (state == 1){
        ...
        // reset aIs_stk_service_running[] flag when world mode changed start
        for (i = 0; i < SIM_COUNT; i++) {
            setStkServiceRunningFlag(RIL_SOCKET_1 + i, false);
        }
    } else if (state == 0){
...
extern int rilOemMain(int request, void *data, size_t datalen, RIL_Token t)
{
    switch (request)
    {
        ...
        case RIL_REQUEST_OEM_HOOK_RAW_REDIRECT:
