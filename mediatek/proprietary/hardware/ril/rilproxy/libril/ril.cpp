...
// check later
    ...
    pRI->token = token;
    RLOGE("[RilProxy] token = %d, request = %d", token, request);
    /// M: ril proxy
...
static void dispatchRilCdmaSmsWriteArgs(Parcel &p, RequestInfo *pRI) {
    ...
    RLOGE("dispatchRilCdmaSmsWriteArgs ENTER. uBearerDataLen = %d", rcsw.message.uBearerDataLen);
    for (digitCount = 0 ; digitCount < digitLimit; digitCount ++) {
        status = p.read(&uct, sizeof(uct));
...
static void dispatchWritePhbEntryExt(Parcel &p, RequestInfo *pRI) {
    ...
    startRequest;
    appendPrintBuf("%s, index = %d, num = %s, type = %d, text = %s, hidden = %d, group = %s, \
            adnumber = %s, adtype = %d, secondtext = %s, email = %s", printBuf, args.index,
            (char*)args.number, args.type, (char*)args.text, args.hidden, (char*)args.group,
            (char*)args.adnumber, args.adtype, (char*)args.secondtext, (char*)args.email);
    closeRequest;
    printRequest(pRI->token, pRI->pCI->requestNumber);
...
static void dispatchSetupDedicateDataCall(Parcel &p, RequestInfo *pRI) {
    ...
    LOGD("dispatchSetupDedicateDataCall [version=%d, ddcId=%d, interfaceId=%d, signalingFlag=%d]",
            version, args.ddcId, args.interfaceId, args.signalingFlag);
...
static int responseReadPbFromRuim(Parcel &p, void *response, size_t responselen) {
    ...
    startResponse;
    appendPrintBuf("%sindex=%d, number=%s, type=%d, name=%s, coding=%d",
            printBuf, p_cur->index, (char*)p_cur->number, p_cur->type,
...
static void dispatchMTKWritePbToRuim(Parcel &p, RequestInfo *pRI) {
    ...
    s_callbacks.onRequest(pRI->pCI->requestNumber, &p_Record, sizeof(p_Record), pRI, pRI->socket_id);
...
static int responseMTKReadPbFromRuim(Parcel &p, void *response, size_t responselen) {
    ...
    for (int i = 0 ; i < num ; i++) {
        ...
        writeStringToParcel(p, p_cur.alphaid);
       appendPrintBuf("%s type=%d, index=%d, number=%s, ton=%d, alphaid=%s",
                printBuf, p_cur.type, p_cur.index, (char*)p_cur.number, p_cur.ton,
        (char*)p_cur.alphaid);
    }
    closeResponse;
...
static int responseReadPhbEntryExt(Parcel &p, void *response, size_t responselen) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        appendPrintBuf("%s [%d: index = %d, number = %s, type = %d, text = %s, hidden = %d, \
                group = %s, adnumber = %s, adtype = %d, sectext = %s, email = %s]",
...
static void dispatchPlayDTMFTone(Parcel &p, RequestInfo *pRI) {
    ...
    s_callbacks.onRequest(pRI->pCI->requestNumber, &dtmfTone, sizeof(dtmfTone), pRI, pRI->socket_id);
...
static void dispatchPlayToneSeq(Parcel &p, RequestInfo *pRI) {
    ...
    s_callbacks.onRequest(pRI->pCI->requestNumber, &toneSeq, sizeof(toneSeq), pRI, pRI->socket_id);
...
static void dispatchUiccIo(Parcel &p, RequestInfo *pRI) {
    ...
    CALL_ONREQUEST(pRI->pCI->requestNumber, &uiccIo, sizeof(RIL_UICC_IO_v6), pRI, pRI->socket_id);
...
static void dispatchUiccAuthentication(Parcel &p, RequestInfo *pRI) {
    ...
    CALL_ONREQUEST(pRI->pCI->requestNumber, &uiccAuth, sizeof(uiccAuth), pRI, pRI->socket_id);
...
RIL_register (const RIL_RadioFunctions *callbacks) {
    ...
    RLOGI("RILPROXY SIM_COUNT: %d", SIM_COUNT);
...
static RequestInfo *
getRequestInfoByToken(RIL_SOCKET_ID socketId, int token) {
...
#if (SIM_COUNT >= 3)
        if (socketId == RIL_SOCKET_3) {
            pendingRequestsMutexHook = &s_pendingRequestsMutex_socket3;
            pendingRequestsHook = &s_pendingRequests_socket3;
        }
#endif
#if (SIM_COUNT >= 4)
    if (socketId == RIL_SOCKET_4) {
        pendingRequestsMutexHook = &s_pendingRequestsMutex_socket4;
        pendingRequestsHook = &s_pendingRequests_socket4;
    }
#endif
...
extern "C" bool
RIL_onRequestCompleteRfx(int slotId, int token, RIL_Errno e, Parcel* parcel) {
    ...
    if (slotId == 0) {
        socketId = RIL_SOCKET_1;
#if (SIM_COUNT >= 2)
    } else if (slotId == 1) {
        socketId = RIL_SOCKET_2;
#if (SIM_COUNT >= 3)
    } else if (slotId == 2) {
        socketId = RIL_SOCKET_3;
#endif
#if (SIM_COUNT >= 4)
    } else if (slotId == 3) {
        socketId = RIL_SOCKET_4;
#endif
#endif
    }

    RequestInfo *pRI = getRequestInfoByToken(socketId, token);
    if (pRI == NULL) {
        RLOGE("RIL_onRequestCompleteRfx: cannot find RI by token (socketId=%d, token=%d)", socketId,
                token);
        return false;
    }
    parcel->setDataPosition(sizeof(uint32_t)*3);
    datalen = parcel->dataAvail();
    data = parcel->readInplace(datalen);

    RIL_onRequestComplete((RIL_Token)pRI, e, (void *)data, datalen);
...
extern "C" bool
RIL_onUnsolicitedResponseRfx(int slotId, int unsolResponse, Parcel* parcel) {
...
#endif
{
    ...
    if ((unsolResponseIndex < 0)
        ...
        //M: ril proxy
        if ((unsolResponse < RIL_UNSOL_VENDOR_BASE) || (unsolResponse >= RIL_UNSOL_VENDOR_BASE +
                (int32_t)NUM_ELEMS(s_mtk_unsolResponses))) {
    ...
    if (unsolResponse > RIL_UNSOL_VENDOR_BASE) {
        RLOGD("RIL_onUnsolicitedResponse: GSM");
...
*/
    switch(request) {
        ...
        case RIL_REQUEST_REDIAL: return "RIL_REQUEST_REDIAL";
        ...
        case RIL_REQUEST_C2K_EMERGENCY_REDIAL: return "RIL_REQUEST_C2K_EMERGENCY_REDIAL";
...
void cacheUrc(int unsolResponse, const void *data, size_t datalen, RIL_SOCKET_ID socket_id){
    ...
    if (unsolResponse != RIL_UNSOL_STK_PROACTIVE_COMMAND
        ...
        && unsolResponse != RIL_UNSOL_EUSIM_READY
