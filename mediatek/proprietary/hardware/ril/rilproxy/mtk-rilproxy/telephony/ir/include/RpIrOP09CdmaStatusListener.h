...
class RpIrOP09CdmaStatusListener : public RfxController, public IRpIrNwControllerListener {
    RFX_DECLARE_CLASS(RpIrOP09CdmaStatusListener);

public:
    RpIrOP09CdmaStatusListener();
    RpIrOP09CdmaStatusListener(RpIrStrategyOP095M *rpIrStrategyOP095M, RpIrController *rpIrController);
    virtual ~RpIrOP09CdmaStatusListener() {}

    virtual void onRadioStateChanged(bool isRadioOn);
    virtual void onPlmnChanged(String8 &plmn);
    virtual bool needWaitImsi();
    virtual void onServiceStateChanged(ServiceType serviceType);
    virtual void onNwsModeSwitchDone();

private:
    String8 convertInvalidMccBySidNid(const String8 &plmn);
    RpIrStrategyOP095M *m_RpIrStrategyOP095M;
