...
void RpIrCdmaHandler::onCT3GDualModeCardChanged(RfxStatusKeyEnum key,
    ...
    if (mIsCT3GCardType) {
        enableSuspend(true);
        mPtrIrController->setIrEnableState(true);
    }
}
...
void RpIrCdmaHandler::onServiceStateChanged(RfxStatusKeyEnum key,
    ...
    if (!CAPABILITY_HAS_CDMA(radio_cap)) {
        logD(RFX_LOG_TAG, "onServiceStateChanged but CMA radio off, ignore");
        return;
    }
...
void RpIrCdmaHandler::registerForServiceStateChanged() {
    logD(RFX_LOG_TAG, "registerForServiceStateChanged");
...
void RpIrCdmaHandler::enableSuspend(bool enabled) {
    ...
    if (!mPtrIrController->isCdmaDualModeSimCard()) {
        logD(RFX_LOG_TAG, "enableSuspend: not ct dual mode card, return");
        return;
    }
...
void RpIrCdmaHandler::enableServiceStateNotify(bool enable)
{
    logD(RFX_LOG_TAG, "enableServiceStateNotify: %s", Bool2Str(enable));
    // TODO: need NW controller provide interface.
}
