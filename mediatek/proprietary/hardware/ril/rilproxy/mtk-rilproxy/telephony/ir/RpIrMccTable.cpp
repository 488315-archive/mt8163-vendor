...
bool RpIrMccTable::isSameCountryCode(int mcc1, int mcc2) {
    if (mcc1 <= 0 || mcc2 <= 0) {
        RFX_LOG_D("RpIrMccTable", "Invalid mcc");
        return false;
    }
    RpIrMccEntry *item1 = NULL;
    RpIrMccEntry *item2 = NULL;
    for (int i = 0; sMccTable[i].mcc != 0; i++) {
        if (mcc1 == sMccTable[i].mcc) {
            item1 = &sMccTable[i];
        }
        if (mcc2 == sMccTable[i].mcc) {
            item2 = &sMccTable[i];
        }
    ...
    if (item1 == NULL || item2 == NULL) {
        ...
        return strcmp(item1->country_code, item2->country_code) == 0 ? true : false;
    }
}
