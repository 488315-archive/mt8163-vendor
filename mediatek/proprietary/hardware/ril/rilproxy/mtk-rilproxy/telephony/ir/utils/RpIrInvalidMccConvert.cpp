...
String8 RpIrInvalidMccConvert::checkMccBySidLtmOff(String8 mccMnc) {
    ...
    property_get("cdma.operator.sid", strSid, strdef);

    memset(strLtmOff, 0, sizeof(char)*PROPERTY_VALUE_MAX);
    property_get("cdma.operator.ltmoffset", strLtmOff, strdef);
...
String8 RpIrInvalidMccConvert::getMccFromConflictTableBySidLtmOff(const char *sSid, const char *sLtmOff) {
    ...
    RFX_LOG_D(TAG, " [getMccFromConflictTableBySidLtmOff] mccSidMapSize = %d", RpIrMccConvertMappingTables::MCC_SID_LTM_OFF_MAP_SIZE);
    ...
    for (int i = 0; i < RpIrMccConvertMappingTables::MCC_SID_LTM_OFF_MAP_SIZE; i++) {
        ...
        RFX_LOG_D(TAG, " [getMccFromConflictTableBySidLtmOff] pRpIrMccSidLtmOff[i].mSid = %d, sid = %d, i=%d, ltm_off = %d, max = %d, min = %d",
...
String8 RpIrInvalidMccConvert::getMccFromMINSTableBySid(const char *sSid) {
    ...
    for (int i = 0; i < RpIrMccConvertMappingTables::MCC_IDD_NDD_SID_MAP_SIZE; i++) {
        ...
        RFX_LOG_D(TAG, " [getMccFromMINSTableBySid] sid = %d, i=%d, pRpIrMccIddNddSid[i].SidMin = %d, pRpIrMccIddNddSid[i].SidMax = %d",
