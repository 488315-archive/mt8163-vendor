...
void requestDataConnection() {
    ...
    p->writeInt32(RIL_UNSOL_VIA_GPS_EVENT);   // urc ID, convert from 1052 to 5004 in rild
...
void checkAgpsTcpConnInd(Parcel *p, int eConnected) {
    ...
    ASSERT_TRUE(request == RIL_REQUEST_AGPS_TCP_CONNIND);
    ASSERT_TRUE(count == 1);
...
TEST_F(AgpsTest, PeerSocketCloseBeforeWhenWrite) {
    ...
    setServiceState(RFX_SLOT_ID_1,
        RpAgpsNSlotController::OUT_OF_SERVICE,
        RADIO_TECH_LTE);
