...
/************************************************************************
 * Class RpOP01CardModeController
 ************************************************************************/
std::mutex mtxop01;
RpOP01CardModeController* RpOP01CardModeController::s_RpOP01CardModeController;

RpOP01CardModeController::RpOP01CardModeController() {
}

RpOP01CardModeController::~RpOP01CardModeController() {
}

RpOP01CardModeController* RpOP01CardModeController::getInstance() {
    if (s_RpOP01CardModeController == NULL) {
        mtxop01.lock();
        if (s_RpOP01CardModeController == NULL) {
            s_RpOP01CardModeController = new RpOP01CardModeController();
        }
        mtxop01.unlock();
    }
    return s_RpOP01CardModeController;
}

int RpOP01CardModeController::getPreferRadioTech(int cardType, int slot,
    int* oldCardModes) {
...
bool RpOP01CardModeController::isCt3GDualMode(int slotId) {
...
void RpOP01CardModeController::calculateCardMode(int* cardTypes, int* oldCardModes,
        int* newCardModes, int* needSwitchCardTypeSlots, int slotNum) {

    RFX_LOG_D(RP_CDMALTE_MODE_TAG,"[SMC][calculateCardMode] >>> OPERATOR_OP01 case");
    ...
    if (cdmaLteCount == 1 && insertCount > 1) {
        ...
        /**
         * OP01 rule: Should set the card's ( which has 3/4 G capability ) mode to csfb.
         * other CT cards' mode should be set to cdmalte.
         * And find ct cards which will been change to Gsm Card.
         */
        RFX_LOG_D(RP_CDMALTE_MODE_TAG, "[SMC]calculateCardMode cdmaLteCount>1");
        if (slotPreferRadioTec[capabilitySlot] == RADIO_TECH_MODE_CDMALTE &&
                cardTypes[capabilitySlot] != CARD_TYPE_NONE) {
            newCardModes[capabilitySlot] = RADIO_TECH_MODE_CSFB;

            mode = slotPreferRadioTec[capabilitySlot];
            if (mode == RADIO_TECH_MODE_CDMALTE
                    && isCt3GDualMode(capabilitySlot)) {
                RFX_LOG_D(RP_CDMALTE_MODE_TAG,
                        "[SMC]calculateCardMode needSwitchCardTypeSlots: %d", capabilitySlot);
                needSwitchCardTypeSlots[capabilitySlot] = capabilitySlot;
            }

            int firstCCard = -1;
            for (int index =0; index < slotNum; index++) {
                if (index != capabilitySlot && firstCCard == -1) {
                    newCardModes[index] = RADIO_TECH_MODE_CDMALTE;
                    firstCCard = index;
                } else if (index != capabilitySlot) {
