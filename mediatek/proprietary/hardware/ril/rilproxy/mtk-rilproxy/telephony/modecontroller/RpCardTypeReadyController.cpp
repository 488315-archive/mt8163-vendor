...
RpCardTypeReadyController::RpCardTypeReadyController() {
    ...
    m_card_type_state = new int[RFX_SLOT_COUNT];
...
void RpCardTypeReadyController::onSimTaskReady(int slotId, RfxStatusKeyEnum key,
    ...
    /// M: If this action is hot plug-in or plug-out, then should set other slots` card state
    /// to CARD_STATE_NO_CHANGED
    if (m_card_type_state[slotId] == CARD_STATE_HOT_PLUGIN ||
            m_card_type_state[slotId] == CARD_STATE_HOT_PLUGOUT) {
        for (int index = 0; index < RFX_SLOT_COUNT; index++) {
            if (index != slotId && m_card_type_state[slotId] != CARD_STATE_NOT_HOT_PLUG) {
