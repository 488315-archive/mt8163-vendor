...
RfxMessage::RfxMessage() :
            ...
            timeStamp(0), clientId(-1), dataObj(NULL), pTimeStamp(0) {
#ifdef RFX_OBJ_DEBUG
...
String8 RfxMessage::toString() const {
    return String8::format(
            ...
            error = %d, pTimeStamp = %11u", typeToString(type), groupToString(source), groupToString(dest),
            requestToString(pId), pId, pToken, requestToString(id), id, token, slotId, error, pTimeStamp);
}
...
sp<RfxMessage> RfxMessage::obtainRequest(int slotId, RILD_RadioTechnology_Group target, int id) {
    ...
    if (RADIO_TECH_GROUP_C2K == target) {
        // check c2k slot
        new_msg->slotId = RpCdmaLteModeController::getCdmaSocketSlotId();
    } else {
...
sp<RfxMessage> RfxMessage::obtainResponse(RIL_Errno e, const sp<RfxMessage>& msg,
    ...
    result->source = msg->getSource();
    result->slotId = msg->getSlotId();
