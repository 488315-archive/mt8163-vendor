...
bool RfxRootController::processMessage(const sp<RfxMessage>& message) {
    ...
    if(message->getId() == RIL_LOCAL_REQUEST_OEM_HOOK_ATCI_INTERNAL &&
            (message->getType() == REQUEST || message->getType() == RESPONSE)) {
        return processAtciRequest(message);
    }

    //RFX_LOG_D(RFX_LOG_TAG, "processMessage() begin");
    // check c2k slot
    if(message->getSource() == RADIO_TECH_GROUP_C2K &&
            (message->getType() == URC || message->getType() == RESPONSE)) {
    ...
    RFX_LOG_D(RFX_LOG_TAG, "processMessage() msg = [%s]", message->toString().string());
    ...
    if (handler == NULL) {
        ...
        if (REQUEST == message->getType()) {
            RFX_LOG_D(RFX_LOG_TAG, "processMessage() handler = NULL ==> request");
            ret = requestToRild(message);
        } else {
            RFX_LOG_D(RFX_LOG_TAG, "processMessage() handler = NULL ==> response/urc");
            ret = responseToRilj(message);
        }
    } else {
        RFX_LOG_D(RFX_LOG_TAG, "processMessage() handler = %p, %s",
