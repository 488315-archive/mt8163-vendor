...
// ASUS Patch Debug Dump
//#define ASUS_FW_DEBUG_EN
...
struct ADLReg {
  ...
  uint8_t bWSOffsetH; // Output threshold
	...
	// Dark Scene Param (for ASUS)
	uint8_t DarkSceneTh;
	uint8_t DarkSceneSlope;
  uint8_t DarkDCGain;
  uint8_t DarkACGain;

  // Binomial Scene Param (for ASUS)
	uint8_t BinomialTh;
	uint8_t BinomialSlope;
  uint8_t BinomialDCGain;
  uint8_t BinomialACGain;
	uint8_t BinomialTarRange;
...
private:
  ...
  uint32_t vSpikeLumaWeight(const uint16_t waHist[LUMA_HIST_NUM_WEIGHT], uint32_t * MaxBinIdx);
  ...
  void CurveFull2Lmt(uint16_t waCurCurve[LUMA_CURVE_NUM]); //
	...
	void ComputeSceneIdx(const uint16_t waHist[LUMA_HIST_NUM_WEIGHT], uint16_t & DarkSceneIdx, uint16_t & BinomialIdx);
