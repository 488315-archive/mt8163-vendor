...
public class CurrentNetworkInfoStatus extends PreferenceActivity {
    ...
    private void getSidNidPrlValue() {
        if (mCdmaPhone != null) {
            setSummaryText(KEY_CURRENT_SIDNID, mCdmaPhone.getServiceState().getSystemId()
                    + "," + mCdmaPhone.getServiceState().getNetworkId());
        }
    ...
    protected void onResume() {
        ...
        if (mCdmaPhone != null) {
            mServiceState = mCdmaPhone.getServiceState();
        } else {
            mServiceState = mPhone.getServiceState();
        }
        mSignalStrength = new SignalStrength(mPhone.getSignalStrength());
        if (isLteDataNwReg()) {
            //mSignalStrength.setLteRsrp(mCdmaPhone.getSignalStrength().getLteDbm());
        }
        updateServiceState();
        updateSignalStrength();
        updateNetworkType();
