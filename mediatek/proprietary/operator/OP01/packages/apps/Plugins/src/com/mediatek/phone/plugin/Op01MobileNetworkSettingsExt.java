        ...
        if (isLTESupport()) {
            if (slotId == -1 || isNotPrimarySIM(slotId) ||
                ...
                //if(mPreferenceScreen != null) {
                    ...
                    } else if (settingsNetworkMode == Phone.NT_MODE_LTE_CDMA_EVDO_GSM_WCDMA &&
                            SystemProperties.get("ro.mtk_c2k_support").equals("1")) {
                        int tempSettingsNetworkMode = Phone.NT_MODE_LTE_GSM_WCDMA;
                        preference.setValue(Integer.toString(tempSettingsNetworkMode));
                        log("updateNetworkTypeSummary, summary:" + preference.getEntry());
                        if (preference.getEntry() != null) {
                            preference.setSummary(preference.getEntry());
                            preference.setEnabled(true);
