...
public class OP01DialPadExtension extends DefaultDialPadExtension implements View.OnLongClickListener{
    ...
    public List<String> getSingleIMEI(List<String> list) {
        ...
        int cdmaSlot = SubscriptionManager.INVALID_SIM_SLOT_INDEX;
        for (int subId : subList) {
            int type = TelephonyManager.getDefault().getCurrentPhoneType(subId);
            Log.d(TAG, "current phone type = " + type);
            if (TelephonyManager.PHONE_TYPE_CDMA == type) {
                cdmaSlot = SubscriptionManager.getSlotId(subId);
                Log.d(TAG, "current cdma slot id = " + cdmaSlot);
            }
        }

        if (cdmaSlot != SubscriptionManager.INVALID_SIM_SLOT_INDEX) {
