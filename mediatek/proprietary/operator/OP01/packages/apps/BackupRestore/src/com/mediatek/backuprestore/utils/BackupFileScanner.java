    ...
    private class ScanThread extends Thread {
            ...
            if (files != null && files.length > 0) {
                for (File currentFile : files) {
                    ...
                    if (currentFile.exists() && currentFile.isDirectory()) {
                        ...
                        if (result != null
                                && result.length() > currentFile.getAbsolutePath().length()) {
                         ...
                         */
                        synchronized (mObject) {
                            if (checkDataForAdd(currentFile.getParentFile().getParentFile()
                                    .getAbsolutePath(), "PERSON_DATA")) {
                                BackupFilePreview mPersonData = new BackupFilePreview(currentFile
                ...
                for (int i = 0; i < mDataResult.size(); i++) {
                    ...
                    if (obj instanceof BackupFilePreview && type.equals("PERSON_DATA")) {
                        ...
                        if (backupFilePreview.getFile().getAbsolutePath().equals(path)) {
                            MyLogger.logD(CLASS_TAG, "checkDataForAdd : PERSON_DATA false path = "
                                    + path);
                            return false;
                        }
                        ...
                        if (oldData.getFilePath().equals(path)) {
                            MyLogger.logD(CLASS_TAG, "checkDataForAdd : OLD_DATA false path = "
                                    + path);
                            return false;
                        }
                    }
        ...
        private String scanBackupFiles(String path) {
            if (path != null && !mIsCanceled) {
                ...
                MyLogger.logD(CLASS_TAG, "scanBackuFile fileIndex length = " + fileIndex.length);
                String phonePath = SDCardUtils.getPhoneDataPath(mContext);
                if (!mIsFullScan && phonePath != null && path.contains(phonePath)) {
                    String pathSplit[] = phonePath.split(File.separator);
                    int index = fileIndex.length - pathSplit.length;
                    MyLogger.logD(CLASS_TAG, "scanBackuFile and index = " + index);
                    return filterFile(new File(path), index - 1);
                } else if (mIsFullScan) {
                    MyLogger.logD(CLASS_TAG, "scanBackuFile and the full scan index is 0 ");
                    return filterFile(new File(path), 0 );
                }
                ...
                for (int i = mDataResult.size() - 1; i >= 0; i--) {
                    ...
                    if (obj instanceof BackupFilePreview) {
                        ...
                        if (sdcard == null && !path.startsWith(phone)) {
                            MyLogger.logE(CLASS_TAG, " 1 checkDataNeedUpdata : checkFile = "
                                    + checkFile.getAbsolutePath());
                            mDataResult.remove(i);
                        } else if (!checkFile.exists()) {
                            MyLogger.logE(CLASS_TAG, " 2 checkDataNeedUpdata : checkFile = "
                                    + checkFile.getAbsolutePath());
                            mDataResult.remove(i);
                        } else {
