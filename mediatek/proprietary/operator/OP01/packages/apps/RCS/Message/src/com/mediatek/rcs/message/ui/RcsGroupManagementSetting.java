...
public class RcsGroupManagementSetting extends PreferenceActivity implements
    ...
    public final void onResume() {
        ...
        boolean isSmsEnabled = RcsMessageUtils.isSmsEnabled(getApplicationContext());
        Log.d(TAG, "onResume() isSmsEnabled:" + isSmsEnabled);
        if (!isSmsEnabled) {
            finish();
            return;
        }
    ...
    public final void onDestroy() {
        ...
        if (mRCSGroup != null) {
            ...
            RCSServiceManager.getInstance().getGroupManager().releaseRCSGroup(mGroupChatId);
        }
    ...
    private static final String GROUP_RICH_SCREEN = "pref_key_group_rich_screen";
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference == mManagerTransfer) {
            ...
            dialog.show();
        } else if (GROUP_RICH_SCREEN.equals(preference.getKey())) {
            startRichSreen();
        }
    ...
    private class RCSGroupAsyncTask extends AsyncTask<Void, Void, Long> {
        ...
        protected Long doInBackground(Void... arg0) {
            mRCSGroup = RCSServiceManager.getInstance().getGroupManager().getRCSGroup(mGroupChatId);
            if (mRCSGroup != null) {
    ...
    private void startRichSreen() {
        PackageManager pm = this.getPackageManager();
        try {
            ApplicationInfo info = pm.getApplicationInfo(PKG_NAME_RICH_SCREEN, 0);
        } catch (NameNotFoundException e) {
            Toast.makeText(this, R.string.please_download_it_plugin_center,
                    Toast.LENGTH_SHORT).show();
            return;
        }
        final ArrayList<RcsGroupMember.GroupMember> members = getGroupMember();
        if (members == null || members.size() < 1) {
            Toast.makeText(this, R.string.group_member_is_empty, Toast.LENGTH_SHORT).show();
            return;
        }
        ArrayList<String> nameList = new ArrayList<String>();
        ArrayList<String> numberList = new ArrayList<String>();
        for (RcsGroupMember.GroupMember m : members) {
            nameList.add(m.getName());
            numberList.add(m.getNumber());
        }
        String mode = MODE_GROUP;
        if (members.size() == 1) {
            mode = MODE_SINGLE_CONTACT;
        }
        Intent intent = new Intent("com.cmdm.rcs.RCSEntryAction");
        intent.putStringArrayListExtra("MsisdnList", numberList);
        intent.putStringArrayListExtra("NameList", nameList);
        intent.putExtra("Mode", mode);
        try {
            startActivity(intent);
        } catch (ActivityNotFoundException ex) {
            String activityNoFound = getString(R.string.activity_no_found);
            Toast.makeText(this, activityNoFound, Toast.LENGTH_SHORT).show();
        }
    }
}
