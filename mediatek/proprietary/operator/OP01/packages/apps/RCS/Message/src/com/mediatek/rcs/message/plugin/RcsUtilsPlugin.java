...
//            RCSMessageManager.createInstance(context);
    ...
    public CharSequence formatIpMessage(CharSequence inputChars, boolean showImg,
        ...
        if (inputBuf == null) {
            if (emoji.hasAnyEmojiExpression(inputChars)) {
                return emoji.getEmojiExpression(inputChars, showImg);
            } else if (EmojiShop.matchEmXml(inputChars.toString())) {
                return EmojiShop.parseEmSmsString(inputChars.toString());
            } else {
                return inputChars;
            }
        } else {
     ...
     * @param rcsId The id in rcs message table.
     * @param ipMessageId the id in stack table.
     */
    public static void unblockingNotifyNewIpMessage(final Context context,
            final long newMsgThreadId, final long rcsId) {
        Log.d(TAG, "unblockingNotifyNewIpMessage: id = " + rcsId);
        new Thread(new Runnable() {
            @Override
            public void run() {
                boolean notifiEnable = true;
                String chatId = RcsMessageUtils.blockingGetGroupChatIdByThread(context,
                                                                                   newMsgThreadId);
                if (!TextUtils.isEmpty(chatId)) {
                    notifiEnable = isGroupNotificationEnable(context, newMsgThreadId);
                    Log.d(TAG, "the group notification enable value = " + notifiEnable);
                }
                if (notifiEnable) {
                    sCallback.blockingIpUpdateNewMessageIndicator(context, newMsgThreadId,
                            false, null);
                }
                if (notifiEnable && isIpPopupNotificationEnable(context, newMsgThreadId)) {
                    notifyNewIpMessageDialog(context, rcsId);
                }
                notifyIpWidgetDatasetChanged(context);
            }
        }, "unblockingNotifyNewIpMessage").start();
    }
    ...
    private static boolean isGroupNotificationEnable(Context context, long threadId) {
        boolean ret = true;
        ...
        if (c != null) {
            try {
                if (c.getCount() > 0) {
                    c.moveToFirst();
                    int enableValue = c.getInt(0);
                    ret = enableValue == IpMessageConsts.GroupNotificationType.NOTIFICATION_ENABLE;
                }
            } finally {
                c.close();
            }
        }
        Log.d(TAG, "[isGroupNotificationEnable]: threadid of " + threadId + "  is: " + ret);
        return ret;
    }
    ...
    private static void notifyNewIpMessageDialog(Context context, long rcsId) {
