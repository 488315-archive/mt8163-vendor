...
public class RcsMessagingNotification extends DefaultIpMessagingNotificationExt {
     ...
     * @param chatId group's chat id
     */
    public static void updateNewGroupInvitation(Participant p, String subject, long threadId,
                                                String chatId) {
        RcsComposeActivity composer = RcsComposeActivity.getRcsComposer();
        Log.d(TAG, "updateNewGroupInvitation: threadId = " + threadId + "chatId = " + chatId
                + "subject = " + subject);
        if (composer != null && composer.getThreadId() == threadId) {
            Log.d(TAG, "inviting group thread is active, don't show group invitation in statusbar");
            return;
        }
        asynUpdateNewGroupInvitation(p, subject, threadId, chatId);
    }

    private static void asynUpdateNewGroupInvitation(final Participant p, final String subject,
                                            final long threadId, final String chatId) {
    ...
    private static void notifyNewGroupInvitation(final Participant p, final String subject,
        ...
        Bitmap largeIcon = PortraitManager.getGroupThumbnailBitmap(hostContext, chatId);
