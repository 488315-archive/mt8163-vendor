...
public class RcsMessageListItem extends DefaultIpMessageListItemExt implements
    ...
    private ImageView mBurnMsgCountDown; // delete msg indicator
    ...
    static final int EVENT_ACCEPT_GROUP_INVITATION = 1002;
    static final int EVENT_REJECT_GROUP_INVITATION = 1003;
    ...
    private final static int mTimerNum = 5;
    ...
    private View mSystemEvent;
    private TextView mSystemEventText;
    private View mGroupButton;
    private Button mGroupAcceptButton;
    private Button mGroupRejectButton;
...
//    public static final int account_icon = 4;
    ...
    public boolean onIpFinishInflate(Context context, TextView bodyTextView,
        ...
        mMessageManager = RCSServiceManager.getInstance().getRcsMessageManager();
...
//                return parentView.findViewById(R.id.avatar);
    ...
    private void initIpMessageResource() {
        if (mMsgListItem != null) {
            ...
            mSystemEvent = mMsgListItem.findViewById(R.id.system_event);
            if (mSystemEvent != null) {
                mSystemEventText = (TextView) mSystemEvent.findViewById(R.id.systen_event_text);
                mGroupButton = mSystemEvent.findViewById(R.id.group_button);
                mGroupAcceptButton = (Button) mSystemEvent.findViewById(R.id.btn_group_accept);
                mGroupRejectButton = (Button) mSystemEvent.findViewById(R.id.btn_group_reject);
            }
        }
    }
    ...
    public boolean onIpBind(IIpMessageItemExt ipMessageItem, long msgId,
        ...
        if (mRcsMessageItem != null && mRcsMessageItem.isSystemEvent()) {
            bindIpSystemEvent(mRcsMessageItem);
            return true;
        }
        bindPartipantInfo(mRcsMessageItem);
    ...
    public void onIpUnbind() {
        ...
        mRcsMessageItem.setGetIpMessageFinishMessage(null);
        mRcsMessageItem.detached();
        if (mMemberInfo != null) {
            mMemberInfo.removeChangedListener(this);
            mMemberInfo = null;
        }
        Log.d(TAG, "drawBurnMsgIndicator: onIpDetachedFromWindow()  mTimeTask = " + mTimeTask);
        if (mTimeTask != null) {
            mTimeTask.setHandler(null);
        }
        if (mBurnMsgCountDown != null) {
            mBurnMsgCountDown.setImageDrawable(null);
        }
        mVisible = false;
    }
...
//            mSenderPhoto.assignContactFromPhone(mRcsMessageItem.mAddress, false);
    ...
    private boolean bindIpmsg() {
        ...
        if (mSystemEvent != null) {
            mSystemEvent.setVisibility(View.GONE);
        }
        boolean processed = bindIpTextItem(mRcsMessageItem);
        if (processed) {
            return true;
        }
        ...
        if (!bindBurnedVoiceMsg(ipMessage)) {
            Log.d(TAG, " bindBurnedVoiceMsg finish");
            return false;
        }
        ...
        if (!processed) {
            ...
            case IpMessageType.PICTURE:
                setIpImageItem(ipMessage);
                break;
            case IpMessageType.VOICE:
                setIpVoiceItem(ipMessage);
                break;
            case IpMessageType.VIDEO:
                setIpVideoItem(ipMessage);
                break;
            case IpMessageType.VCARD:
                setIpVCardItem(ipMessage);
                break;
            case IpMessageType.GEOLOC:
                setIpGeolocItem(ipMessage);
                break;
            case IpMessageType.EMOTICON:
                setIpEmShopItem(ipMessage);
                break;
            default:
...
//                mMessageManager.sendBurnDeliveryReport(
//                        ipMessage.getTo(), ipMessage.getMessageId());
    ...
    private boolean bindBurnedMsgIndicator(IpMessage ipMessage) {
        Log.d(TAG, " [BurnedMsg]: drawBurnMsgIndicator bindBurnedMsgIndicator()");
        if (ipMessage == null || !ipMessage.getBurnedMessage()) {
            Log.d(TAG, "[BurnedMsg] drawBurnMsgIndicator ipMessage is null or not burn message");
        ...
        if (ipMessage != null) {
            Log.d(TAG, " [BurnedMsg]:  drawBurnMsgIndicator ipMessage.getBurnedMessage() = "
        ...
        if (mDirection == TYPE_OUTGOING) {
            ...
            if ((ipMessage.getType() == IpMessageType.TEXT
                    ...
                    && ((IpAttachMessage) ipMessage).getRcsStatus() == Status.FINISHED)) {
                Log.d(TAG, " [BurnedMsg]: drawBurnMsgIndicator sent text message or finished ft");
                drawBurnMsgIndicator(mRcsMessageItem);
    ...
    private boolean bindIpTextItem(RcsMessageItem item) {
        if (item.mRcsMsgType == MessageType.IM) {
            ...
            if (!TextUtils.isEmpty(mRcsMessageItem.mBody)) {
                if (EmojiShop.matchEmXml(mRcsMessageItem.mBody)) {
                    setIpTextItem(EmojiShop.parseEmSmsString(mRcsMessageItem.mBody));
                } else {
                    setIpTextItem(mRcsMessageItem.mBody);
                }
    ...
    private void setIpTextItem(String body) {
    ...
    private void setIpEmShopItem(IpMessage ipMessage) {
    ...
    private void setIpImageItem(IpMessage ipMessage) {
   ...
   private void setIpVCardItem(IpMessage ipMessage) {
    ...
    private void drawBurnMsgIndicator(RcsMessageItem msgItem) {
        Log.d(TAG, "drawBurnMsgIndicator() ");
        mBurnMsgCountDown.setVisibility(View.VISIBLE);
        startTimer();
    }
    ...
    private void startTimer() {
        Log.d(TAG, "drawBurnMsgIndicator: startTimer() ");

        mTimeTask = sBurnedTimerMap.get(Long.valueOf(mRcsMessageItem.mMsgId));
        Log.d(TAG, "drawBurnMsgIndicator: startTimer()  mMsgId = " + mRcsMessageItem.mMsgId +
        ...
        if (mTimeTask != null) {
            ...
            Log.d(TAG, "drawBurnMsgIndicator: startTimer()  count = " + count);
            if (count > 0) {
                mBurnMsgCountDown.setImageResource(ipbarmsgshareIconArr[count - 1]);
            } else {
            ...
            int count = mTimerNum;
            Log.d(TAG, "drawBurnMsgIndicator: text sentTime = " + sentTime);
            if (sentTime > 0) {
                int timer = (int) ((System.currentTimeMillis() - sentTime) / 1000);
                Log.d(TAG, "drawBurnMsgIndicator: timer = " + timer);
                if (timer >= 5) {
  ...
  private class BurnedMsgTask extends TimerTask {
      ...
      public int getCurrentCount() {
        ...
        public void run() {
            ...
            Log.d(TAG, " drawBurnMsgIndicator:  send  msg.what = " +
                    msg.what + " mBurnedMsgId = "+mBurnedMsgId);
            if (mCallBackHandler != null) {
                mCallBackHandler.sendMessage(msg);
            }
            if (mCount == -1) {
                Log.d(TAG, " drawBurnMsgIndicator:  this.cancel() ");
                this.cancel();
                //TODO: delete the msg
                if (!mVisible) {
    ...
    Handler burnedMsgHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            Log.d(TAG, "drawBurnMsgIndicator: icon num = " + num + ", " +
                "mVisible: " + mVisible + " msgId = " + msgId);
            if (!mVisible || msgId != mRcsMessageItem.mMsgId) {
                return;
            }
            if (num > 0 ) {
                mBurnMsgCountDown.setImageResource(ipbarmsgshareIconArr[num - 1]);
            } else if (num == 0 ) {
                mBurnMsgCountDown.setImageDrawable(null);
                mBurnMsgCountDown.setVisibility(View.GONE);
    ...
    private void deleteBurnedMsg(final long msgBurnedId) {
        Log.w(TAG, " drawBurnMsgIndicator, deleteBurnedMsg(), msgBurnedId = " + msgBurnedId);
        new Thread(new Runnable() {
            public void run() {
                try {
                    Log.d(TAG, " [BurnedMsg]:  deleteBurnedMsg");
                    mMessageManager.deleteRCSMessage(msgBurnedId);
    ...
    private boolean bindIpSystemEvent(RcsMessageItem item) {
        Log.d(TAG, "bindIpSystemEvent(): msgId = " + item.mMsgId);
        if (item == null) {
        ...
        if (mSystemEvent == null) {
            Log.e(TAG, "bindSystemEvent but systemEvent view is null");
            return false;
        }
        mSystemEvent.setVisibility(View.VISIBLE);
        mSystemEventText.setVisibility(View.VISIBLE);
        String body = item.mBody;
        if (!TextUtils.isEmpty(body)) {
            mSystemEventText.setText(item.mBody);
        }
        if (item.mRcsMsgClass >= RcsLog.Class.SYSTEM
                && item.mRcsStatus == RcsLog.InvitationStatus.INVITING) {
            mGroupButton.setVisibility(View.VISIBLE);
            mGroupAcceptButton.setOnClickListener(mGroupOnClickListener);
            mGroupRejectButton.setOnClickListener(mGroupOnClickListener);
        } else {
            mGroupButton.setVisibility(View.GONE);
        }
        Log.d(TAG, "bindIpSystemEvent(): finish");
        return true;
    }
    private OnClickListener mGroupOnClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            int msgWhat = 0;
            if (v == mGroupAcceptButton) {
                msgWhat = EVENT_ACCEPT_GROUP_INVITATION;
            } else if (v == mGroupRejectButton) {
                msgWhat = EVENT_REJECT_GROUP_INVITATION;
            }
            if (msgWhat > 0) {
                Message msg = mMsgListItemHandler.obtainMessage(msgWhat, mRcsMessageItem);
                msg.sendToTarget();
            }
        }
    };

    private void bindPartipantInfo(RcsMessageItem item) {
        if (mThreadId != -1) {
            if (!mIsGroupItem) {
