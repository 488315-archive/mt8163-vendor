...
public class PortraitManager implements PortraitService.UpdateListener, INotifyListener {
    ...
    private PortraitManager(Context context) {
        ...
        float width = context.getResources().getDimension(R.dimen.stoke_width_for_group_thumbnail);
        int color = context.getResources().getColor(R.color.stoke_color_for_group_thumbnail);
        mPService.setGroupThumbnailBoder(width, color);
    ...
    public GroupThumbnail getGroupPortrait(String chatId) {
        GroupThumbnail groupThumbnail = mGroupThumbnailCache.get(chatId);
        Log.d(TAG, "getGroupPortrait: chatId = " + chatId + ", groupThumbnail = " + groupThumbnail);
        if (groupThumbnail == null) {
            Bitmap bitmap = mPService.requestGroupThumbnail(chatId);
            groupThumbnail = new GroupThumbnail(chatId, bitmap);
    ...
    public static BitmapDrawable getGroupBitmapDrawable(Context context, String chatId) {
        Log.e(TAG, "[getGroupBitmapDrawable]: chatId is " + chatId);
        if (TextUtils.isEmpty(chatId)) {
            return null;
        }
        PortraitManager pm = PortraitManager.getInstance();
        Bitmap bitmap = pm.mPService.getGroupThumbnail(chatId);
        GroupThumbnail groupThumbnail = pm.mGroupThumbnailCache.get(chatId);
        if (groupThumbnail != null) {
            groupThumbnail.update(bitmap);
        }
    ...
    public static Bitmap getGroupThumbnailBitmap(Context context, String chatId) {
        if (TextUtils.isEmpty(chatId)) {
        ...
        Bitmap bitmap = pm.mPService.getGroupThumbnail(chatId);
        GroupThumbnail groupThumbnail = pm.mGroupThumbnailCache.get(chatId);
        if (groupThumbnail != null && bitmap != null) {
            groupThumbnail.update(bitmap);
        }
        Log.e(TAG, "getGroupBitmapDrawable: is null");
        return bitmap;
    ...
    public void clearAllGroupThumbnails() {
        Log.d(TAG, "clearAllGroupThumbnails");
        mGroupThumbnailCache.evictAll();
    }

    private void updateGroupThumbnailbyChatId(String chatId) {
        GroupThumbnail groupThumbnail = mGroupThumbnailCache.get(chatId);
        if (groupThumbnail != null) {
            Bitmap bitmap = mPService.requestGroupThumbnail(chatId);
            groupThumbnail.update(bitmap);
