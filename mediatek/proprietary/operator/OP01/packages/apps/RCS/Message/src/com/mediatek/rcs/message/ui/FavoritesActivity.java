...
public class FavoritesActivity extends ListActivity {
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
        ...
        if (!RcsMessageUtils.isSmsEnabled(getApplicationContext())) {
            MenuItem item = menu.findItem(R.id.menu_foward);
            if (item != null) {
    ...
    class CarrymoreActionMode implements ActionMode.Callback {
        ...
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            ...
            case R.id.favorite_foward:
                if (!RcsMessageUtils.isSupportRcsForward(FavoritesActivity.this)) {
                    ...
                    if (mCheckedCount > 1) {
                        Toast.makeText(FavoritesActivity.this, R.string.forward_items_more_than_one,
                                Toast.LENGTH_SHORT).show();
                ...
                if (mCheckedCount > 5) {
                    Toast.makeText(FavoritesActivity.this, R.string.forward_items_more_than_five,
                            Toast.LENGTH_SHORT).show();
                    setAllItemChecked(false);
                    return false;
                }
                    ...
                    for (int id : mCheckedItemIds) {
                        ...
                        if (type == ChatService.MMS) {
                            Toast.makeText(FavoritesActivity.this,
                                    R.string.mms_cannot_forward_with_others,
                                    Toast.LENGTH_SHORT).show();
                            setAllItemChecked(false);
                            cursor.close();
                            return false;
        ...
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            ...
            updateTitle();
            if (!RcsMessageUtils.isSmsEnabled(getApplicationContext())) {
                Log.d(TAG, "onCreateActionMode, Mms is not default sms application," +
                        "remove forward item");
                MenuItem item = menu.findItem(R.id.favorite_foward);
                if (item != null) {
                    item.setVisible(false);
                }
            }
    ...
    private Intent getClickIntent(Cursor cursor) {
        ...
        if (type == ChatService.IM || type == ChatService.SMS) {
            ...
            intent.putExtra("type", IpMessageType.TEXT);
        ...
        if (ct != null) {
            if (ct.equals(Constants.CT_TYPE_VEDIO)) {
                ...
                vidioIntent.putExtra("type", IpMessageType.VIDEO);
                ...
                imageIntent.putExtra("type", IpMessageType.PICTURE);
