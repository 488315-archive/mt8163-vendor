...
public class RcsConversation extends DefaultIpConversationExt {
    ...
    public int onIpFillFromCursor(Context context, Cursor c, int recipientSize, String number,
        ...
        Log.d(TAG, "onIpFillFromCursor, threadId = " + threadId);
        ...
        if (RcsConversationList.mStickyThreadsSet != null) {
            if (mIsSticky) {
                RcsConversationList.mStickyThreadsSet.add(threadId);
            } else {
                RcsConversationList.mStickyThreadsSet.remove(threadId);
            }
        }
    ...
    public long getThreadId() {
        long threadId = mConversationCallback.getThreadIdCallback();
        return threadId;
    }
    ...
    public long guaranteeIpThreadId(long threadId) {
        ...
        if (threadId <= 0) {
            threadId = RCSDataBaseUtils.createGroupThreadByChatId(
                    ContextCacher.getHostContext(), mGroupChatId);
        }
        Log.d(TAG, "[guaranteeIpThreadId] threadId = " + threadId);
        return threadId;
    }
...
//        mThreadId = info.getThreadId();
    ...
    public Uri getUri() {
        ...
        if (threadId <= 0)
            return null;
        return getConversationUri(threadId);
    }
     ...
     // If we have no Uri to mark (as in the case of a conversation that
        ...
        Uri threadUriOfUnreadCount =
            ...
            Cursor c = context.getContentResolver().query(uri,
                    ...
                    new String[]{Long.toString(threadId)}, null);
            if (c != null) {
                try {
                ...
                if (allNeedUpdateCount >= UPDATE_LIMIT) {
                    while (updateCount > 0) {
                        updateCount = context.getContentResolver().update(
                                ...
                                new String[] {threadId + "", threadId + "", threadId + ""});
                    ...
                    updateCount = context.getContentResolver().update(uri, sReadContentValues,
                            RCSMSG_UNREAD_SELECTION, new String[]{Long.toString(threadId)});
                    Log.d(TAG, "markAsRead-updateThread: updateCount=" + updateCount);
                }
