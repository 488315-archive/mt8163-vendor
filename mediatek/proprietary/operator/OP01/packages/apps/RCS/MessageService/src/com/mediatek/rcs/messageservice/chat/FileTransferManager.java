...
public class FileTransferManager {
    ...
    private static class SentFileTransferManager implements SentFileTransfer.IOnSendFinishListener{
        ...
        public void onAddSentFileTransfer(SentFileTransfer sentFileTransfer) {
            ...
            if (null != sentFileTransfer) {
                ...
                //check list status log
                Log.d(TAG, "check list status, onAddSentFileTransfer, mPendingList.size() = "
                + mPendingList.size() + "mActiveList.size() = " + mActiveList.size() +
                " mResendableList.size() = " + mResendableList.size());
        ...
        public synchronized void autoResendFileTransfer() {
            Log.d(TAG, "autoResendFileTransfer enter");
            int size = mResendableList.size();
            Log.d(TAG, "ResendableList size = " + size);
            for (SentFileTransfer fileTransfer: mResendableList) {
                if (fileTransfer.canAutoResend()) {
                    fileTransfer.mResendType = SentFileTransferManager.AUTO_RESEND;
                    resendFileTransfer(fileTransfer, true);
                }
            }
        }

        public void resendFileTransfer(SentFileTransfer sentFileTransfer,
                              boolean haveInstanceInResendableList) {
            Log.d(TAG, "resendFileTransfer enter");
            //check list status log
            Log.d(TAG, "check list status, resendFileTransfer, mPendingList.size() = "
                + mPendingList.size() + "mActiveList.size() = " + mActiveList.size() +
                " mResendableList.size() = " + mResendableList.size());

            sentFileTransfer.recordRetryTimes();

            if (isResendingInstance(sentFileTransfer)) {
                Log.d(TAG, "resendFileTransfer, it is a exist resending instance !");
                return;
            }
            if (haveInstanceInResendableList && sentFileTransfer != null) {
        ...
        public void onSendFinish(final SentFileTransfer sentFileTransfer, final Result result) {
            ...
            if (mActiveList.contains(sentFileTransfer)) {
                ...
                //check list status log
                Log.d(TAG, "check list status, onSendFinish, mPendingList.size() = "
                + mPendingList.size() + "mActiveList.size() = " + mActiveList.size() +
                " mResendableList.size() = " + mResendableList.size());
    ...
    public static class SentFileTransfer {
        ...
        public boolean canAutoResend() {
            Log.d(TAG, "canAutoResend(), retryTimes = " + retryTimes);
            if (retryTimes > 3) {
                return false;
            } else {
                return true;
        ...
        public void recordRetryTimes() {
            retryTimes++;
            Log.d(TAG, "recordRetryTimes(), retryTimes = " + retryTimes);
        }
        ...
        protected void resend() {
            ...
            if (instance != null && (mChatType == ONE2ONE || mChatType == ONE2MULTI)) {
                ...
                try {
                    if (mResendType == SentFileTransferManager.AUTO_RESEND) {
                        ...
                        IGroupChat groupChat = mService.getOrCreateGroupChat(mChatId);
                        groupChat.resendFile(mIpMessageId);
                    } else {
                        fileTransferService = instance.getFileTransferApi();
                        Log.d(TAG, "resumeGroupFileTransfer,  mChatSessionId = " + mChatSessionId);
                        mFileTransferObject = fileTransferService.resumeFileTransfer(
                                mFileTransferTag, mFileTransferListener);
                    }
                } catch (JoynServiceException e) {
                    Log.e(TAG, "Can't get fileTransferService! ");
                }
            ...
            try {
                if (null != mFileTransferObject) {
                    ...
                    FTChatWindow.updateFileTansferTag(fileTransferId);
                    ...
                    try {
                        ...
                        if (mChatType == ONE2ONE) {
                            ...
                            if (mBurnType == true) {
                                ...
                                mFileTransferObject = fileTransferService.transferFile(
                                        mContact, mFileStruct.mFilePath,
                                        getThumnailFile(mFileStruct.mFilePath),
                                        0,
                                        FileTransfer.Type.BURNED,
                                        mFileTransferListener);
                            ...
                            mFileTransferObject = fileTransferService.transferFileToMultiple(
                                    contacts,
                                    mFileStruct.mFilePath,
                                    null,
                                    mDuration,
                        ...
                        if (null != mFileTransferObject
                            ...
                            mIpMessageId = FTChatWindow.updateFileTansferTag(fileTransferId,
                                    oldFileTransferTag);
    ...
    protected class ReceiveFileTransfer {
        ...
        private class FileTransferReceiverListener extends FileTransferListener {
            ...
            public void onFileTransferred(String filename) {
                ...
                FTChatWindow.updateFilePath(filename);
                if (isAutoAccept) {
    ...
    public void handleResendFileTransfer(long ipMsgId) {
        ...
        if (isDummyId(ipMsgId)) {
            ...
            boolean needSaveDb = false;
            boolean haveInstanceInResendableList = true;

            if (null != fileTransfer) {
                Log.d(TAG, "handleResendFileTransfer, null != fileTransfer");
                fileTransfer.mSentFileTransferType = SentFileTransferManager.TO_RESUME;
                mOutGoingFileTransferManager.resendFileTransfer(fileTransfer,
                                  haveInstanceInResendableList);
            } else {
                ...
                haveInstanceInResendableList = false;
                mOutGoingFileTransferManager.resendFileTransfer(fileTransfer,
                                            haveInstanceInResendableList);
            }
        }
    ...
    public void handleRecevieFileTransferInvitation(Intent invitation) {
        ...
        String type = invitation.getStringExtra(FileTransferIntent.EXTRA_TRANSFERTYPE);
        boolean isBurn = type.equals(FileTransfer.Type.BURNED);
    ...
    public void handleReAcceptFileTransferInGroup(String fileTransferTag) {
        Log.w(TAG, "handleReAcceptFileTransferInGroup() entry...");
        Log.w(TAG, "fileTransferTag = " + fileTransferTag);
        ReceiveFileTransfer receiveFileTransfer = mReceiveFileTransferManager
                .findFileTransferByTagFromActiveList(fileTransferTag);
        if (null != receiveFileTransfer) {
            Log.w(TAG, "null != receiveFileTransfer, call reAcceptFileTransferInvitation directly");
    ...
    public void onStatusChanged(boolean status) {
        ...
        if (status == true) {
            // resend it
            mOutGoingFileTransferManager.autoResendFileTransfer();
        }
    }
