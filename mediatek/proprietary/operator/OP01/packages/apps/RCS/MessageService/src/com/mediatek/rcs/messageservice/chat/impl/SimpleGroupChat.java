...
import com.mediatek.rcs.messageservice.chat.FileTransferManager;
import com.mediatek.rcs.messageservice.chat.GsmaManager;
import com.mediatek.rcs.messageservice.chat.IGroupChat;
...
public class SimpleGroupChat extends BaseChatImpl implements IGroupChat {
     ...
     */
    @Override
    public void handleInvitation(String rejoinId, Participant participant) {
        Logger.d(TAG, "handleInvitation() entry: " + rejoinId);
        InvitationInfo info = new InvitationInfo(rejoinId, participant);
        mGroupSession.sendMessage(OP_GROUP_INVITATION, info);
    }
    ...
    public void resendChatMessage(final String messageId) {
        ...
        SendingMessage sendingMsg = findSendingMessageFromList(messageId);
        if (sendingMsg != null && (sendingMsg.state == SendingMessage.SNEDING ||
                sendingMsg.state == SendingMessage.RESENDING)) {
            Logger.d(TAG, "resendChatMessage() already resending: " + messageId);
            return;
        }

        if (sendingMsg == null) {
            sendingMsg = addMsgToSendingList(messageId);
            Logger.d(TAG, "sendMessage() setup timer msg");
            Message timerMsg = mWorkHandler.obtainMessage(BASE_OP_SEND_MESSAGE_TIMEOUT, sendingMsg);
            mWorkHandler.sendMessageDelayed(timerMsg, MAX_MSG_PENDING_TIME);
        }
    ...
    public void downloadFile(String fileTag) {
        FileTransferManager.getInstance(mService).handleAcceptFileTransferInGroup(fileTag);
        //we modify the RCS Stack and make app can download file directly.
        //the file transfer download can not dependency on the Group State.
    ...
    @Override
    public void redownloadFile(String fileTag) {
        //redownload is different.
    ...
    public void addParticipants(List<Participant> participants) {
        mGroupConfigHandler.obtainMessage(OP_ADD_PARTICIPANT, participants).sendToTarget();
    }

    @Override
    public void removeParticipants(List<Participant> participants) {
        mGroupConfigHandler.obtainMessage(OP_REMOVE_PARTICIPANT, participants).sendToTarget();
    }

    @Override
    public void modifySubject(String subject) {
        mGroupConfigHandler.obtainMessage(OP_MODIFY_SUBJECT, subject).sendToTarget();
    }

    @Override
    public void modifyGroupLocalName(String localName) {
        RCSDataBaseUtils.modifyGroupNickName(mContext, getChatId(), localName);
    }

    @Override
    public void modifyRemoteAlias(String alias) {
        mGroupConfigHandler.obtainMessage(OP_MODIFY_MYNICKNAME, alias).sendToTarget();
    }

    @Override
    public void transferChairman(Participant participant) {
        mGroupConfigHandler.obtainMessage(OP_TRANSFER_CHAIRMAN, participant).sendToTarget();
    }

    @Override
    public void quit() {
        mGroupConfigHandler.obtainMessage(OP_QUIT_GROUP).sendToTarget();
    }

    @Override
    public void abort() {
        mGroupConfigHandler.obtainMessage(OP_ABORT_GROUP).sendToTarget();
    }

    @Override
    public void onReceiveChatMessage(ExtendMessage message) {
        Logger.d(TAG, "onReceiveChatMessage() entry, messageId: " + message.getId());
        ExtendMessage messageEx = message;
        int messageType = messageEx.getMessageType() == ChatLog.Message.Type.EMOTICON ?
    ...
    private final class SessionStateMachine extends StateMachine {
        ...
        class UnavailableState extends State {
            ...
            public boolean processMessage(Message msg) {
                ...
                case OP_GROUP_INVITATION:
                    ...
                    InvitationInfo info = (InvitationInfo) msg.obj;
                    mGroupData.addRejoinId(info.mRejoinId);
                    try {
                        mGroupChat = GsmaManager.getInstance().getChatApi()
                                .getGroupChat(getChatId());
                        int state = mGroupChat.getState();
                        ...
                        // report UI
                        mService.getListener().onNewInvite(info.mParticipant, mGroupData.subject,
                                mGroupData.chatId);
                ...
                case NOTIFICATION_RECEIVE_MESSAGE:
                    ExtendMessage message = (ExtendMessage) msg.obj;
            ...
            try {
                if (resend == 0) {
                    ...
                    if (type == Class.EMOTICON) {
                        messageId = mGroupChat.sendMessage(para, ChatLog.Message.Type.EMOTICON);
                    } else {
                    ...
                    SendingMessage sendingmsg = findSendingMessageFromList(smsMsgId);
                    if (sendingmsg != null) {
                        sendingmsg.rcsMsgId = messageId;
        ...
        private void handleStartGroupResult(boolean result) {
            if (result) {
                String contact = "";
                for (Participant participant : mGroupData.invitingParticipants) {
                    contact = contact + RCSDataBaseUtils.getContactDisplayName(mContext,
                            participant.getContact()) + ",";
                }
                contact = contact.substring(0, contact.length() - 1);
                String body = mContext.getString(R.string.group_invite_to_group, contact);
                addGroupSysMessage(getChatId(), null, body, false, true);
        ...
        private void handleChatMessageSendFail(final long smsMsgId) {
            onSendChatMessageResult(smsMsgId, false);
        }
        ...
        private void handleModifySubjectResult(int result) {
            if (result == GroupChat.ReasonCode.SUCCESSFUL) {
                // mSubject = ////TODO:
                mGroupData.syncSubject();
                String body = mContext.getString(R.string.group_subject_modified,
                        mGroupData.subject);
                addGroupSysMessage(getChatId(), null, body, false, false);
        ...
        private void handleChairmanTransfered(final String chairman) {
            ...
            boolean isMe = PhoneNumberUtils.compare(chairman, me);
        ...
        private void handleReceiveChatMessage(final ExtendMessage message) {
    ...
    private class SimpleGroupChatListener extends GroupChatListener {
        ...
        /**
         * Callback called when a new ExtendMessage has been received
         *
         * @param message
         *            New chat message
         * @see ChatMessage
        ...
        public void onNewExtendMessage(ExtendMessage message) {
            Logger.d(TAG, "onNewExtendMessage() entry Contact: " + message.getContact() +
        ...
        public void onReportMessageFailedContact(String msgId, String contact) {
            Logger.d(TAG, "onReportMessageFailedContact() entry, msgId: " + msgId + "contact: "
                    + contact);
            // this is because the SENT result must be handled after message send,
            // so add to the FSM and run in a queue.
    ...
    private class SimpleGroupChatData {
        ...
        void handleFullConferenceNotify(List<ConferenceUser> users) {
            ...
            mService.getListener().onSyncFullConferenceNotifyDone(chatId);
        }
    ...
    class InvitationInfo {
        public String mRejoinId;
        public Participant mParticipant;
        public InvitationInfo(String rejoinId, Participant participant) {
            mRejoinId = rejoinId;
