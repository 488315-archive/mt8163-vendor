...
public class PictureItem extends MessageListItem {
    ...
    public void bind(MessageData messageData) {
        ...
        if (mMessageData.getMessageContent().direction == Constants.MESSAGE_DIRECTION_OUTGOING) {
            ...
            String ext = Utils.getFileExtension(imagePath);
            ...
            if (Utils.isExistsFile(imagePath)) {
                if (mIsGif) {
                    drawGif(mImageContent, mIpImageSizeBg, 0, imagePath, DRAW_TYPE_NORMAL);
                } else {
    ...
    protected void updateAfterDownload(final int index, String path) {
        if (index == GIF) {
            ...
            drawGif(mImageContent, mIpImageSizeBg, 0, path, DRAW_TYPE_NORMAL);
    ...
    private boolean drawGif(GifImageView iv, View bgView, int index,
        ...
        if (!iv.drawGif(path)) {
            return drawThumbnail(iv, bgView, index, path, type);
        }
