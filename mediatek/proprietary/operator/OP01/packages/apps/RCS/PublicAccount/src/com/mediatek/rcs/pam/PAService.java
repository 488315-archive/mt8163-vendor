...
public class PAService {
    ...
    private ConcurrentHashMap<Long, IPAServiceCallback> mWeekCallbacks =
            new ConcurrentHashMap<Long, IPAServiceCallback>();
    private List<ServiceConnectNotify> mNotifyList = null;

    private PAService(final Context context, final ServiceConnectNotify notify) {
        ...
        mServiceConnection = new ServiceConnection() {
            ...
            public synchronized void onServiceConnected(ComponentName name, IBinder service) {
                ...
                for (ServiceConnectNotify notifyItem : mNotifyList) {
                    notifyItem.onServiceConnected();
                }
            }
        ...
        mNotifyList = new ArrayList<ServiceConnectNotify>();
    ...
    public static synchronized void init(Context context, ServiceConnectNotify notify) {
        Log.d(TAG, "init()");
        if (mInstance == null) {
            Log.d(TAG, "new PAService()");
            mInstance = new PAService(context, notify);
        } else {
            if (mInstance.mServiceImpl != null) {
                Log.d(TAG, "init() callback directly");
                notify.onServiceConnected();
            } else {
                mInstance.mNotifyList.add(notify);
            }
        }
    }
    ...
    public boolean isServiceConnected(long token) {
        return mServiceImpl != null;
    }

    // TODO use gsma termial api exception instead
    public boolean isRcsServiceConnected(long token) {
        try {
    ...
    private void checkRcsConnectStatus(long token) {
        if (token == Constants.INVALID) {
            throw new Error("Invalid token");
        }
        if (!isRcsServiceConnected(token)) {
            throw new Error("Service not connected");
        }
    }
