...
public abstract class ClientQueryActivity extends Activity {
    ...
    private SimpleCallback mPAClientCallBack = new SimpleCallback() {
        ...
        public void doWhenComplete(long requestId, int resultCode,
            ...
            } else {//this is possible,means pressed cancel button
                //do nothing
            }
    ...
    private int getPageSize() {
        final int DEFAULT_ITEM_HEIGHT = 60;
        final int DEFAULT_PAGE_SIZE = 12;
        final float scale = mContext.getResources().getDisplayMetrics().density;
        int itemHeight = (int )(scale*DEFAULT_ITEM_HEIGHT+0.5f);

        View content = getWindow().findViewById(Window.ID_ANDROID_CONTENT);
        if (content == null) {
            return DEFAULT_PAGE_SIZE;
        }
        int viewHeight = content.getRootView().getBottom() - content.getTop();
        Log.d(mTag, "getPageSize, viewHeight = " + viewHeight);
        if (viewHeight <= 0) {
            return DEFAULT_PAGE_SIZE;
        }
        if (viewHeight % itemHeight == 0) {
            return viewHeight/itemHeight;
        } else {
            return viewHeight/itemHeight + 1;
        }
    }
}
