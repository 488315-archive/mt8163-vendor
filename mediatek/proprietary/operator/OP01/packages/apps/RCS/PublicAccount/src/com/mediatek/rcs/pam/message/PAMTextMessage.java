...
public class PAMTextMessage extends PAMBaseMessage {
    ...
    private String sendInternal(ExtendChat chat, String msg) throws JoynServiceException {
        String sourceId;
        Log.d(TAG, "sendMessage Start");
        sourceId = chat.sendMessage(msg, ChatLog.Message.Type.PUBLIC);
        Log.d(TAG, "sendMessage End. sourceId=" + sourceId);
        return sourceId;
    }
    ...
    private ExtendChat getChat() {
        ExtendChat chat = mMessageHelper.getChatCache(mToken, mUuid);
        if (null == chat) {
            chat = createNewChat();
            if (null != chat) {
    ...
    private ExtendChat createNewChat() {
        Log.d(TAG, "createNewChat()");
        ExtendChat chat = null;
        try {
            chat = mMessageHelper.getChatService().openSingleChat(
