...
public class RichCallInsController {
    ...
    public static void clearActivityMap(Activity activity) {
        ...
        Iterator<Map.Entry<Integer, Activity>> it =
            mHashActivity.entrySet().iterator();
        while(it.hasNext()) {
            Map.Entry<Integer, Activity> entry = it.next();
            Activity entryActivity = entry.getValue();
            if (entryActivity == activity) {
                Log.d(TAG, "clear hashmap item~~~");
                it.remove();
            }
        }
