...
bool mt_is_ftl_encrpted(const char* path){
	Volume* v = volume_for_path(path);
    if (v == NULL) {
        printf("unknown volume for path [%s]\n", path);
        return -1;
    }
	if (strcmp(v->fs_type, "ext4") == 0 && strstr(v->blk_device, "ftl")){
		if (strstr(v->fs_options, "forceencrypt"))
			printf("path=%s is ftl and encrpt\n", path);
		return 1;
	}
	return 0;
}
...
int userdata_backup(uint64_t backup_size)
{
...
#ifdef MTK_FTL_SUPPORT
	ui->Print("userdata_backup check ftl\n");
	if (mt_is_ftl_encrpted("/data")){
		phone_encrypted = 0;
		ui->Print("userdata_backup force encrypted\n");
	}
#endif
    ...
    if (get_nand_type() == 0) {
        ui->Print("Trying to attach ubifs /data \n");
        if (ensure_ubi_attach("/data") < 0){
            ui->Print("Error:userdata_backup attach user data fail\n");
            return INSTALL_ERROR;
		}
    }
