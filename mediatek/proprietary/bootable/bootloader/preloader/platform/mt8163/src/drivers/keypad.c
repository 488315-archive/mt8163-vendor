...
/* USB download w/o VBAT mode enable/disable bit. This bit would lock keypad,so it should be disable after power-on*/
	pmic_config_interface(STRUP_CON3, 0x0, PMIC_RG_USBDL_EN_MASK, PMIC_RG_USBDL_EN_SHIFT);
	...
	pmic_config_interface(STRUP_CON3, 0x0, PMIC_RG_FCHR_PU_EN_MASK, PMIC_RG_FCHR_PU_EN_SHIFT);
#else
  /* Enable pull-up resistor for HOMEKEY pin */
	pmic_config_interface(STRUP_CON3, 0x1, PMIC_RG_FCHR_PU_EN_MASK, PMIC_RG_FCHR_PU_EN_SHIFT);
#endif
