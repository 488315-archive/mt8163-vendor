...
)
{
    ...
    CAM_LOGD("[DumpToFile] S_u4RunningNumber: %d.\n", S_u4RunningNumber);
    CAM_LOGD("[DumpToFile] opening file [%s]\n", fname);
    ...
    CAM_LOGD("[DumpToFile] writing %d bytes to file [%s]\n", size, fname);
    while (written < size) {
    ...
    CAM_LOGD("[DumpToFile] done writing %d bytes to file [%s] in %d passes\n", size, fname, cnt);
    fclose(pFp);
...
MVOID getHDRExpSetting(const HDRExpSettingInputParam_T& rInput, HDRExpSettingOutputParam_T& rOutput)
{
    ...
    if(rInput.u4AOEMode==1) //Dynamic EV Bracket
    {
        ...
        if(dfGainDiff[0]!=0)
        {
            ...
            if((dfGainDiff[1]/dfGainDiff[0]) < dfGainDiffTh)//Use SE, LE only
            {
                ...
                {
                    CAM_LOGD("SWHDR Error calculation of 2 frm dfGainDiff = %f\n",dfGainDiff[0]);
                }
            }
            else
            {
    ...
    if(hdr_debug_mode)
    {
        ...
        CAM_LOGD("SWHDR AE Mode = %d\n",rInput.u4AOEMode);
        CAM_LOGD("SWHDR FrmNum = %d\n",rOutput.u4OutputFrameNum);
        CAM_LOGD("SWHDR Input delta EV(Bottom) = %f, TopEV = %f, Avg0_40 = %d Avg0_40_2 = %f\n",dfTargetBottomEV, dfTargetTopEV, rInput.i4LE_LowAvg, DarkAvg);

        CAM_LOGD("SWHDR AE HDRFlag Mode = %d\n",HDRFlag);
        CAM_LOGD("SWHDR Input 0EV Time = %d, Gain = %d Flare = %d \n",rInput.u4ExpTimeInUS0EV, rInput.u4SensorGain0EV,rInput.u1FlareOffset0EV);
        CAM_LOGD("SWHDR Output 0 Time = %d, Gain = %d \n",rOutput.u4ExpTimeInUS[0], rOutput.u4SensorGain[0]);
        CAM_LOGD("SWHDR Output 1 Time = %d, Gain = %d \n",rOutput.u4ExpTimeInUS[1], rOutput.u4SensorGain[1]);
        CAM_LOGD("SWHDR Output 2 Time = %d, Gain = %d \n",rOutput.u4ExpTimeInUS[2], rOutput.u4SensorGain[2]);
        CAM_LOGD("SWHDR Flare Ofset = %d %d %d \n",rOutput.u1FlareOffset[0], rOutput.u1FlareOffset[1], rOutput.u1FlareOffset[2]);
        CAM_LOGD("SWHDR Gain Diff = %d %d\n",rOutput.u4FinalGainDiff[0], rOutput.u4FinalGainDiff[1]);
#endif
