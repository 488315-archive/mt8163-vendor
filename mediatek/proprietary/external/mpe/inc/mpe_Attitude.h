...
/*
 * 1) only 2-DOF rotation (without ?œazimuth??
 * 2) confuse by linear acceleration and gravity
 * 3) delayed response
 */
...
typedef struct MPE_OUT {
  float orientation[3];
  float rotationVector[3];
  float linearAcceleration[3];
  float gravity[3];
  float global_acceleration[3];
  float rotationM[9];
  float acc_static_level;
  float gyro_static_level;
}MPE_OUT, *LPMPE_OUT;
...
/*
 * Returns heading of the device for compass-like application.
 *
 * The value of compass_heading has tilt-compensated feature
 * And the way to compensate the tilt follows definition of Microsoft
 * The three elements of compass heading angles are [heading, 0, 0].
 *
 * @param angles a 3-element array contains the result of compass heading
 * @return 0 on success, < 0 on failure
 */
int mpe_get_compass_heading(float *angles);
int mpe_get_rotation_matrix(float *matrix);
int mpe_get_global_acceleration(float *acceleration);
int mpe_get_mpe_out(LPMPE_OUT pmpe_out);
/*
 * Set the sensor fusion mode
 *
 * The sensor fusion algorithm can compute the posture information based
 * on the different combination of sensors. Currently we support four
 * combinations:
 *
 * 0. Accelerometer + Gyroscope + Magnetometer (default)
 * 1. Accelerometer
...
int mpe_get_fusion_mode();
/*
 * Set a yaw angle as the center
 *
 * @param yaw A yaw angle degree used as the center to the user
 */
void mpe_set_center_yaw_angle(float yaw);
/*
 * Get the version of this library
 *
 * @param version a string contains version information
 * @param len the length of first parameter
 */
int get_version(char *version, unsigned int len);
/*
 * Get build time of this library
 *
 * @param buildate a string contains the build time
 * @param len the length of first parameter
 */
int get_build_date(char *buildate, unsigned int len);
int mpe_re_initialize(void);
int mpe_get_magnetic_bias(float *mbias);
