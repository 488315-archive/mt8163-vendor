...
enum {
    ...
    TRANSACTION_backupToBinRegionAll,
...
public:
    ...
    virtual int backupToBinRegionAll()=0;
};
...
public:
	...
	int backupToBinRegionAll()
    {
        Parcel data,reply;
        data.writeInterfaceToken(INvRAMAgent::getInterfaceDescriptor());
        remote()->transact(TRANSACTION_backupToBinRegionAll,data,&reply);

        int ret = reply.readInt32();
        if (ret < 0)
        {
            NVRAM_LOG("backupToBinRegionAll Error.\n");
            return 0;
        }
...
public:
    ...
    virtual int backupToBinRegionAll();
};
