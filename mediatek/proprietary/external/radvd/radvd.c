...
#define RADVD_CHIPTEST_CONF "/etc/radvd/radvd_chiptest.conf"
#endif
...
main(int argc, char *argv[])
{
    ...
    if (!property_get("persist.chiptest.enable", prop_value, NULL)) {
        flog(LOG_INFO, "get persist.chiptest.enable failed!");
        conf_file = PATH_RADVD_CONF;
    }else {
        if(1 == atoi(prop_value)) {
            flog(LOG_INFO, "get persist.chiptest.enable success,is dongle mode!");
            conf_file = RADVD_CHIPTEST_CONF;
            signal(SIGUSR2, SIG_IGN);
        }else {
            flog(LOG_INFO, "get persist.chiptest.enable success, is not dongle mode!!");
            conf_file = PATH_RADVD_CONF;
        }
    }
#endif
