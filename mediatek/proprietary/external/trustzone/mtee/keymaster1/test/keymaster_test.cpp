...
public:
    UniqueKey(keymaster1_device_t** dev, uint8_t* bytes, size_t length) :
            UniqueBlob(bytes, length), mDevice(dev) {
    }

    ~UniqueKey() {
        if (mDevice != NULL && *mDevice != NULL) {
            keymaster1_device_t* dev = *mDevice;
...
public:
    static void SetUpTestCase() {
        ...
        std::cout << "Using keymaster module: ";

        ASSERT_EQ(0, CreateDevice(&sDevice))
    ...
    static void TearDownTestCase() {
        ASSERT_EQ(0, keymaster1_close(sDevice));
    }

protected:
    static keymaster1_device_t* sDevice;
	static int CreateDevice(keymaster1_device_t **dev) {
        std::cerr << "Creating hardware device" << std::endl;
	    ...
	    if ((*dev)->common.module->module_api_version >= KEYMASTER_MODULE_API_VERSION_1_0) {
			ALOGE("open keymaster device in %s (%s)",
	            	KEYSTORE_KEYMASTER, strerror(-rc));
	        return 0;
	    }
		return 0;
...
keymaster1_device_t* KeymasterBaseTest::sDevice = NULL;
...
INSTANTIATE_TEST_CASE_P(EC,
                        ...
                        ::testing::Values(224U, 256U, 384U, 521U));
...
TEST_F(KeymasterTest, GenerateKeyPair_UnknownType_Failure) {
    ...
    uint8_t* key_blob;
    size_t key_blob_length;
	uint32_t param[3] = {0, 0 , 0};
    ASSERT_NE(0,
            sDevice->generate_keypair(sDevice, key_type, (const void*)param, &key_blob, &key_blob_length))
