...
/*
 * height offset of logo. By Bright.L yongyida 2017.1 
 */
...
void fill_animation_logo(int index, void *fill_addr, void * dec_logo_addr, void * logo_addr, LCM_SCREEN_T phical_screen)
{
    ...
    /// Modified by: Bright.L yongyida 2017.1 @{
    if (phical_screen.rotation == 90) {
        hOffset = phical_screen.allignWidth - phical_screen.width;
        SLOGD("[show_animation_common: %s %d]hOffset = %d\n",__FUNCTION__,__LINE__, hOffset);
    } else {
        hOffset = 0;
    }
    RECT_REGION_T rect = {0, 0, logo_width, logo_height + hOffset};
    /// by: Bright.L @}
