        ...
        case AT_SET_OP:
            ...
            else if(actionID == 2)//action: SW READ
            {
                ...
                if (RegBase < 0xFFFF0000) {
                        ...
                        client.getTuningField(module, field, &value);

                        sprintf(response, "%08x\r\n", value);
                        ALOGD("[PQ_CMD] SR: %d:%d = 0x%x(%d)", module, field, (unsigned int)(value), value);
                    #else
                        sprintf(response, "%08x\r\n", 0);
                        ALOGD("[PQ_CMD] Not support tuning");
                    #endif
                } else {
                    DISP_READ_REG params;
                    ...
                    params.mask = 0xFFFFFFFF;
                        ...
                        if (params.reg >= SWREG_PQDS_DS_EN && params.reg < SWREG_PQDS_END) {
                            client.getTuningField(IPQService::MOD_DYNAMIC_SHARPNESS, RegBase - SWREG_PQDS_DS_EN, &value);
                            params.val = value;
                        }
                        else if (params.reg >= SWREG_TDSHP_REG_GAIN_MID && params.reg < SWREG_TDSHP_REG_END) {
                            client.getTuningField(IPQService::MOD_TDSHP_REG, RegBase- SWREG_TDSHP_REG_GAIN_MID, &value);
                            params.val = value;
                        }
                        else
                    #endif
            ...
            else if(actionID == 3)//action: SW WRITE
            {
                ...
                if (RegBase < 0xFFFF0000) {
                        ...
                        PQClient &client(PQClient::getInstance());
                        client.setTuningField(module, field, value);

                        sprintf(response, "%08x\r\n", value);
                        ALOGD("[PQ_CMD] SW: %d:%d = 0x%x(%d)", module, field, (unsigned int)(value), value);
                    #else
                        sprintf(response, "%08x\r\n", 0);
                        ALOGD("[PQ_CMD] Not support tuning");
                    #endif
                    ...
                    params.mask = 0xFFFFFFFF;
                        ...
                        PQClient &client(PQClient::getInstance());
                        if (params.reg >= SWREG_PQDS_DS_EN && params.reg < SWREG_PQDS_END) {
                            client.setTuningField(IPQService::MOD_DYNAMIC_SHARPNESS, RegBase - SWREG_PQDS_DS_EN, value);
                        }
                        else if (params.reg >= SWREG_TDSHP_REG_GAIN_MID && params.reg < SWREG_TDSHP_REG_END) {
                            client.setTuningField(IPQService::MOD_TDSHP_REG, RegBase- SWREG_TDSHP_REG_GAIN_MID, value);
                        }
                        else if (params.reg == SWREG_TDSHP_TUNING_MODE) {
                            client.setTDSHPFlag(params.val);
                        }
                        else
                    #endif
