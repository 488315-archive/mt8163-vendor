...
bool isCdmaLteDcSupport() {
    ...
    LOGATCI(LOG_DEBUG, "isSvlteSupport: %d", isSvlteSupport);

    property_get("ro.mtk_srlte_support", property_value, "0");
    isSrlteSupport = atoi(property_value);
    LOGATCI(LOG_DEBUG, "isSrlteSupport: %d", isSrlteSupport);
...
////
        ...
        if (isMsgCmdInput && !((int)buffer[0] == CTRL_CHAR_LF && readCount == 1)) {
