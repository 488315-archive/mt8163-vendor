                        ...
                        if (!mServerName.equals("")) {
                            ...
                            for (int i = 0; i < TelephonyManager.getDefault().getSimCount(); i++) {
                                ...
                                if (enabled != null && enabled.length() > 0 &&
                                        !"0".equals(enabled)) {
                                    // Disable VSIM and reset modem if VSIM has been enabled and
                                    // socket disconnect.
                                    // FIXME: User might not excepted recover to physical
                                    // SIM automatically?
                                    ...
                                    log("Disable VSIM and reset modem since socket disconnected.");
                                    return;
                                }
                            }
                            log("Socket disconnected and vsim is disabled.");
                        }
                    } catch (IOException e2) {
            ...
            switch (type) {
                ...
                case ExternalSimConstants.REQUEST_TYPE_PLUG_IN: {
                    ...
                    if (slotId == mCPhoneId
                        ...
                        // FIXME: capability switch might not trigger modem reset in some project.
                        // set result according to sub ready state
