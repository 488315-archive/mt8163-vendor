...
public class DefaultServiceStateExt implements IServiceStateExt {
    ...
    */
    ...
    private IccCardConstants.CardType getCdmaCardType(int slotId) {
        if (slotId < 0 || slotId >= PROPERTY_RIL_CDMA_CARD_TYPE.length) {
            log("getCdmaCardType: invalid slotId " + slotId);
            return null;
        }
        ...
        log("getCdmaCardType result: " + result + "  mCdmaCardType: " + mCdmaCardType);
     ...
     */
    public String updateOpAlphaLongForHK(String oriOperatorLong, String numeric, int slotId)  {
        boolean cdmaLteSupport = isCdmaLteDcSupport();
        log("oriOperatorLong= " + oriOperatorLong + " numeric= " + numeric + ", cdmaLteSupport = "
            + cdmaLteSupport + ", slotId = " + slotId);

        if (cdmaLteSupport) {
            String strOperatorOverride = "";

            if (numeric != null && ((numeric.equals("45403")) || (numeric.equals("45404")))) {
                IccCardConstants.CardType cardType = IccCardConstants.CardType.CARD_NOT_INSERTED;
                cardType = getCdmaCardType(slotId);
                boolean isCdma4GSim = cardType == IccCardConstants.CardType.CT_4G_UICC_CARD;
