...
/**
 * This class is used to implement {@link RscpRscSensor} callbacks.
 */
public class RscpRscSensorCallback extends ServerBaseCallback {
    private static final boolean DBG = true;
    private static final String TAG = "RscpRscSensorCallback";
    ...
    void dispatchCharacteristicReadRequest(BluetoothDevice device, int requestId, int offset,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_RSCS)) {
             if (charUuid.equals(GattUuid.CHAR_RSC_FEATURE)) {
                base = new RscFeature(characteristic.getValue(), characteristic);
                this.onRscsRscFeatureReadRequest(
                        device, requestId, offset, (RscFeature) base);
                return;
            } else if (charUuid.equals(GattUuid.CHAR_SENSOR_LOCATION)) {
                base = new SensorLocation(characteristic.getValue(), characteristic);
                this.onRscsSensorLocationReadRequest(
    ...
    void dispatchCharacteristicWriteRequest(BluetoothDevice device, int requestId,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_RSCS)) {
             if (charUuid.equals(GattUuid.CHAR_SC_CONTROL_POINT)) {
                ...
                this.onRscsScControlPointWriteRequest(
    ...
    void dispatchDescriptorWriteRequest(BluetoothDevice device, int requestId,
        ...
        if (srvcUuid.equals(GattUuid.SRVC_RSCS)) {
             if (charUuid.equals(GattUuid.CHAR_RSC_MEASUREMENT) &&
                    descrUuid.equals(GattUuid.DESCR_CLIENT_CHARACTERISTIC_CONFIGURATION)) {
                this.onRscsRscMeasurementCccdWriteRequest(
                        device, requestId, descriptor, preparedWrite, responseNeeded, offset,
                        value);
                return;
            } else if (charUuid.equals(GattUuid.CHAR_SC_CONTROL_POINT) &&
                    descrUuid.equals(GattUuid.DESCR_CLIENT_CHARACTERISTIC_CONFIGURATION)) {
                this.onRscsScControlPointCccdWriteRequest(
                        device, requestId, descriptor, preparedWrite, responseNeeded, offset,
                        value);
                return;
            }
    ...
    /**
     * A remote client has requested to read Rscs:RscFeature
     * local characteristic.
     *
     * @param device The remote device that has requested the read operation
     * @param requestId The Id of the request
     * @param offset Offset into the value of the characteristic
     * @param rscFeature Characteristic to be read
     */
    public void onRscsRscFeatureReadRequest(
            BluetoothDevice device, int requestId, int offset,
            RscFeature rscFeature) {
        if (DBG) {
            Log.d(TAG, "onRscsRscFeatureReadRequest(): offset=" + offset);
        }

        sendResponse(device, requestId, offset,
                rscFeature.getValue(offset));
    }

    /**
     * A remote client has requested to read Rscs:SensorLocation
     * local characteristic.
     *
     * @param device The remote device that has requested the read operation
     * @param requestId The Id of the request
     ...
     */
    public void onRscsSensorLocationReadRequest(
            BluetoothDevice device, int requestId, int offset,
            SensorLocation sensorLocation) {
        if (DBG) {
            Log.d(TAG, "onRscsSensorLocationReadRequest(): offset=" + offset);
        }
    ...
    /**
     * A remote client has requested to write to Rscs:ScControlPoint
     * local characteristic.
     *
     ...
     */
    public void onRscsScControlPointWriteRequest(
            BluetoothDevice device, int requestId,
            ScControlPoint scControlPoint,
            boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        if (DBG) {
            Log.d(TAG, "onRscsScControlPointWriteRequest()");
        }
    ...
    /**
     * A remote client has requested to read Rscs:RscMeasurement:
     ...
     */
    public void onRscsRscMeasurementCccdReadRequest(
            BluetoothDevice device, int requestId, int offset, BluetoothGattDescriptor descriptor) {
        if (DBG) {
            Log.d(TAG, "onRscsRscMeasurementCccdReadRequest()");
        }
    ...
    /**
     * A remote client has requested to read Rscs:ScControlPoint:
     * Cccd local descriptor.
     *
     * @param device The remote device that has requested the read operation
     * @param requestId The Id of the request
     ...
     * @param descriptor Descriptor to be read
     */
    public void onRscsScControlPointCccdReadRequest(
            BluetoothDevice device, int requestId, int offset, BluetoothGattDescriptor descriptor) {
        if (DBG) {
            Log.d(TAG, "onRscsScControlPointCccdReadRequest()");
        }
    ...
    /**
     * A remote client has requested to write Rscs:RscMeasurement:
     * Cccd local descriptor.
     *
     * @param device The remote device that has requested the write operation
     * @param requestId The Id of the request
     ...
     */
    public void onRscsRscMeasurementCccdWriteRequest(
            BluetoothDevice device, int requestId, BluetoothGattDescriptor descriptor,
            boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        if (DBG) {
            Log.d(TAG, "onRscsRscMeasurementCccdWriteRequest()");
        }
    ...
    /**
     * A remote client has requested to write Rscs:ScControlPoint:
     * Cccd local descriptor.
     *
     * @param device The remote device that has requested the write operation
     * @param requestId The Id of the request
     ...
     * @param value The value the client wants to assign to the descriptor
     */
    public void onRscsScControlPointCccdWriteRequest(
            BluetoothDevice device, int requestId, BluetoothGattDescriptor descriptor,
            boolean preparedWrite, boolean responseNeeded, int offset, byte[] value) {
        if (DBG) {
            Log.d(TAG, "onRscsScControlPointCccdWriteRequest()");
        }
