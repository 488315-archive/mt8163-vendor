...
status_t SensorHubManager::assertStateLocked()
{
    if (mServer == NULL) {
        ...
        if (validContext(SensorContext::Activity::CONTEXT_TYPE)) {
            ...
            mDataList.add(SensorContext::Activity::DATA_INDEX_WALKING);
            mDataList.add(SensorContext::Activity::DATA_INDEX_RUNNING);
            mDataList.add(SensorContext::Activity::DATA_INDEX_CLIMBING);
        }
        ...
        if (validContext(SensorContext::NoiseLevel::CONTEXT_TYPE)) {
            mDataList.add(SensorContext::NoiseLevel::DATA_INDEX_NOISELEVEL_VALUE);
            mDataList.add(SensorContext::NoiseLevel::DATA_INDEX_TIMESTAMP);
        }
        if (validContext(SensorContext::FreeFall::CONTEXT_TYPE)) {
            mDataList.add(SensorContext::FreeFall::DATA_INDEX_FREEFALL_VALUE);
            mDataList.add(SensorContext::FreeFall::DATA_INDEX_TIMESTAMP);
        }
        if (validContext(SensorContext::Tap::CONTEXT_TYPE)) {
            mDataList.add(SensorContext::Tap::DATA_INDEX_TAP_VALUE);
            mDataList.add(SensorContext::Tap::DATA_INDEX_TIMESTAMP);
        }
        if (validContext(SensorContext::Twist::CONTEXT_TYPE)) {
           mDataList.add(SensorContext::Twist::DATA_INDEX_TWIST_VALUE);
           mDataList.add(SensorContext::Twist::DATA_INDEX_TIMESTAMP);
        }
        if (validContext(SensorContext::Snapshot::CONTEXT_TYPE)) {
            mDataList.add(SensorContext::Snapshot::DATA_INDEX_SNAPSHOT_VALUE);
            mDataList.add(SensorContext::Snapshot::DATA_INDEX_TIMESTAMP);
        }
        if (validContext(SensorContext::PDR::CONTEXT_TYPE)) {
           mDataList.add(SensorContext::PDR::DATA_INDEX_PDR_X);
           mDataList.add(SensorContext::PDR::DATA_INDEX_PDR_Y);
           mDataList.add(SensorContext::PDR::DATA_INDEX_PDR_Z);
           mDataList.add(SensorContext::PDR::DATA_INDEX_TIMESTAMP);
        }
    }
    return NO_ERROR;
}
...
void SensorHubManager::addConGesture(int gesture, int cgesture)
{
    Mutex::Autolock _l(mLock);
    if (assertStateLocked() == NO_ERROR) {
        ALOGV("SensorhubManager addConGesture: gesture=%d, cgesture=%d", gesture, cgesture);
        mServer->addConGesture(gesture, cgesture);
    }
    //ALOGV("addConGesture: gesture=%d, cgesture=%d", gesture, cgesture);
}

void SensorHubManager::cancelConGesture(int gesture , int cgesture)
{
    Mutex::Autolock _l(mLock);
    if (assertStateLocked() == NO_ERROR) {
        ALOGV("SensorhubManager cancelConGesture: gesture=%d, cgesture=%d", gesture, cgesture);
        mServer->cancelConGesture(gesture, cgesture);
    }
    //ALOGV("cancelConGesture: gesture=%d, cgesture=%d", gesture, cgesture);
}
