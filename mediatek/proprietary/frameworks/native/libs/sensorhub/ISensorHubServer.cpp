...
enum {
    ...
    ADD_CONFIGURABLE_GESTURE,
    CANCEL_CONFIGURABLE_GESTURE,
};
...
public:
    ...
    virtual void addConGesture(int gesture, int cgesture)
    {
        Parcel data, reply;
        data.writeInterfaceToken(ISensorHubServer::getInterfaceDescriptor());
        data.writeInt32(gesture);
        data.writeInt32(cgesture);
        remote()->transact(ADD_CONFIGURABLE_GESTURE, data, &reply);
    }

    virtual void cancelConGesture(int gesture, int cgesture)
    {
        Parcel data, reply;
        data.writeInterfaceToken(ISensorHubServer::getInterfaceDescriptor());
        data.writeInt32(gesture);
        data.writeInt32(cgesture);
        remote()->transact(CANCEL_CONFIGURABLE_GESTURE, data, &reply);
    }
};
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        case ADD_CONFIGURABLE_GESTURE: {
            CHECK_INTERFACE(ISensorHubServer, data, reply);
            int gesture = data.readInt32();
            int cgesture = data.readInt32();
            addConGesture(gesture, cgesture);
            return NO_ERROR;
        } break;
        case CANCEL_CONFIGURABLE_GESTURE: {
            CHECK_INTERFACE(ISensorHubServer, data, reply);
            int gesture = data.readInt32();
            int cgesture = data.readInt32();
            cancelConGesture(gesture, cgesture);
            return NO_ERROR;
        } break;
        default:
