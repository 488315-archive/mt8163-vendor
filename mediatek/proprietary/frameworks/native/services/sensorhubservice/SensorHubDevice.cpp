...
SensorHubDevice::SensorHubDevice()
{
    ...
    //shf_open();
    status_t result = shf_open();
    if (result < 0) {
        ALOGE("HAL shf_open fail!"); ;
    }
    ...
    uint8_t contextsdata[] = { //should sync with shf_define.h
        ...
        SHF_DATA_INDEX_NOISE_LEVEL_VALUE,
        SHF_DATA_INDEX_FREE_FALL_VALUE,
        SHF_DATA_INDEX_TAP_VALUE,
        SHF_DATA_INDEX_TWIST_VALUE,
        SHF_DATA_INDEX_SNAPSHOT_VALUE,
        SHF_DATA_INDEX_PDR_X,
    };
    uint32_t contexts[] = { //should sync with sensors.h
        ...
        SensorContext::NoiseLevel::CONTEXT_TYPE,
        SensorContext::FreeFall::CONTEXT_TYPE,
        SensorContext::Tap::CONTEXT_TYPE,
        SensorContext::Twist::CONTEXT_TYPE,
        SensorContext::Snapshot::CONTEXT_TYPE,
        SensorContext::PDR::CONTEXT_TYPE,
    };
    ...
    uint8_t datas[] = {
        ...
        */
        ...
        SHF_DATA_INDEX_NOISE_LEVEL_VALUE,
        SHF_DATA_INDEX_NOISE_LEVEL_TIME,

        SHF_DATA_INDEX_FREE_FALL_VALUE,
        SHF_DATA_INDEX_FREE_FALL_TIME,

        SHF_DATA_INDEX_TAP_VALUE,
        SHF_DATA_INDEX_TAP_TIME,

        SHF_DATA_INDEX_TWIST_VALUE,
        SHF_DATA_INDEX_TWIST_TIME,

        SHF_DATA_INDEX_SNAPSHOT_VALUE,
        SHF_DATA_INDEX_SNAPSHOT_TIME,

        SHF_DATA_INDEX_PDR_X,
        SHF_DATA_INDEX_PDR_Y,
        SHF_DATA_INDEX_PDR_Z,
        SHF_DATA_INDEX_PDR_TIME,
    };
...
void SensorHubDevice::addConGesture(uint8_t gesture, uint8_t cgesture)
{
    ALOGV("SensorHubDevice addConGesture gesture=%d, cgesture=%d", gesture, cgesture);
    shf_add_con_gesture(gesture, cgesture);
}

void SensorHubDevice::cancelConGesture(uint8_t gesture, uint8_t cgesture)
{
    ALOGV("SensorHubDevice cancelConGesture gesture=%d, cgesture=%d", gesture, cgesture);
    shf_cancel_con_gesture(gesture, cgesture);
}
